...
function planProvenance(rows){
  const first=rows[0]||{};return{
    sourceKind:'FINANCE_PAYMENT_PLAN_MATERIALIZED',sourceRecordId:s(first?.id)||null,proposalRecordId:s(first?.proposal_record_id)||null,
...
export async function buildPaymentScheduleAuthority(sql,{paymentPlan=null,incomingPayments=[],incomingPaymentAllocations=[],dealAllocationTotals=[],dealFinanceSummaries=[]}={}){
  ...
  // Fully paid obligations need no future-plan inference: Finance summary + exact bank/allocation proof is sufficient.
