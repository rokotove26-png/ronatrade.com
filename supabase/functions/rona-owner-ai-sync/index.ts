...
// Runtime business reads remain in runtime.ts. Owner Payments V4 enriches only the
// authenticated Admin payment read model and consumes existing upstream handoff state read-only.

import postgres from 'postgres';
import { enrichOwnerPaymentsCanonicalV4 } from './owner-payments-canonical-v4.ts';

const nativeServe = Deno.serve.bind(Deno);
const DB = Deno.env.get('SUPABASE_DB_URL');
const v4sql = DB ? postgres(DB,{prepare:false,max:1,idle_timeout:1,max_lifetime:30,connect_timeout:5}) : null;
...
nativeServe(async (req) => {
  ...
  if (normalizedPath !== '/admin/sync' || !response.ok || !v4sql) return response;
  const payload = await response.json().catch(()=>null);
  if (!payload?.data?.financeFragment) return new Response(JSON.stringify({ok:false,code:'OWNER_PAYMENTS_V4_FINANCE_FRAGMENT_MISSING'}),{status:502,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}});
  payload.data.financeFragment = await enrichOwnerPaymentsCanonicalV4(payload.data.financeFragment,v4sql);
