...
import { buildOperationsCenter, enrichOperationsDeals } from "./operations-center.ts";
...
async function adminSnapshot() {
  ...
  const dealRows = await sql`
    select d.id deal_key,d.deal_id,d.business_status,d.finance_status::text,d.accounting_closure_status::text,
           d.lifecycle_state::text,d.updated_at deal_updated_at,cl.client_id,cl.legal_name,ct.contract_id,
           ct.contract_status,ct.signed_at,ct.signed_contract_confirmed_at,ct.current_signed_document_id,
           coalesce(w.payment_handoff_state,'NOT_SENT') payment_handoff_state,w.payment_handoff_at,
           app.application_id,app.product,app.delivery_basis,app.destination,aw.supplier_approved_at,
           shp.shipment_id,shp.shipment_status::text delivery_status,shp.updated_at shipment_updated_at,
           coalesce(doc_stats.document_count,0)::integer document_count,
           coalesce(doc_stats.unchecked_document_count,0)::integer unchecked_document_count
    ...
    left join lateral (
      select a.id,a.application_id,a.product,a.delivery_basis,a.destination
      from portal_private.client_applications a
      where a.linked_deal_key=d.id and a.lifecycle_state<>'ARCHIVED'::portal_private.lifecycle_state_enum
      order by a.updated_at desc limit 1
    ) app on true
    left join portal_private.owner_application_workflow aw on aw.application_key=app.id
    left join lateral (
      select s.shipment_id,s.shipment_status,s.updated_at
      from portal_private.shipments s
      where s.deal_key=d.id and s.lifecycle_state<>'ARCHIVED'::portal_private.lifecycle_state_enum
      order by s.updated_at desc limit 1
    ) shp on true
    left join lateral (
      select count(*) document_count,count(*) filter(where odd.checked_by_admin=false) unchecked_document_count
      from portal_private.owner_deal_documents odd
      join portal_private.documents doc on doc.id=odd.document_key
      where odd.deal_key=d.id and doc.lifecycle_state='ACTIVE'::portal_private.lifecycle_state_enum
    ) doc_stats on true
    where d.lifecycle_state<>'ARCHIVED'::portal_private.lifecycle_state_enum
    order by d.updated_at desc`;
  const deals = enrichOperationsDeals(dealRows);
  ...
  const operations = await buildOperationsCenter(sql);
  ...
  return {
    ...
    operationalConflicts, radio, analytics, news, operations
  };
}
...
async function reviewOwnerDocument(ctx,req,documentId){
  const body=await jsonBody(req),checked=body.checked===undefined?true:body.checked===true;
  return sql.begin(async tx=>{
    const rows=await tx`
      update portal_private.owner_deal_documents odd
      set checked_by_admin=${checked},checked_at=case when ${checked} then now() else null end,
          checked_by=case when ${checked} then ${ctx.userId}::uuid else null end,updated_at=now()
      from portal_private.documents doc,portal_private.deals d
      where doc.id=odd.document_key and d.id=odd.deal_key and doc.document_id=${documentId}
        and doc.lifecycle_state='ACTIVE'::portal_private.lifecycle_state_enum
      returning odd.id,d.deal_id,doc.document_id,odd.document_kind,odd.checked_by_admin,odd.checked_at`;
    if(!rows.length)throw Object.assign(new Error('DOCUMENT_NOT_FOUND'),{status:404});
    await audit(tx,ctx,checked?'OWNER_DOCUMENT_REVIEWED':'OWNER_DOCUMENT_REVIEW_CLEARED','DOCUMENT',documentId,req,{dealId:rows[0].deal_id,documentKind:rows[0].document_kind});
    return rows[0];
  });
}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    m=path.match(/^\/admin\/documents\/([^/]+)\/review$/);if(m&&method==='POST'){requireRole(ctx,'ADMIN');return send(200,{ok:true,data:await reviewOwnerDocument(ctx,req,decodeURIComponent(m[1]))})}
