...
async function uploadSignedAddendum(c:Ctx,req:Request,dealId:string){
  ...
  try{
    await sql.begin(async(tx:any)=>{
      ...
      await tx`insert into portal_private.owner_deal_workflow(deal_key,payment_handoff_state,payment_handoff_at,payment_handoff_by,signed_supplement_document_key,updated_at) values(${deal.deal_key}::uuid,'READY',now(),${c.user}::uuid,${documentKey}::uuid,now()) on conflict(deal_key) do update set payment_handoff_state='READY',payment_handoff_at=now(),payment_handoff_by=${c.user}::uuid,signed_supplement_document_key=${documentKey}::uuid,updated_at=now()`;
