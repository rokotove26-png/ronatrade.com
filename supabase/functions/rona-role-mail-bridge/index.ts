...
async function triggerFinanceCashStatementIntake(mailbox:string,uid:number){
  if(mailbox!=="finance@ronaoil.com")return null;
  const token=await runtimeToken();
  if(!token)throw new Error("mail bridge runtime token missing");
