...
function pathOf(req) {
  ...
  const marker = "/rona-admin-control-plane";
...
function validatePassword(v) {
  ...
  if (p.length < 10 || !/[a-zа-яё]/i.test(p) || !/[A-ZА-ЯЁ]/.test(p) || !/[0-9]/.test(p) || !/[^A-Za-zА-Яа-яЁё0-9]/.test(p)) {
    throw Object.assign(new Error("PASSWORD_POLICY_FAILED"), { status: 400 });
  }
  return p;
}
  ...
  const rows = await sql`
    ...
    where u.id=${userId}::uuid
      and not exists(
        ...
        where r.user_id=u.id and r.role='ADMIN'::portal_private.portal_role_enum
          and r.status='ACTIVE'::portal_private.binding_status_enum and r.revoked_at is null
      )
    limit 1
  `;
    ...
    where ct.contract_id=${contractId}
      ...
      and exists(
        ...
        where ae.action='BILATERAL_CONTRACT_ATTACH_CONFIRMED'
          and ae.entity_type='CONTRACT' and ae.entity_id=ct.contract_id
          and ae.actor_user_id=ct.signed_contract_confirmed_by
...
async function accessSnapshot() {
  const users = await sql`
    ...
    where left(coalesce(u.source_system,''),3)<>'QA_'
      ...
      and exists(
        select 1 from portal_private.portal_user_roles xr
        where xr.user_id=u.id and xr.role in ('CLIENT'::portal_private.portal_role_enum,'AGENT'::portal_private.portal_role_enum)
          and xr.status='ACTIVE'::portal_private.binding_status_enum and xr.revoked_at is null
      )
      and not exists(
        select 1 from portal_private.portal_user_roles ar
        where ar.user_id=u.id and ar.role='ADMIN'::portal_private.portal_role_enum
          and ar.status='ACTIVE'::portal_private.binding_status_enum and ar.revoked_at is null
      )
  ...
  const clientBindings = await sql`
  ...
  `;
  for (const b of clientBindings) {
    const u = byUser.get(String(b.user_id));
    if (!u) continue;
    u.bindings.push({
      id:String(b.id),company:String(b.legal_name),clientId:String(b.client_id),contractId:String(b.contract_id),
      status:String(b.status),role:"Уполномоченный представитель",dealScopeMode:String(b.deal_scope_mode || "")
    });
  }
  const agentBindings = await sql`
    select b.id,b.user_id,b.status::text,ap.agent_person_id,ap.display_alias,ale.agent_legal_entity_id,ale.legal_name
  ...
  for (const b of agentBindings) {
    ...
    if (!u) continue;
    u.bindings.push({
      id:String(b.id),company:String(b.legal_name),clientId:String(b.agent_legal_entity_id),contractId:String(b.agent_person_id),
      status:String(b.status),role:"Агент",dealScopeMode:"AGENT_FIXED_SCOPE"
    });
  }
...
async function availableProfiles() {
  ...
  for (const r of rows) {
    ...
    if (!byPerson.has(id)) byPerson.set(id, {
      agentPersonKey:String(r.agent_person_key),agentPersonId:id,
      displayAlias:String(r.display_alias || r.full_name || id),fullName:r.full_name?String(r.full_name):null,
      legalEntities:new Map(),
    });
    byPerson.get(id).legalEntities.set(String(r.agent_legal_entity_id), {
      agentLegalEntityKey:String(r.agent_legal_entity_key),agentLegalEntityId:String(r.agent_legal_entity_id),legalName:String(r.legal_name)
    });
  }
...
async function resolveAgentProfile(name, explicitScope) {
  ...
  const matches = profiles.filter(p => scope ? p.agentPersonId === scope : [p.displayAlias,p.fullName].filter(Boolean).some(x=>normalizeName(x)===normalized));
...
async function createAuthUser(email, password, login, name, extra = {}) {
  const { data, error } = await service.auth.admin.createUser({
    ...
    user_metadata:{rona_portal_login:login,rona_portal_display_name:name,...extra},
  });
...
async function createClientUser(ctx, req, b) {
  const name=requiredText(b.name,"NAME",200),login=requiredText(b.login,"LOGIN",160),email=requiredText(b.email,"EMAIL",320).toLowerCase();
  const password=validatePassword(b.initialPassword);
  ...
  for (const id of requested) {
    const c = await bindingEligible(id);
    if (c) eligible.push(c); else pending.push(id);
  }
  ...
  try {
    const result = await sql.begin(async tx => {
      ...
      await tx`
        ...
        values(${userId}::uuid,${authId}::uuid,${login},${name},'ACTIVE'::portal_private.portal_user_status_enum,'ADMIN_PORTAL','ADMIN_EXCLUSIVE_CLIENT_V1',now(),'CONFIRMED'::portal_private.authority_state_enum,'ACTIVE'::portal_private.lifecycle_state_enum,now(),null,false,null,now())
      `;
      ...
      for (const c of eligible) {
        await tx`
          ...
          values(${userId}::uuid,${c.client_key}::uuid,${c.contract_key}::uuid,'ACTIVE'::portal_private.binding_status_enum,${ctx.user}::uuid,${`Admin Portal: ${String(b.bindingRole || "Authorized representative")}`},'ADMIN_PORTAL','ADMIN_EXCLUSIVE_CLIENT_V1',now(),'CONFIRMED'::portal_private.authority_state_enum,'ACTIVE'::portal_private.lifecycle_state_enum,'ALL_CONTRACT_DEALS')
        `;
      }
      await audit(tx,ctx,"CLIENT_PORTAL_USER_CREATED_BY_ADMIN","PORTAL_USER",userId,req,{login,email,requested_contract_ids:requested,linked_contract_ids:eligible.map(x=>String(x.contract_id)),pending_contract_ids:pending,password_admin_set:true});
      return {userId,linkedContractIds:eligible.map(x=>String(x.contract_id)),pendingContractIds:pending};
    });
    return result;
  } catch (e) {
...
async function createAgentUser(ctx, req, b) {
  ...
  const exists=await sql`
    select 1 from portal_private.agent_user_bindings ub
...
async function unblockUser(ctx,req,userId) {
  ...
  await sql.begin(async tx=>{
    ...
    const suspendedClients=await tx`select b.id,ct.contract_id from portal_private.client_user_bindings b join portal_private.contracts ct on ct.id=b.contract_key where b.user_id=${userId}::uuid and b.status='SUSPENDED'::portal_private.binding_status_enum and b.revoked_at is null`;
    for (const b of suspendedClients) {
...
async function forwardLegacy(req,path) {
  ...
  return fetch(`${LEGACY_AUTHORITY}${path}${url.search}`,init);
}

async function bootstrap(req) {
  const upstream=await forwardLegacy(req,"/bootstrap");
  ...
  const profiles=await availableProfiles();
  data.adminControlPlane={mode:"ADMIN_EXCLUSIVE",passwordOwner:"ADMINISTRATOR",accountCreationIndependentFromContract:true,agentProfiles:profiles.map(p=>({agentPersonId:p.agentPersonId,displayAlias:p.displayAlias,legalEntityIds:p.legalEntities.map(x=>x.agentLegalEntityId)}))};
  return {ok:true,data};
}
...
Deno.serve(async req=>{
  ...
  try {
    ...
    const password=path.match(/^\/access\/users\/([0-9a-f-]+)\/password$/i);
    if (password && req.method==="POST") return send(200,{ok:true,...await setPassword(ctx,req,password[1])});
