...
export async function clientBootstrap(c:Ctx){
  ...
  const contexts=adminEntity
    ?await sql`
      select distinct
        cl.client_id,cl.legal_name,cl.registration_country,cl.registered_address,
        null::text as contact_phone,
        ct.contract_id,ct.current_external_contract_number,ct.contract_status,ct.effective_from,ct.effective_to,
        ct.signed_contract_confirmed_at,ct.updated_at
    ...
    `
    :await sql`select distinct cl.client_id,cl.legal_name,cl.registration_country,cl.registered_address,bp.contact_phone,ct.contract_id,ct.current_external_contract_number,ct.contract_status,ct.effective_from,ct.effective_to,ct.signed_contract_confirmed_at,ct.updated_at from portal_private.client_user_bindings b join portal_private.clients cl on cl.id=b.client_key join portal_private.contracts ct on ct.id=b.contract_key left join portal_private.client_user_binding_profiles bp on bp.binding_id=b.id where b.user_id=${c.user}::uuid and (${bound}::uuid is null or b.client_key=${bound}::uuid) and portal_private.client_user_has_contract_access(${c.user}::uuid,ct.id,now()) order by cl.legal_name,ct.contract_id`;

  const priceAuthority=adminEntity
    ?await sql`
      select distinct on (ops.source_publication_item_key)
        ops.source_publication_item_key as publication_item_id,
        p.publication_id,p.status::text as publication_status,
        p.authority_state::text as publication_authority_state,p.lifecycle_state::text as publication_lifecycle_state,
        pi.authority_state::text as item_authority_state,pi.lifecycle_state::text as item_lifecycle_state,
        ops.product,ops.producer,ops.supplier,ops.basis as snapshot_basis,ops.final_station,
        ops.sale_price as snapshot_price,ops.currency as snapshot_currency,ops.payment_terms,ops.commercial_terms,
        ops.business_status,ops.publish_client,ops.client_published_at,ops.updated_at
      ...
      where ops.source_publication_item_key is not null
        and ops.business_status='PUBLISHED' and ops.publish_client=true
        ...
        and pi.item_type='PRICE'::portal_private.publication_item_type_enum and pi.distribution_allowed=true
        ...
        and (
          ...
          or exists(
            ...
            where pct.client_key=${bound}::uuid and pct.publication_key=p.id
        ...
        and (
          ...
          or exists(
            ...
            where b.user_id=${c.user}::uuid
              and (${bound}::uuid is null or b.client_key=${bound}::uuid)
  ...
  return{
    ...
    data_contract:adminEntity?"1.4-admin-entity-preview":"1.4",
    requires_context_selection:contexts.length>1,
    contexts,price_authority:priceAuthority,selected_context:null,
    applications:[] as unknown[],deals:[] as unknown[],documents:[] as unknown[],payments:[] as unknown[],
    shipments:[] as unknown[],rail_documents:[] as unknown[],market:[] as unknown[],notifications:[] as unknown[],
    admin_entity_preview:adminEntity
  };
}
...
export async function clientShipments(c:Ctx,clientId:string,contractId:string){
  ...
  const context=adminEntity
    ?await sql`select ct.id as contract_key,cl.id as client_key from portal_private.clients cl join portal_private.contracts ct on ct.client_key=cl.id where cl.client_id=${clientId} and ct.contract_id=${contractId} and cl.id=${bound}::uuid limit 1`
    :await sql`select ct.id as contract_key,cl.id as client_key from portal_private.clients cl join portal_private.contracts ct on ct.client_key=cl.id where cl.client_id=${clientId} and ct.contract_id=${contractId} and (${bound}::uuid is null or cl.id=${bound}::uuid) and portal_private.client_user_has_contract_access(${c.user}::uuid,ct.id,now()) limit 1`;
  if(context.length!==1)return null;
  return adminEntity
    ?await sql`select s.id as shipment_key,s.shipment_id,d.deal_id,s.shipment_status::text,s.origin_location,s.destination_location,s.planned_departure_at,s.actual_departure_at,s.planned_arrival_at,s.actual_arrival_at,s.closed_at from portal_private.shipments s join portal_private.deals d on d.id=s.deal_key join portal_private.clients cl on cl.id=d.client_key join portal_private.contracts ct on ct.id=d.contract_key where cl.id=${bound}::uuid and cl.client_id=${clientId} and ct.contract_id=${contractId} and s.client_key=cl.id order by s.created_at desc`
