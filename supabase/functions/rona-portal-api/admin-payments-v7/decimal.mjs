...
export function parseDecimal(value) {
  ...
  if (typeof value === 'number') {
    if (!Number.isSafeInteger(value)) {
      throw new TypeError('Binary-float money is forbidden; pass a decimal string');
    }
...
function align(a, b) {
  const x = parseDecimalLike(a);
  const y = parseDecimalLike(b);
  const scale = Math.max(x.scale, y.scale);
  return {
    a: x.coefficient * pow10(scale - x.scale),
    b: y.coefficient * pow10(scale - y.scale),
    scale,
  };
}
...
export function decimalAdd(a, b) {
  const x = align(a, b);
  return normalizeDecimal({ coefficient: x.a + x.b, scale: x.scale });
}
...
export function decimalAbs(a) {
  const x = parseDecimalLike(a);
  return { coefficient: x.coefficient < 0n ? -x.coefficient : x.coefficient, scale: x.scale };
}
...
export function decimalMulInteger(a, integer) {
  ...
  const x = parseDecimalLike(a);
  return normalizeDecimal({ coefficient: x.coefficient * BigInt(integer), scale: x.scale });
}
...
export function decimalToString(value) {
  const x = normalizeDecimal(parseDecimalLike(value));
  const negative = x.coefficient < 0n;
  const digits = (negative ? -x.coefficient : x.coefficient).toString();
  if (x.scale === 0) return `${negative ? '-' : ''}${digits}`;
  const padded = digits.padStart(x.scale + 1, '0');
  const cut = padded.length - x.scale;
  return `${negative ? '-' : ''}${padded.slice(0, cut)}.${padded.slice(cut)}`;
}

export function decimalSum(values) {
  let total = parseDecimal('0');
  for (const value of values) total = decimalAdd(total, value);
  return total;
}

export function decimalMin(a, b) {
  return decimalCompare(a, b) <= 0 ? parseDecimalLike(a) : parseDecimalLike(b);
}
