...
function rawExactLines(claim) {
  return (claim?.lines || []).filter((line) => upper(line.amount_status || 'EXACT') === 'EXACT');
}
...
export function validateAttributionIntegrity({ payment, claim, validDealKeys = [] }) {
  ...
  try {
    ...
    const lines = [...aggregate.entries()].map(([deal_key, amount]) => ({ deal_key, amount: canonicalDecimalString(amount), currency: upper(payment.currency), amount_status: 'EXACT' }))
    ...
    if (decimalCompare(allocatedAmount, canonicalDecimalString(payment.amount)) !== 0) {
      return { status: 'TO_VERIFY', reason: 'ATTRIBUTION_AMOUNT_COVERAGE_MISMATCH', lines: [], allocated_amount: allocatedAmount };
    }
...
function canonicalPhysicalLines(rows, paymentCurrency) {
  ...
  for (const row of rows) {
    ...
    const key = String(row.deal_key);
    const amount = canonicalDecimalString(row.amount);
    const previous = aggregate.get(key);
    aggregate.set(key, previous ? decimalToString(decimalAdd(previous, amount)) : amount);
  }
  return [...aggregate.entries()].map(([deal_key, amount]) => ({ deal_key, amount: canonicalDecimalString(amount), currency: upper(paymentCurrency) }))
    .sort((a, b) => a.deal_key.localeCompare(b.deal_key));
}
...
function sameCanonicalLines(authorityLines, physicalRows, paymentCurrency) {
  ...
  const authority = authorityLines.map((line) => ({ deal_key: String(line.deal_key), amount: canonicalDecimalString(line.amount), currency: upper(line.currency) }))
    .sort((a, b) => a.deal_key.localeCompare(b.deal_key));
...
function materializationStatus(payment, claim, canonicalLines, physicalAllocations) {
  ...
  if (current.length && sameCanonicalLines(canonicalLines, current, payment.currency)) {
    const staleAlsoPresent = physical.some((row) => row.current !== true && row.deal_key);
...
function reconciliationBase(payment) {
  return { payment_key: String(payment.payment_key), payment_id: payment.payment_id, owner_action_required: false, allowed_owner_actions: [], current_lines: [], scope_deal_keys: [], authority_refs: payment.authority_refs || [] };
}
...
export function reconcilePaymentEvent(payment, allClaims, physicalAllocations, capabilities = {}, validDealKeys = []) {
  ...
  if (upper(payment.kind) === 'FX_CONVERSION') {
    return { ...base, reconciliation_class: 'FX_CONVERSION_NOT_APPLICABLE', status: 'AUTHORITATIVE', reason: null, integrity_status: 'NOT_APPLICABLE', materialization_status: 'NOT_APPLICABLE', business_disposition: 'NOT_APPLICABLE', fee_attribution_state: null, current_authority_id: null };
  }
  ...
  if (resolved.status === 'TO_VERIFY') {
    ...
    return { ...base, reconciliation_class: 'AUTHORITY_CONFLICT', status: 'TO_VERIFY', reason: 'AUTHORITY_CONFLICT', integrity_status: 'TO_VERIFY', scope_deal_keys: candidateDealKeys, materialization_status: 'AUTHORITY_CONFLICT', business_disposition: null, fee_attribution_state: null, authority_refs: [...base.authority_refs, ...resolved.authority_refs], current_authority_id: null };
  }
  ...
  if (!claim) {
    ...
    const classification = capabilityPresent && !hasKnownCandidateScope ? 'GENUINELY_UNALLOCATED' : 'AUTHORITY_MATERIALIZATION_REQUIRED';
    return {
      ...base, reconciliation_class: classification, status: classification === 'GENUINELY_UNALLOCATED' ? 'AUTHORITATIVE' : 'TO_VERIFY',
      reason: classification === 'GENUINELY_UNALLOCATED' ? null : 'AUTHORITY_MATERIALIZATION_REQUIRED', integrity_status: 'NOT_APPLICABLE',
      ...
      scope_deal_keys: unique(payment.candidate_deal_ids || []), materialization_status: 'NO_CURRENT_BUSINESS_AUTHORITY', business_disposition: null,
      fee_attribution_state: null, current_authority_id: null,
    };
  }

  const disposition = upper(claim.disposition || claim.decision_type);
  if (disposition === 'ASSIGN_ADVANCE_PAYMENT') {
    return {
      ...base, reconciliation_class: 'RESOLVED', status: 'AUTHORITATIVE', reason: null, integrity_status: 'NOT_APPLICABLE',
      scope_deal_keys: [], materialization_status: 'NOT_APPLICABLE', business_disposition: 'ASSIGN_ADVANCE_PAYMENT', fee_attribution_state: null,
      authority_refs: [...base.authority_refs, ...resolved.authority_refs], current_authority_id: claim.id || null,
    };
  }

  const integrity = validateAttributionIntegrity({ payment, claim, validDealKeys });
  const hasExactLines = rawExactLines(claim).length > 0;
  if (hasExactLines && integrity.status !== 'AUTHORITATIVE') {
    return {
      ...base, reconciliation_class: 'ATTRIBUTION_INTEGRITY_ERROR', status: 'TO_VERIFY', reason: 'ATTRIBUTION_INTEGRITY_ERROR', integrity_status: 'TO_VERIFY',
      integrity_reason: integrity.reason, scope_deal_keys: unique([...(claim.scope_deal_keys || []), ...(claim.lines || []).map((line) => line.deal_key)]),
      materialization_status: 'NOT_EVALUATED_INTEGRITY_ERROR', business_disposition: disposition || null, fee_attribution_state: upper(claim.classification) === 'ASSOCIATED_BANK_FEE' ? 'TO_VERIFY' : null,
      authority_refs: [...base.authority_refs, ...resolved.authority_refs], current_authority_id: claim.id || null,
    };
  }

  const canonicalLines = integrity.status === 'AUTHORITATIVE' ? integrity.lines : [];
  const classified = classifyResolvedClaim(claim, canonicalLines);
  const materialization = materializationStatus(payment, claim, canonicalLines, physicalAllocations || []);
  ...
  if (classified.reconciliationClass === 'ASSOCIATED_BANK_FEE') {
    if (canonicalLines.length) feeAttributionState = 'EXACT';
    else if (upper(payment.allocation_applicability) === 'NOT_APPLICABLE') feeAttributionState = 'NOT_APPLICABLE';
    else if (classified.scope.length) feeAttributionState = 'SHARED_SCOPE';
    else feeAttributionState = 'TO_VERIFY';
  ...
  return {
    ...base, reconciliation_class: classified.reconciliationClass, status, reason,
    integrity_status: canonicalLines.length ? 'AUTHORITATIVE' : 'NOT_APPLICABLE', current_lines: canonicalLines,
    scope_deal_keys: classified.scope, materialization_status: materialization, business_disposition: classified.businessDisposition,
    principal_payment_key: claim.principal_payment_key || null, fee_attribution_state: feeAttributionState,
    authority_refs: [...base.authority_refs, ...resolved.authority_refs], current_authority_id: claim.id || null,
  };
}

export function reconcileAllPayments(payments, attributionClaims, physicalAllocations, capabilities, validDealKeys = []) {
  return (payments || []).map((payment) => reconcilePaymentEvent(payment, attributionClaims, physicalAllocations, capabilities, validDealKeys));
}

export function validateExactAllocationCoverage(payment, reconciliation) {
  if (reconciliation.integrity_status === 'AUTHORITATIVE') {
    let allocated = parseDecimal('0');
    for (const line of reconciliation.current_lines || []) allocated = decimalAdd(allocated, line.amount);
