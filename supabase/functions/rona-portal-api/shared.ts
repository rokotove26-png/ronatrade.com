...
export function isAdminEntityAgent(c:Ctx){return c.impersonation?.effectiveRole==="AGENT"&&c.impersonation.subjectMode==="ADMIN_ENTITY"&&Boolean(c.impersonation.targetAgentPersonKey)}
...
export async function authenticate(req:Request):Promise<Ctx|null>{
  ...
  const effective=impersonation.subjectMode==="ADMIN_ENTITY"&&impersonation.targetClientKey
    ...
    :impersonation.subjectMode==="ADMIN_ENTITY"&&impersonation.targetAgentPersonKey
      ?await sql`select coalesce(display_alias,full_name,agent_person_id) as display_name from portal_private.agent_persons where id=${impersonation.targetAgentPersonKey}::uuid limit 1`
...
export async function sessionScope(c:Ctx){
  ...
  if(c.roles.includes("AGENT")){
    ...
    const rows=isAdminEntityAgent(c)
      ?await sql`select distinct ap.agent_person_id,null::text as agent_legal_entity_id,cl.client_id,ct.contract_id,d.deal_id from portal_private.agent_persons ap join portal_private.agent_client_assignments a on a.agent_person_key=ap.id left join portal_private.agent_deal_terms t on t.assignment_id=a.id and t.status='ACTIVE'::portal_private.binding_status_enum and t.lifecycle_state='ACTIVE'::portal_private.lifecycle_state_enum left join portal_private.deals d on d.id=t.deal_key left join portal_private.clients cl on cl.id=a.client_key left join portal_private.contracts ct on ct.client_key=cl.id and (d.contract_key is null or ct.id=d.contract_key) where ap.id=${bound}::uuid and ap.lifecycle_state='ACTIVE'::portal_private.lifecycle_state_enum and a.status='ACTIVE'::portal_private.binding_status_enum and a.lifecycle_state='ACTIVE'::portal_private.lifecycle_state_enum`
      :await sql`select distinct ap.agent_person_id,ale.agent_legal_entity_id,cl.client_id,ct.contract_id,d.deal_id from portal_private.agent_user_bindings ub join portal_private.agent_persons ap on ap.id=ub.agent_person_key left join portal_private.agent_legal_entities ale on ale.id=ub.agent_legal_entity_key join portal_private.agent_client_assignments a on a.agent_person_key=ub.agent_person_key and (ub.agent_legal_entity_key is null or a.agent_legal_entity_key is not distinct from ub.agent_legal_entity_key) join portal_private.agent_deal_terms t on t.assignment_id=a.id join portal_private.deals d on d.id=t.deal_key join portal_private.clients cl on cl.id=d.client_key join portal_private.contracts ct on ct.id=d.contract_key where ub.user_id=${c.user}::uuid and (${bound}::uuid is null or ap.id=${bound}::uuid) and portal_private.agent_user_has_deal_view_access(${c.user}::uuid,d.id,now())`;
    for(const r of rows){persons.add(String(r.agent_person_id));if(r.agent_legal_entity_id)legalEntities.add(String(r.agent_legal_entity_id));if(r.client_id)clients.add(String(r.client_id));if(r.contract_id)contracts.add(String(r.contract_id));if(r.deal_id)deals.add(String(r.deal_id))}
  }
