...
export function dedupeClientPaymentRowsV7(payments,allowedPaymentIds,allowedPaymentScopes=new Set()){
  const rows=Array.isArray(payments)?payments:[];
  const ids=allowedPaymentIds instanceof Set?allowedPaymentIds:new Set(Array.isArray(allowedPaymentIds)?allowedPaymentIds:[]);
  const scopes=allowedPaymentScopes instanceof Set?allowedPaymentScopes:new Set(Array.isArray(allowedPaymentScopes)?allowedPaymentScopes:[]);
  const seen=new Set();
  const result=[];
