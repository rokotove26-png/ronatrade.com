...
declare
  ...
  v_handoff text; v_addendum int; v_invoice int; v_signed int;
  v_amount numeric; v_currency text; v_auth text; v_life text; v_finance_ready boolean;
  v_expect_amount numeric; v_expect_currency text;
begin
  ...
  select d.id,w.product_confirmed_at,w.quantity_confirmed_at,coalesce(w.cancellation_state,'ACTIVE'),
         coalesce(w.payment_handoff_state,'NOT_SENT'),w.payment_expectation_amount,w.payment_expectation_currency,
         fs.client_remaining_amount,fs.currency,fs.authority_state,fs.lifecycle_state
    into v_deal,v_prod,v_qty,v_cancel,v_handoff,v_expect_amount,v_expect_currency,
         v_amount,v_currency,v_auth,v_life
  ...
  if v_handoff='SENT' then
    return jsonb_build_object('dealId',p_deal_id,'state','SENT','amount',v_expect_amount,'currency',v_expect_currency,'idempotent',true);
  end if;

  v_finance_ready := v_auth='CONFIRMED' and v_life='ACTIVE' and v_amount is not null and v_amount>=0 and coalesce(btrim(v_currency),'')<>'';
  ...
  update portal_private.owner_deal_workflow
     set payment_handoff_state='SENT',payment_handoff_at=coalesce(payment_handoff_at,now()),payment_handoff_by=v_actor,
         payment_expectation_state=case when v_finance_ready and v_amount>0 then 'ACTIVE' when v_finance_ready then 'NOT_CREATED' else payment_expectation_state end,
         payment_expectation_amount=case when v_finance_ready then v_amount else payment_expectation_amount end,
         payment_expectation_currency=case when v_finance_ready then v_currency else payment_expectation_currency end,
         updated_at=now()
   where deal_key=v_deal;

  if v_finance_ready and v_amount>0 then
  ...
  return jsonb_build_object(
    'dealId',p_deal_id,
    'state','SENT',
    'amount',case when v_finance_ready then v_amount else null end,
    'currency',case when v_finance_ready then v_currency else null end,
    'financePending',not v_finance_ready,
    'idempotent',false
  );
end $function$;

comment on function public.owner_r1_send_to_payments(text) is
'Admin R1 canonical payment handoff. Active deal + confirmed product/volume + active SIGNED_ADDENDUM lineage + INVOICE are pre-handoff prerequisites. Finance summary is downstream: its absence never blocks SENT; authoritative finance, when already available, retains existing payment expectation/plan materialization.';
