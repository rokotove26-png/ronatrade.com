-- Generic source-driven lifecycle executor. No production record IDs or expected quantities.
-- The immutable intake/event is evidence, not itself a business application.
...
declare a portal_private.client_applications; w portal_private.owner_application_workflow;
  reason text; protected boolean;
begin
  ...
  if a.status::text in ('REJECTED','CANCELLED')
     or coalesce(w.business_status,'') in ('REJECTED','CANCELLED','SUPPLIER_REJECTED','RESOURCE_DENIED') then
    reason:=case when a.status::text='REJECTED' or coalesce(w.business_status,'') in
      ('REJECTED','SUPPLIER_REJECTED','RESOURCE_DENIED') then 'REJECTED_BY_BUSINESS_PROCESS' else 'CANCELLED_BY_BUSINESS_PROCESS' end;
  else
    -- Staleness/duplication is a business determination, not an arbitrary age cutoff.
    -- Existing protected server-side audit supplies the explicit decision when applicable.
    select ae.metadata->>'reason_code' into reason from portal_private.audit_events ae
      where ae.entity_type='APPLICATION' and ae.entity_id=a.application_id
        and ae.action='APPLICATION_RETIREMENT_CONFIRMED' and ae.actor_role::text='ADMIN'
        and ae.actor_user_id is not null
        and ae.metadata->>'disposition'='DELETE'
        and ae.metadata->>'reason_code' in ('STALE','DUPLICATE','TECHNICAL_ERROR','RESOURCE_DENIED')
      order by ae.occurred_at desc limit 1;
  end if;
  if reason is not null then
    protected:=a.linked_deal_key is not null or exists(
      select 1 from portal_private.deal_registrations where application_key=a.id);
    return jsonb_build_object('decision',case when protected then 'BLOCKED' else 'DELETE' end,
      ...
      'business_reason',reason,'source_reason',a.decision_reason,
      'linked_deal_key',a.linked_deal_key);
  end if;
  ...
  return jsonb_build_object('decision','KEEP','reason',case when a.linked_deal_key is not null
    then 'CANONICAL_REGISTERED_DEAL' when a.status::text='CLOSED'
    then 'COMPLETED_REAL_BUSINESS_APPLICATION' else 'ACTIVE_REAL_BUSINESS_APPLICATION' end);
end $$;
...
set search_path='pg_catalog','portal_private' as $$
  select jsonb_build_object(
    'applications',coalesce((select jsonb_agg(jsonb_build_object(
      'record_type','CLIENT_APPLICATION','record_id',a.application_id,'application_key',a.id,
      'client_id',c.client_id,'client_name',c.legal_name,'contract_id',ct.contract_id,
      ...
      'source_intake_id',r.primary_intake_id,'deal_id',d.deal_id,
      'classification',portal_private.application_retention_decision_v2(a.id)) order by a.created_at,a.id)
    ...
    'intakes',coalesce((select jsonb_agg(jsonb_build_object(
      'record_type','IMMUTABLE_INTAKE','record_id',i.intake_id,'source_kind',i.source_kind,
      'source_record_id',i.source_record_id,'client_id',c.client_id,'contract_id',ct.contract_id,
      'source_submitted_at',i.source_submitted_at,'source_fingerprint',i.source_fingerprint,
      'actionable_type',i.actionable_type,'routing_state',i.routing_state,
      'business_application_id',r.application_id,'disposition',sd.disposition,'reason',sd.reason_code)
      order by i.source_submitted_at,i.intake_id)
      from portal_private.client_intake_v1 i
      left join portal_private.clients c on c.id=i.client_key
    ...
    'counts',jsonb_build_object(
      'live_business_rows',(select count(*) from portal_private.client_applications),
...
begin
  ...
  -- These relation-level locks are confined to this one application's evidence.
  ...
  select coalesce(jsonb_agg(to_jsonb(l) order by l.line_no),'[]'::jsonb) into lines
    from portal_private.application_lines l where l.application_key=a.id;
  ...
  insert into portal_private.client_application_tombstones_v2
    (application_key,application_id,primary_intake_id,reason_code,source_fingerprint,
     application_snapshot,lines_snapshot,workflow_snapshot,operations_identity_key)
    values(a.id,a.application_id,r.primary_intake_id,decision->>'reason',fingerprint,
      to_jsonb(a),lines,workflow,authority);
  insert into portal_private.client_application_audit_v2
    ...
    values(a.application_id,r.primary_intake_id,'BUSINESS_APPLICATION_DELETE_AUTHORIZED',authority,
      decision||jsonb_build_object('snapshot_sha256',fingerprint));
  insert into portal_private.client_application_source_disposition_v2(intake_id,disposition,reason_code,application_key)
    ...
    on conflict(intake_id) do update set disposition='DELETED',reason_code=excluded.reason_code,
      application_key=excluded.application_key;
  -- Detach only live-row FKs. Immutable source IDs/payloads/timestamps and task evidence survive.
  update portal_private.client_intake_v1 set application_key=null where application_key=a.id;
  update portal_private.staff_tasks set application_key=null,
    status=case when status::text in ('NEW','ACKNOWLEDGED','IN_PROGRESS','WAITING')
      then 'CLOSED'::portal_private.staff_task_status_enum else status end
...
set search_path='pg_catalog','portal_private' as $$
declare i portal_private.client_intake_v1; a portal_private.client_applications; p jsonb;
  authority uuid; app_key uuid; app_id text; old_task_status text; source_application text;
  pub_item uuid; pub_key uuid; source_quantity numeric; source_destination text; source_product text;
begin
  authority:=portal_private.application_operations_authority_v2();
  ...
  if exists(select 1 from portal_private.client_application_source_disposition_v2
    where intake_id=i.intake_id and disposition in ('DELETED','TECHNICAL_ONLY','TERMINAL_REQUEST')) then return null; end if;
  if exists(select 1 from portal_private.client_application_tombstones_v2 where primary_intake_id=i.intake_id)
  ...
  if i.source_kind='CLIENT_APPLICATION' then
    ...
    if (i.client_key,i.contract_key) is distinct from (a.client_key,a.contract_key)
    ...
    insert into portal_private.client_application_source_disposition_v2(intake_id,disposition,reason_code,application_key)
      values(i.intake_id,'BUSINESS','CANONICAL_CLIENT_APPLICATION',a.id)
  ...
  if i.actionable_type<>'DELIVERED_PRICE_CALCULATION_REQUEST_V1' then
    insert into portal_private.client_application_source_disposition_v2(intake_id,disposition,reason_code)
      values(i.intake_id,'TECHNICAL_ONLY',case when i.actionable_type like 'APPLICATION_DETAILS_%'
        then 'DETAIL_EVENT_IS_NOT_AN_APPLICATION' else 'NON_APPLICATION_ACTIONABLE_EVENT' end)
      on conflict(intake_id) do nothing;
    return null;
  end if;
  -- Reuse an already materialized source before looking at its later business/task state.
  ...
  if old_task_status in ('COMPLETED','CLOSED','REJECTED','DECIDED') then
    insert into portal_private.client_application_source_disposition_v2(intake_id,disposition,reason_code)
      values(i.intake_id,'TERMINAL_REQUEST','HISTORICAL_REQUEST_PROCESS_ALREADY_FINISHED') on conflict(intake_id) do nothing;
    return null;
  end if;
  if not exists(select 1 from portal_private.clients cl join portal_private.contracts ct
       on ct.client_key=cl.id where cl.id=i.client_key and ct.id=i.contract_key
       and nullif(btrim(cl.legal_name),'') is not null)
  ...
  if jsonb_typeof(p->'quantity_tonnes')<>'number' then raise exception 'APPLICATION_SOURCE_QUANTITY_INVALID'; end if;
  source_quantity:=(p->>'quantity_tonnes')::numeric;
  source_product:=nullif(btrim(p->>'product'),'');
  source_destination:=case when jsonb_typeof(p->'destination')='object' then
      nullif(btrim(p#>>'{destination,station}'),'') else nullif(btrim(p->>'destination'),'') end;
  if source_quantity<=0 or source_product is null or source_destination is null
  ...
  insert into portal_private.client_applications
    (id,application_id,client_key,contract_key,source_intake_key,source_publication_id,source_publication_item_id,
     product,quantity_tonnes,delivery_period_from,delivery_period_to,delivery_basis,destination,
     delivery_method,payment_terms,price_mode,proposed_price,proposed_currency,status,submitted_at,
     source_system,source_version,source_timestamp,authority_state,lifecycle_state,source_price_mode,source_submission_state)
    values(app_key,app_id,i.client_key,i.contract_key,i.intake_id,pub_key,pub_item,
      source_product,source_quantity,nullif(p#>>'{shipment,period_from}','')::timestamptz::date,
      nullif(p#>>'{shipment,period_to}','')::timestamptz::date,
      p#>>'{shipment,source_basis}',source_destination,'TO_BE_CONFIRMED',
      coalesce(nullif(p#>>'{commercial,payment_terms}',''),'TO_BE_AGREED'),
      'REQUEST_DELIVERED_PRICE',null,null,'SUBMITTED',i.source_submitted_at,
      'CLIENT_INTAKE','APPLICATION_BUSINESS_V2',i.source_submitted_at,'SOURCE_RECEIVED','ACTIVE',
      'REQUEST_DELIVERED_PRICE','SOURCE_INTAKE:'||i.intake_id::text);
  ...
  -- Existing routing work is linked, not recreated. This also makes shared AI task reads resolve the business ID.
  update portal_private.staff_tasks st set application_key=app_key,
      title='Application '||app_id,
      description=coalesce(st.description,'')||E'\nCanonical application: '||app_id
    where (st.source_reverse_event_key=i.source_internal_key
       or st.id in (select staff_task_id from portal_private.client_intake_task_links_v1 where intake_id=i.intake_id))
      and st.application_key is null;
  insert into portal_private.client_application_source_disposition_v2(intake_id,disposition,reason_code,application_key)
    values(i.intake_id,'BUSINESS','MATERIALIZED_FROM_AUTHORITATIVE_INTAKE',app_key)
    on conflict(intake_id) do nothing;
  return app_id;
end $$;
...
begin
  ...
  for j in select * from portal_private.client_application_jobs_v2
    where state in ('PENDING','RETRY') and next_attempt_at<=now()
    order by next_attempt_at,intake_id for update skip locked
    limit greatest(1,least(coalesce(p_limit,100),1000)) loop
    begin
      app_id:=portal_private.materialize_client_application_v2(j.intake_id);
      update portal_private.client_application_jobs_v2 set state='DONE',attempts=attempts+1,
        last_error=null,updated_at=now() where intake_id=j.intake_id;
      processed:=processed+1;
    exception when others then
      errors:=errors+1;
      update portal_private.client_application_jobs_v2 set state='RETRY',attempts=attempts+1,
        last_error=left(sqlerrm,300),next_attempt_at=now()+interval '1 minute',updated_at=now()
        where intake_id=j.intake_id;
    end;
  end loop;
...
begin
  ...
  if coalesce((select enabled from portal_private.client_application_policy_v2 where singleton),false) then
    begin
      ...
      update portal_private.client_application_jobs_v2 set state='DONE',attempts=attempts+1,updated_at=now()
...
-- The source-intake-backed business row must not manufacture another intake or staff task.
...
begin
  if new.source_intake_key is not null then
    if not exists(select 1 from portal_private.client_intake_v1 where intake_id=new.source_intake_key
      and client_key=new.client_key and contract_key=new.contract_key)
      then raise exception 'APPLICATION_SOURCE_CONTEXT_CONFLICT'; end if;
    update portal_private.client_intake_v1 set application_key=new.id where intake_id=new.source_intake_key;
    perform portal_private.ensure_application_registry_v2(new.id);
    return new;
  end if;
...
returns jsonb language plpgsql security definer
set search_path='pg_catalog','portal_private' as $$
declare a record; before_inventory jsonb; results jsonb:='[]'::jsonb; run_key uuid;
  authority uuid; jobs_result jsonb;
begin
  if not coalesce((select enabled from portal_private.client_application_policy_v2 where singleton),false)
    then return jsonb_build_object('state','DISABLED'); end if;
  authority:=portal_private.application_operations_authority_v2();
  -- Capture a durable machine-readable source inventory BEFORE materialization or deletion.
  before_inventory:=portal_private.application_inventory_v2();
  insert into portal_private.client_application_inventory_runs_v2(inventory,source_fingerprint,operations_identity_key)
    values(before_inventory,encode(extensions.digest(before_inventory::text,'sha256'),'hex'),authority)
    returning run_id into run_key;
  for a in select id from portal_private.client_applications where not exists(
    select 1 from portal_private.client_application_registry_v2 r where r.application_key=client_applications.id)
  ...
  insert into portal_private.client_application_jobs_v2(intake_id)
    select i.intake_id from portal_private.client_intake_v1 i
      where not exists(select 1 from portal_private.client_application_jobs_v2 j where j.intake_id=i.intake_id)
      order by i.source_submitted_at,i.intake_id
      limit greatest(1,least(coalesce(p_limit,500),5000)) on conflict do nothing;
  ...
  for a in select id from portal_private.client_applications
    where portal_private.application_retention_decision_v2(id)->>'decision'='DELETE'
    order by created_at,id limit greatest(1,least(coalesce(p_limit,500),5000)) loop
    results:=results||jsonb_build_array(portal_private.retire_client_application_v2(a.id));
  end loop;
  return jsonb_build_object('inventory_run_id',run_key,'jobs',jobs_result,'retirements',results,
    'after',portal_private.application_inventory_v2()->'counts');
end $$;

revoke all on function portal_private.application_retention_decision_v2(uuid),
  portal_private.application_inventory_v2(),portal_private.retire_client_application_v2(uuid),
  portal_private.materialize_client_application_v2(uuid),portal_private.process_application_jobs_v2(integer),
  portal_private.application_intake_business_trigger_v2(),portal_private.client_intake_application_trigger_v1(),
  portal_private.reconcile_client_applications_v2(integer) from public,anon,authenticated;
