    ...
    if found then
      ...
      if v_binding_id is null then
        insert into portal_private.client_user_bindings(
          ...
          v_user_id, v_contract.client_key, v_contract.contract_key,
          'PENDING', now(), new.actor_user_id, now(),
          'Administrator opened Client Portal account without confirmed signed contract; no client data access until contract confirmation.',
          'ADMIN_PORTAL', 'ADMIN_EXCLUSIVE_CLIENT_OPEN_WITHOUT_CONTRACT_V1', now(),
          'CONFIRMED', 'ACTIVE', 'ALL_CONTRACT_DEALS'
        ) returning id into v_binding_id;

        insert into portal_private.client_user_binding_profiles(
          binding_id, representation_role, contact_email, created_by
        ) values (
          v_binding_id, 'Уполномоченный представитель', nullif(new.metadata->>'email',''), new.actor_user_id
...
begin
  ...
  select ct.id as contract_key, ct.client_key, ct.contract_id
   ...
   where ct.contract_id = new.entity_id
     ...
     and exists (
       select 1 from portal_private.storage_objects so
  ...
  loop
    ...
    select b.id into v_binding_id
     ...
     where b.user_id = v_pending.user_id
       and b.contract_key = v_contract.contract_key
       and b.status in ('PENDING'::portal_private.binding_status_enum,'ACTIVE'::portal_private.binding_status_enum)
       and b.revoked_at is null
     order by case when b.status='ACTIVE'::portal_private.binding_status_enum then 0 else 1 end, b.created_at desc
     limit 1;
    ...
    if v_binding_id is null then
      ...
      update portal_private.client_user_bindings
         set status = 'ACTIVE'::portal_private.binding_status_enum,
             granted_by = new.actor_user_id,
             granted_at = now(),
             reason = 'Administrator uploaded/confirmed the signed contract after opening the account without contract.',
             source_version = 'ADMIN_EXCLUSIVE_CLIENT_PENDING_PROMOTION_V1',
