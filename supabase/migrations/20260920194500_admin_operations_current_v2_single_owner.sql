-- Operations Current V2 single-owner read model and real Client/Agent network presence.
-- Safe migration: V1 is retained for rollback but no longer used by the V10 Operations UI.

create table if not exists portal_private.portal_presence_connections_v1 (
  connection_id uuid primary key,
  portal_user_id uuid not null references portal_private.portal_users(id) on delete cascade,
  portal_role text not null check (portal_role in ('CLIENT','AGENT')),
...
declare
  v_actor uuid;
  v_roles text[];
  v_role text := upper(trim(coalesce(p_portal_role,'')));
  v_session text := auth.jwt()->>'session_id';
begin
  if p_connection_id is null then
    raise exception using errcode='22023', message='PRESENCE_CONNECTION_ID_REQUIRED';
  end if;

  select a.portal_user_id,a.roles
    into v_actor,v_roles
  from portal_private.resolve_portal_auth(auth.uid(),v_session) a
  ...
  if 'ADMIN'=any(v_roles) or 'RONA_OPERATOR'=any(v_roles) then
    delete from portal_private.portal_presence_connections_v1
     where connection_id=p_connection_id and portal_user_id=v_actor;
    return jsonb_build_object('tracked',false,'reason','INTERNAL_ROLE_EXCLUDED');
  end if;

  if v_role not in ('CLIENT','AGENT') or not (v_role=any(v_roles)) then
    raise exception using errcode='42501', message='PRESENCE_ROLE_DENIED';
  end if;

  if not coalesce(p_online,true) then
    delete from portal_private.portal_presence_connections_v1
     where connection_id=p_connection_id and portal_user_id=v_actor;
    return jsonb_build_object('tracked',false,'role',v_role,'connectionId',p_connection_id);
  end if;

  insert into portal_private.portal_presence_connections_v1(
    connection_id,portal_user_id,portal_role,session_subject,started_at,last_seen_at
  )
  values(
    p_connection_id,v_actor,v_role,v_session,clock_timestamp(),clock_timestamp()
  )
  on conflict(connection_id) do update
     set portal_user_id=excluded.portal_user_id,
         portal_role=excluded.portal_role,
         session_subject=excluded.session_subject,
         last_seen_at=clock_timestamp();

  delete from portal_private.portal_presence_connections_v1
   where last_seen_at < clock_timestamp()-interval '1 day';

  return jsonb_build_object(
    'tracked',true,
    'role',v_role,
    'connectionId',p_connection_id,
    'seenAt',clock_timestamp()
  );
end
$function$
;

revoke all on function public.rona_portal_presence_heartbeat_v1(uuid,text,boolean) from public, anon;
grant execute on function public.rona_portal_presence_heartbeat_v1(uuid,text,boolean) to authenticated, service_role;
...
deal_actions as (
  select
    ...
    case when d.due_now>0 then 'Оплата · '||d.deal_id else 'Сделка · '||d.deal_id end as title,
...
is 'Single authoritative read model for Admin Operations Flightdeck V10. Finance V8, Rail current, active documents, normalized actions and live Client/Agent presence only.';

comment on function public.rona_admin_operations_current_v1()
is 'DEPRECATED rollback-only Operations read model. Active Admin Operations UI must use rona_admin_operations_current_v2().';

comment on table portal_private.portal_presence_connections_v1
is 'Short-lived real-user network presence heartbeats for CLIENT/AGENT portal sessions. Admin/operator and admin impersonation are excluded by the heartbeat authority.';
