...
set enabled=false,
    ...
    protocol_version='AI_STAFF_COMMUNICATION_PROTOCOL_V1_3',
    worker_version='1.6.0',
    last_probe_at=null,
    last_probe_model=null,
    last_error_code='PROVIDER_SWITCH_PENDING_PROBE',
    last_error_at=now(),
    activated_at=null,
    updated_at=now()
...
begin
  ...
  update portal_private.ai_model_executor_control
     set enabled=true,state='ENABLED',execute_after=now(),activated_at=now(),last_error_code=null,last_error_at=null,worker_version='1.6.0',updated_at=now()
   where singleton=true returning * into c;
  update portal_private.ai_runtime_control
     set protocol_version='AI_STAFF_COMMUNICATION_PROTOCOL_V1_3',model_execution_state='ENABLED',worker_version='1.6.0',updated_at=now()
   where singleton=true;
...
begin
  update portal_private.ai_model_executor_control
     set enabled=false,state='BLOCKED',last_error_code=left(coalesce(p_reason,'MANUAL_BLOCK'),160),last_error_at=now(),worker_version='1.6.0',updated_at=now()
   where singleton=true returning * into c;
  update portal_private.ai_runtime_control
     set model_execution_state='BLOCKED',protocol_version='AI_STAFF_COMMUNICATION_PROTOCOL_V1_3',worker_version='1.6.0',updated_at=now()
   where singleton=true;
