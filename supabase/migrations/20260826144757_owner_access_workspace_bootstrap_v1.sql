...
begin
  ...
  from (
    ...
    where ae.entity_type='PORTAL_USER'
      and exists(select 1 from portal_private.portal_users u where u.id::text=ae.entity_id)
...
$function$;
revoke all on function public.owner_access_workspace_bootstrap(integer) from public;
revoke execute on function public.owner_access_workspace_bootstrap(integer) from anon;
grant execute on function public.owner_access_workspace_bootstrap(integer) to authenticated;
