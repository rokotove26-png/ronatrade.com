...
create table if not exists portal_private.client_intake_v1 (
  intake_id uuid primary key default pg_catalog.gen_random_uuid(),
  durable_id uuid not null unique,
  source_kind text not null check (source_kind in ('CLIENT_APPLICATION','PORTAL_REVERSE_EVENT')),
...
create unique index if not exists uq_client_intake_source_idempotency_v1
  on portal_private.client_intake_v1(source_kind,source_idempotency_key)
...
create table if not exists portal_private.client_intake_routing_outbox_v1 (
  ...
  intake_id uuid not null references portal_private.client_intake_v1(intake_id) on delete restrict,
  stage_key text not null,
  state text not null check (state in ('PENDING','QUEUED','PROCESSING','APPLIED','FAILED_RETRYABLE','DEAD_LETTER')),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique(intake_id,stage_key)
);

create table if not exists portal_private.client_intake_task_links_v1 (
  intake_id uuid not null references portal_private.client_intake_v1(intake_id) on delete restrict,
  stage_key text not null,
  staff_task_id uuid not null references portal_private.staff_tasks(id) on delete restrict,
  linked_at timestamptz not null default now(),
  primary key(intake_id,stage_key),
  unique(staff_task_id)
);

create table if not exists portal_private.client_intake_corrections_v1 (
  correction_id uuid primary key default pg_catalog.gen_random_uuid(),
  intake_id uuid not null references portal_private.client_intake_v1(intake_id) on delete restrict,
  field_path text not null,
  source_value jsonb not null,
  corrected_value jsonb not null,
  correction_authority text not null,
  correction_reason text not null,
  corrected_at timestamptz not null default now(),
  source_record text not null,
  created_at timestamptz not null default now()
);
create index if not exists ix_client_intake_corrections_current_v1
  on portal_private.client_intake_corrections_v1(intake_id,field_path,corrected_at desc,correction_id desc);
...
create table if not exists portal_private.client_intake_audit_v1 (
  ...
  intake_id uuid references portal_private.client_intake_v1(intake_id) on delete restrict,
...
create or replace function portal_private.guard_client_intake_correction_append_only_v1()
returns trigger language plpgsql set search_path='pg_catalog','portal_private' as $$
begin
  raise exception 'CLIENT_INTAKE_CORRECTION_APPEND_ONLY';
end $$;
drop trigger if exists guard_client_intake_correction_append_only_v1 on portal_private.client_intake_corrections_v1;
create trigger guard_client_intake_correction_append_only_v1
before update or delete on portal_private.client_intake_corrections_v1
for each row execute function portal_private.guard_client_intake_correction_append_only_v1();

create or replace function portal_private.client_intake_actionable_type_v1(
  p_source_kind text,p_source_event_type text,p_source_payload jsonb
) returns text language sql immutable set search_path='pg_catalog','portal_private' as $$
  select case
    when p_source_kind='CLIENT_APPLICATION' and upper(coalesce(p_source_payload->>'price_mode',''))='ACCEPT_PUBLISHED_PRICE'
      then 'PUBLISHED_PRICE_APPLICATION'
    when p_source_kind='CLIENT_APPLICATION' and upper(coalesce(p_source_payload->>'price_mode',''))='CLIENT_PROPOSED_PRICE'
      then 'CLIENT_PROPOSED_PRICE_APPLICATION'
    when p_source_kind='CLIENT_APPLICATION'
      then 'UNSUPPORTED_PRICE_MODE'
    when p_source_kind='PORTAL_REVERSE_EVENT' and nullif(upper(trim(coalesce(p_source_payload->>'message_type',''))),'') is not null
      then upper(trim(p_source_payload->>'message_type'))
    else upper(trim(coalesce(p_source_event_type,'UNKNOWN_CLIENT_ACTIONABLE')))
  end
$$;

create or replace function portal_private.client_intake_policy_v1(
  p_source_kind text,p_actionable_type text,p_as_of timestamptz default now()
) returns portal_private.client_intake_routing_registry_v1
language sql stable set search_path='pg_catalog','portal_private' as $$
  select p.*
  from portal_private.client_intake_routing_registry_v1 p
  where p.source_kind=p_source_kind
    and p.actionable_type=p_actionable_type
    and p.effective_at<=p_as_of
    and (p.disabled_at is null or p.disabled_at>p_as_of)
  order by p.priority desc,p.effective_at desc,p.policy_key desc
  limit 1
$$;
...
begin
  ...
  for c in
    select field_path,corrected_value
    from portal_private.client_intake_corrections_v1
    where intake_id=p_intake_id
    order by corrected_at,correction_id
  loop
    if c.field_path like 'payload.%' then
      v:=pg_catalog.jsonb_set(v,pg_catalog.string_to_array(pg_catalog.substring(c.field_path from 9),'.'),c.corrected_value,true);
    else
      v:=pg_catalog.jsonb_set(v,pg_catalog.string_to_array(c.field_path,'.'),c.corrected_value,true);
    end if;
  end loop;
  return v;
end $$;
...
create or replace function portal_private.ensure_client_intake_v1(
  p_source_kind text,p_source_record_id text,p_source_internal_key uuid,p_source_event_type text,
  p_source_idempotency_key text,p_source_payload jsonb,p_source_submitted_at timestamptz,
...
declare
  v_type text;
  v_policy portal_private.client_intake_routing_registry_v1;
  v_intake uuid;
  v_existing portal_private.client_intake_v1;
  v_state text;
begin
  if coalesce(trim(p_source_record_id),'')='' or coalesce(trim(p_source_kind),'')='' then
    raise exception 'CLIENT_INTAKE_SOURCE_ID_REQUIRED';
  end if;
  select * into v_existing from portal_private.client_intake_v1
   where source_kind=p_source_kind and source_record_id=p_source_record_id;
  if found then
    if v_existing.source_fingerprint<>p_source_fingerprint then raise exception 'CLIENT_INTAKE_SOURCE_IMMUTABLE_CONFLICT'; end if;
    return v_existing.intake_id;
  end if;
  if nullif(trim(coalesce(p_source_idempotency_key,'')),'') is not null then
    select * into v_existing from portal_private.client_intake_v1
     where source_kind=p_source_kind and source_idempotency_key=p_source_idempotency_key;
    if found then
      if v_existing.source_fingerprint<>p_source_fingerprint then raise exception 'CLIENT_INTAKE_IDEMPOTENCY_CONFLICT'; end if;
      return v_existing.intake_id;
    end if;
  end if;
  ...
  insert into portal_private.client_intake_v1(
    durable_id,source_kind,source_record_id,source_internal_key,source_event_type,actionable_type,
    source_idempotency_key,source_fingerprint,source_payload,source_submitted_at,client_key,contract_key,deal_key,
    client_visible,admin_visible,routing_policy_key,responsible_role,task_required,acknowledgement_required,routing_state,routing_reason
  ) values(
    pg_catalog.gen_random_uuid(),p_source_kind,p_source_record_id,p_source_internal_key,upper(p_source_event_type),v_type,
    nullif(trim(p_source_idempotency_key),''),p_source_fingerprint,coalesce(p_source_payload,'{}'::jsonb),coalesce(p_source_submitted_at,now()),
    p_client_key,p_contract_key,p_deal_key,coalesce(v_policy.client_visible,true),coalesce(v_policy.admin_visible,true),
    v_policy.policy_key,v_policy.responsible_role,coalesce(v_policy.task_required,false),coalesce(v_policy.acknowledgement_required,false),
    v_state,case when v_policy.policy_key is null then 'ROUTING_POLICY_MISSING' end
  ) returning intake_id into v_intake;

  insert into portal_private.client_intake_routing_outbox_v1(intake_id,stage_key,state,last_error_code)
  values(v_intake,'RESPONSIBLE_ROLE_ROUTING',v_state,case when v_policy.policy_key is null then 'ROUTING_POLICY_MISSING' end)
  ...
  values(v_intake,'INTAKE_CREATED','RESPONSIBLE_ROLE_ROUTING',pg_catalog.jsonb_build_object(
    'source_kind',p_source_kind,'source_record_id',p_source_record_id,'actionable_type',v_type,'policy_key',v_policy.policy_key));
  return v_intake;
end $$;
...
begin
  ...
  v_payload:=pg_catalog.jsonb_build_object(
    'application_id',a.application_id,'price_mode',a.price_mode::text,'quantity_tonnes',a.quantity_tonnes,
    'product',a.product,'destination',a.destination,'source_publication_id',a.source_publication_id,
  ...
  return portal_private.ensure_client_intake_v1('CLIENT_APPLICATION',a.application_id,a.id,'CLIENT_APPLICATION_SUBMIT',a.application_id,
    v_payload,coalesce(a.submitted_at,a.created_at),a.client_key,a.contract_key,a.linked_deal_key,v_fp);
end $$;
...
begin
  for o in
    select * from portal_private.client_intake_routing_outbox_v1
    where state in ('PENDING','QUEUED','FAILED_RETRYABLE') and (next_attempt_at is null or next_attempt_at<=now())
    order by created_at,outbox_id
    for update skip locked
    limit greatest(1,least(coalesce(p_limit,100),1000))
  loop
    update portal_private.client_intake_routing_outbox_v1
      set state='PROCESSING',attempt_count=attempt_count+1,processing_started_at=now(),updated_at=now()
      where outbox_id=o.outbox_id;
    begin
      select * into i from portal_private.client_intake_v1 where intake_id=o.intake_id for update;
      select * into p from portal_private.client_intake_policy_v1(i.source_kind,i.actionable_type,now());
    ...
    exception when others then
      update portal_private.client_intake_routing_outbox_v1
      set state=case when attempt_count>=max_attempts then 'DEAD_LETTER' else 'FAILED_RETRYABLE' end,
          ...
          next_attempt_at=case when attempt_count>=max_attempts then null else now()+interval '1 minute' end,
          updated_at=now()
      where outbox_id=o.outbox_id;
      update portal_private.client_intake_v1 i2
      ...
      where q.outbox_id=o.outbox_id and i2.intake_id=q.intake_id;
    end;
  end loop;
  return query
  select q.intake_id,q.stage_key,q.state,l.staff_task_id
  from portal_private.client_intake_routing_outbox_v1 q
  left join portal_private.client_intake_task_links_v1 l using(intake_id,stage_key)
  order by q.created_at;
end $$;

create or replace function portal_private.client_intake_outbox_auto_consumer_v1()
returns trigger language plpgsql security definer set search_path='pg_catalog','portal_private' as $$
begin
  if new.state in ('PENDING','QUEUED','FAILED_RETRYABLE') then
    perform portal_private.process_client_intake_outbox_v1(1);
  end if;
  return new;
end $$;
drop trigger if exists client_intake_outbox_auto_consumer_v1 on portal_private.client_intake_routing_outbox_v1;
create trigger client_intake_outbox_auto_consumer_v1
after insert on portal_private.client_intake_routing_outbox_v1
for each row execute function portal_private.client_intake_outbox_auto_consumer_v1();

create or replace function portal_private.client_intake_reverse_event_trigger_v1()
returns trigger language plpgsql set search_path='pg_catalog','portal_private' as $$
begin
  if new.actor_role='CLIENT'::portal_private.portal_role_enum and new.event_type like 'CLIENT_%' then
...
create trigger client_intake_reverse_event_v1
after insert on portal_private.portal_reverse_events
for each row execute function portal_private.client_intake_reverse_event_trigger_v1();
...
create trigger client_intake_application_v1
after insert on portal_private.client_applications
for each row execute function portal_private.client_intake_application_trigger_v1();

-- Preserve non-Client legacy routing; Client events delegate to the canonical intake.
...
begin
  if new.processing_state in ('REJECTED','FAILED','DEAD_LETTER')
     ...
     or new.authority_domain ilike 'QA%' then return new; end if;
  if new.actor_role='CLIENT'::portal_private.portal_role_enum and new.event_type like 'CLIENT_%' then
    perform portal_private.ensure_client_intake_from_reverse_event_v1(new.event_id);
    return new;
  end if;
  ...
  insert into portal_private.staff_tasks(
    task_id,title,status,priority,authority_domain,assigned_functional_role,client_key,contract_key,deal_key,
    source_reverse_event_key,source_type,source_object_id,source_version,qa_only,created_by
  ) values(
    tid,'Событие портала: '||new.event_type,'NEW','NORMAL',new.authority_domain,role,new.client_key,new.contract_key,new.deal_key,
    new.id,'PORTAL_REVERSE_EVENT',new.event_id,new.source_version,false,new.actor_user_id
...
create or replace function portal_private.reconcile_client_intake_v1(
  p_limit integer default 500,p_stuck_interval interval default interval '5 minutes'
) returns jsonb language plpgsql security definer set search_path='pg_catalog','portal_private' as $$
declare r record;
begin
  for r in
    select event_id from portal_private.portal_reverse_events
    where actor_role='CLIENT'::portal_private.portal_role_enum and event_type like 'CLIENT_%'
      and processing_state not in ('REJECTED','FAILED','DEAD_LETTER')
    order by created_at
  ...
  for r in
    select application_id from portal_private.client_applications
  ...
  select i.intake_id,'RESPONSIBLE_ROLE_ROUTING',
         ...
         case when i.routing_policy_key is null then 'ROUTING_POLICY_RECHECK' end,now()
  ...
  where q.state='PROCESSING' and coalesce(q.processing_started_at,q.updated_at)<now()-p_stuck_interval;
  ...
  from portal_private.client_intake_routing_outbox_v1 q
  where q.intake_id=i.intake_id and q.state='FAILED_RETRYABLE' and q.last_error_code='STUCK_PROCESSING_RECOVERED';

  update portal_private.client_intake_v1
  set client_visible=true,admin_visible=true,updated_at=now()
  where not client_visible or not admin_visible;

  update portal_private.client_intake_routing_outbox_v1 q
  set state='FAILED_RETRYABLE',last_error_code='REQUIRED_TASK_MISSING',next_attempt_at=now(),updated_at=now()
  from portal_private.client_intake_v1 i
  where q.intake_id=i.intake_id and q.stage_key='RESPONSIBLE_ROLE_ROUTING' and q.state='APPLIED' and i.task_required
    and not exists(
  ...
  return (select pg_catalog.jsonb_build_object(
    ...
    'stuck_intake_count',(select count(*) from portal_private.client_intake_routing_outbox_v1 where state='PROCESSING' and coalesce(processing_started_at,updated_at)<now()-p_stuck_interval),
    'oldest_stuck_intake_age',(select max(now()-coalesce(processing_started_at,updated_at)) from portal_private.client_intake_routing_outbox_v1 where state='PROCESSING' and coalesce(processing_started_at,updated_at)<now()-p_stuck_interval)
  ) from portal_private.client_intake_v1);
end $$;

create or replace function portal_private.client_intake_reconciliation_tick_v1()
returns jsonb language sql security definer set search_path='pg_catalog','portal_private' as $$
  select portal_private.reconcile_client_intake_v1(500,interval '5 minutes')
$$;

create or replace view portal_private.client_intake_client_projection_v1 as
select i.intake_id,i.durable_id,i.source_kind,i.source_record_id as source_id,i.actionable_type,
       i.routing_state as status,i.routing_reason,i.responsible_role,i.source_submitted_at as submitted_at,
       portal_private.client_intake_effective_payload_v1(i.intake_id) as effective_payload,
       i.client_key,i.contract_key,i.deal_key,i.created_at,i.updated_at
from portal_private.client_intake_v1 i
where i.client_visible=true;

create or replace view portal_private.client_intake_admin_projection_v1 as
select i.intake_id,i.durable_id,i.source_kind,i.source_record_id as source_id,i.actionable_type,
       i.routing_state as status,i.routing_reason,i.responsible_role,i.task_required,i.acknowledgement_required,
       i.source_submitted_at as submitted_at,portal_private.client_intake_effective_payload_v1(i.intake_id) as effective_payload,
       i.client_key,i.contract_key,i.deal_key,i.created_at,i.updated_at
from portal_private.client_intake_v1 i
where i.admin_visible=true;

create or replace function portal_private.client_intake_projection_for_lk_v1(
  p_audience text,p_client_id text default null,p_contract_id text default null
) returns table(
  intake_id uuid,durable_id uuid,source_kind text,source_id text,actionable_type text,status text,
  routing_reason text,responsible_role text,submitted_at timestamptz,effective_payload jsonb,
  client_id text,contract_id text,deal_id text
) language sql stable security definer set search_path='pg_catalog','portal_private' as $$
  select i.intake_id,i.durable_id,i.source_kind,i.source_record_id,i.actionable_type,i.routing_state,
         i.routing_reason,i.responsible_role::text,i.source_submitted_at,
         portal_private.client_intake_effective_payload_v1(i.intake_id),cl.client_id,ct.contract_id,d.deal_id
  from portal_private.client_intake_v1 i
  left join portal_private.clients cl on cl.id=i.client_key
  left join portal_private.contracts ct on ct.id=i.contract_key
  left join portal_private.deals d on d.id=i.deal_key
  where case upper(coalesce(p_audience,'')) when 'CLIENT' then i.client_visible when 'ADMIN' then i.admin_visible else false end
...
returns table(intake_id uuid,durable_id uuid,source_id text,submitted_at timestamptz,status text)
language sql stable security definer set search_path='pg_catalog','portal_private' as $$
  select i.intake_id,i.durable_id,i.source_record_id,i.source_submitted_at,i.routing_state
  from portal_private.client_intake_v1 i
  where i.source_kind=p_source_kind and i.source_record_id=p_source_id
  limit 1
$$;

create or replace view portal_private.client_intake_metrics_v1 as
select
  count(*)::bigint as actionable_client_intake_total,
  count(*) filter(where routing_state<>'APPLIED')::bigint as unrouted_client_intake_count,
  count(*) filter(where not client_visible)::bigint as client_invisible_intake_count,
  count(*) filter(where not admin_visible)::bigint as admin_invisible_intake_count,
  count(*) filter(where not client_visible and not admin_visible)::bigint as dual_invisible_actionable_intake_count,
  (select count(*) from portal_private.client_intake_routing_outbox_v1 where state='PROCESSING' and coalesce(processing_started_at,updated_at)<now()-interval '5 minutes')::bigint as stuck_intake_count,
  (select max(now()-coalesce(processing_started_at,updated_at)) from portal_private.client_intake_routing_outbox_v1 where state='PROCESSING' and coalesce(processing_started_at,updated_at)<now()-interval '5 minutes') as oldest_stuck_intake_age
from portal_private.client_intake_v1;

-- Production has pg_cron. Disposable PostgreSQL 17 CI intentionally skips this block when pg_cron is absent.
do $client_intake_cron$
begin
  if exists(select 1 from pg_catalog.pg_namespace where nspname='cron')
     and exists(select 1 from pg_catalog.pg_proc p join pg_catalog.pg_namespace n on n.oid=p.pronamespace where n.nspname='cron' and p.proname='schedule') then
    perform cron.schedule('rona-client-intake-reconcile-v1','*/2 * * * *','select portal_private.client_intake_reconciliation_tick_v1();');
  end if;
end
$client_intake_cron$;

revoke all on portal_private.client_intake_v1,portal_private.client_intake_routing_registry_v1,
  portal_private.client_intake_routing_outbox_v1,portal_private.client_intake_task_links_v1,
  portal_private.client_intake_corrections_v1,portal_private.client_intake_audit_v1
  from public,anon,authenticated;
revoke all on portal_private.client_intake_client_projection_v1,portal_private.client_intake_admin_projection_v1,
  portal_private.client_intake_metrics_v1
  from public,anon,authenticated;
