...
create table if not exists portal_private.owner_price_list_controls (
  ...
  publication_key uuid not null unique references portal_private.publications(id) on delete restrict,
  internal_state text not null default 'RECEIVED_IN_ADMIN',
  ...
  agent_published_at timestamptz,
  published_by uuid references portal_private.portal_users(id) on delete restrict,
  ...
  constraint owner_price_list_controls_internal_state_check check (internal_state in ('RECEIVED_IN_ADMIN','READY_FOR_REVIEW','CURRENT','RETIRED')),
  constraint owner_price_list_controls_source_refs_check check (jsonb_typeof(source_refs)='array')
);
...
create table if not exists portal_private.owner_price_change_proposals (
  ...
  rejected_by uuid references portal_private.portal_users(id) on delete restrict,
  rejection_reason text,
  new_publication_key uuid references portal_private.publications(id) on delete restrict,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint owner_price_change_proposals_status_check check (proposal_status in ('UPDATE_AVAILABLE','BLOCKED','APPLIED','REJECTED','SUPERSEDED')),
  constraint owner_price_change_proposals_changes_check check (jsonb_typeof(changes)='array'),
  constraint owner_price_change_proposals_source_refs_check check (jsonb_typeof(source_refs)='array')
);
...
create table if not exists portal_private.owner_agent_commercial_proposals (
  ...
  publication_key uuid not null references portal_private.publications(id) on delete restrict,
  agent_legal_entity_key uuid not null references portal_private.agent_legal_entities(id) on delete restrict,
  version integer not null default 1 check (version>0),
  document_key uuid references portal_private.documents(id) on delete restrict,
  request_event_key uuid references portal_private.portal_reverse_events(id) on delete restrict,
  status text not null default 'REQUESTED' check (status in ('REQUESTED','GENERATED','OWNER_REVIEWED','SENT','BLOCKED','RETIRED','STALE_PRICE_SOURCE')),
  generated_at timestamptz,
  owner_reviewed_at timestamptz,
  owner_reviewed_by uuid references portal_private.portal_users(id) on delete restrict,
  sent_at timestamptz,
  sent_by uuid references portal_private.portal_users(id) on delete restrict,
  source_refs jsonb not null default '[]'::jsonb,
  authority_state portal_private.authority_state_enum not null default 'SOURCE_RECEIVED',
  lifecycle_state portal_private.lifecycle_state_enum not null default 'ACTIVE',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique(publication_key,agent_legal_entity_key,version),
  constraint owner_agent_commercial_proposals_source_refs_check check (jsonb_typeof(source_refs)='array')
);

create index if not exists owner_price_change_proposals_open_idx on portal_private.owner_price_change_proposals(proposal_status,received_at desc);
create index if not exists owner_agent_cp_publication_idx on portal_private.owner_agent_commercial_proposals(publication_key,status);
...
begin
 ...
 into v_cp from portal_private.owner_agent_commercial_proposals c join portal_private.publications p on p.id=c.publication_key join portal_private.agent_legal_entities a on a.id=c.agent_legal_entity_key left join portal_private.documents d on d.id=c.document_key where c.lifecycle_state='ACTIVE'::portal_private.lifecycle_state_enum order by c.updated_at desc limit 50;
...
-- Reconcile pre-existing control row with the actual current snapshot publication flags.
