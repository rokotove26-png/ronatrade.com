-- Preserve the existing authorization and publication-eligibility checks while
-- making the client+contract+intent key a transactional idempotency boundary.
...
create or replace function portal_private.server_client_submit_application_v12(
  ...
  p_delivery_period_from date,p_delivery_period_to date,p_idempotency_key text,
...
begin
  ...
  if btrim(coalesce(p_destination_country,''))='' or btrim(coalesce(p_destination_station,''))=''
    then raise exception 'DESTINATION_REQUIRED'; end if;
  if p_delivery_period_to is not null and p_delivery_period_from is not null and p_delivery_period_to<p_delivery_period_from
    then raise exception 'DELIVERY_PERIOD_INVALID'; end if;
  if btrim(coalesce(p_idempotency_key,''))='' or length(p_idempotency_key)>160
    then raise exception 'IDEMPOTENCY_REQUIRED'; end if;
  ...
  where au.session_allowed and 'CLIENT'=any(au.roles)
    ...
    and pi.item_type::text='PRICE' and pi.distribution_allowed
    ...
    and ((pub.audience<>'SELECTED_CLIENTS' and pi.audience<>'SELECTED_CLIENTS') or exists(
      select 1 from portal_private.publication_client_targets pct where pct.publication_key=pub.id
      and pct.client_key=cl.id and (pct.target_scope='PUBLICATION' or (pct.target_scope='ITEM' and pct.publication_item_key=pi.id))))
  ...
  if p_price_mode::text='ACCEPT_PUBLISHED_PRICE' then
    ...
    if p_proposed_price is null or p_proposed_price<=0 or p_proposed_currency is null then raise exception 'PROPOSED_PRICE_REQUIRED'; end if;
  ...
  fingerprint:=encode(extensions.digest(jsonb_build_object('publication_item_id',p_publication_item_id,
    'quantity',p_quantity_tonnes,'price_mode',p_price_mode::text,'proposed_price',p_proposed_price,
    'proposed_currency',p_proposed_currency,'country',btrim(p_destination_country),'station',btrim(p_destination_station),
    'period_from',p_delivery_period_from,'period_to',p_delivery_period_to)::text,'sha256'),'hex');
  ...
  select * into existing from portal_private.client_applications a
  ...
  if found then
    -- Legacy receipts did not retain all request fields. Do not invent their fingerprint.
  ...
  insert into portal_private.client_applications
    ...
    select app_id,r.client_key,r.contract_key,pi.publication_key,r.publication_item_key,r.product,p_quantity_tonnes,
      ...
      'V1_2_IDEMPOTENCY:'||p_idempotency_key from portal_private.publication_items pi
      where pi.id=r.publication_item_key returning id into app_key;
  insert into portal_private.application_lines
    (application_key,line_no,publication_item_key,product,quantity_tonnes,price_mode,published_price,proposed_price,currency,source_mode)
...
begin
  if jsonb_typeof(p_body)<>'object' or jsonb_typeof(p_body->'applicationDetails')<>'object'
  ...
  select c.id,t.id,au.portal_user_id into cl,ct,user_key
    from portal_private.resolve_portal_auth(p_auth_user,p_session_id) au
    ...
    where au.session_allowed and 'CLIENT'=any(au.roles)
      and portal_private.client_user_has_contract_access(au.portal_user_id,t.id,now());
  ...
  if found then
    ...
    if details->>'message_type'<>'APPLICATION_DETAILS_V5'
    ...
    select * into app from portal_private.server_client_submit_application_v12(p_auth_user,p_session_id,
      p_body->>'clientId',p_body->>'contractId',(p_body->>'publicationItemId')::uuid,
      (p_body->>'quantityTonnes')::numeric,(p_body->>'priceMode')::portal_private.price_mode_enum,
      nullif(p_body->>'proposedPrice','')::numeric,nullif(p_body->>'proposedCurrency','')::character,
      p_body->>'destinationCountry',p_body->>'destinationStation',
      nullif(p_body->>'deliveryPeriodFrom','')::timestamptz::date,
      nullif(p_body->>'deliveryPeriodTo','')::timestamptz::date,key,p_request_id,p_correlation_id);
    ...
    select * into ev from portal_private.server_submit_reverse_event(p_auth_user,p_session_id::text,
      'CLIENT_MESSAGE_SUBMIT','APPLICATION','APPLICATION',app.application_id,
      p_body->>'clientId',p_body->>'contractId',null,details,key||'-DETAILS',p_request_id,p_correlation_id);
    insert into portal_private.client_application_bundle_receipts_v2
      ...
      values(cl,ct,key,fp,app.application_id,ev.event_id);
  end if;
  select ci.intake_id,ci.durable_id,ci.source_record_id,ci.source_submitted_at,ci.routing_state
    into i from portal_private.client_intake_v1 ci join portal_private.client_applications a on a.id=ci.application_key
    where a.application_id=app.application_id and ci.source_kind='CLIENT_APPLICATION';
  if i.intake_id is null or not exists(select 1 from portal_private.client_intake_task_links_v1 l
    where l.intake_id=i.intake_id) then raise exception 'APPLICATION_DURABILITY_OR_ROUTING_MISSING'; end if;
