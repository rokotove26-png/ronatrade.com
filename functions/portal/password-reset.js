...
async function gate(token){try{const r=await fetch(CONTROL,{headers:{authorization:`Bearer ${token}`,accept:'application/json'}});if(!r.ok)return null;const j=await r.json();return j?.ok?j:null}catch{return null}}
export async function onRequestGet({request}){
  const c=cookies(request.headers.get('cookie'));
  if(c[RESET_COOKIE]!=='1')return redirect('/portal/login?passwordChange=required');
  const user=await currentUser(c[ACCESS_COOKIE]);
  if(!user||String(user.id||'')!==OWNER_ID||String(user.email||'').toLowerCase()!==OWNER_EMAIL)return redirect('/portal/login?passwordChange=expired');
  const state=await gate(c[ACCESS_COOKIE]);
  if(!state)return new Response(page('Сервис обязательной смены пароля временно недоступен.'),{status:503,headers:SECURITY});
  if(!state.must_change_password)return redirect('/portal/select');
  return new Response(page(),{status:200,headers:SECURITY});
}
