...
function enableExistingAgentAccessOption(){try{if(window.__RONA_ADMIN_AGENT_ACCESS_READY__!==true)return;const select=document.getElementById('newRole');if(!select)return;for(const option of Array.from(select.options||[])){if(String(option.textContent||'').trim().startsWith('Агент'))option.disabled=false}}catch(_e){}}
function installAgentAccessActivation(){if(window.__RONA_ADMIN_AGENT_ACCESS_ACTIVATION__)return;window.__RONA_ADMIN_AGENT_ACCESS_ACTIVATION__=true;document.addEventListener('click',()=>queueMicrotask(enableExistingAgentAccessOption),true)}
