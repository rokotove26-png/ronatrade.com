...
async function askAdminPassword(firstLabel='Установите первоначальный пароль для учётной записи'){const dialogs=window.RONA_ADMIN_DIALOGS;if(!dialogs||typeof dialogs.password!=='function')adminPasswordError('ADMIN_DIALOG_UNAVAILABLE');return String(await dialogs.password(firstLabel))}
...
async function setAccessUserPassword(req){const password=String(req.password||await askAdminPassword('Установите новый пароль'));const j=await call('/access/users/'+encodeURIComponent(req.userId)+'/password',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({password})});await refresh();return j}
