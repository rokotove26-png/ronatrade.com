...
async function refresh(){const pair=await Promise.all([coreBootstrap(),call('/bootstrap')]);const authority=pair[1].data||{};applyLiveContracts(authority);applySignedGate(authority);applyAccess(authority);window.__RONA_ADMIN_LIVE_SNAPSHOT__={core:pair[0],authority:authority,at:new Date().toISOString()};window.__RONA_ADMIN_LIVE_ERROR__=null;window.__RONA_ADMIN_LIVE_READY__=true;return window.__RONA_ADMIN_LIVE_SNAPSHOT__}
