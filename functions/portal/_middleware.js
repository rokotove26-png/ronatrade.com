...
function adminPasswordError(code){const e=new Error(code);e.code=code;throw e}
function askAdminPassword(firstLabel='Установите первоначальный пароль для учётной записи'){const first=window.prompt(firstLabel+'\nНе менее 10 символов: заглавная и строчная буквы, цифра и специальный символ.');if(first===null)adminPasswordError('ADMIN_PASSWORD_CANCELLED');const second=window.prompt('Повторите пароль');if(second===null)adminPasswordError('ADMIN_PASSWORD_CANCELLED');if(first!==second)adminPasswordError('ADMIN_PASSWORD_MISMATCH');return String(first)}
async function createAccessUser(req){const initialPassword=String(req.initialPassword||askAdminPassword());const payload=Object.assign({},req,{email:String(document.querySelector('#newEmail')?.value||req.email||'').trim(),phone:String(document.querySelector('#newPhone')?.value||req.phone||'').trim(),initialPassword});const j=await call('/access/users',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});await refresh();return j}
async function setAccessUserPassword(req){const password=String(req.password||askAdminPassword('Установите новый пароль'));const j=await call('/access/users/'+encodeURIComponent(req.userId)+'/password',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({password})});await refresh();return j}
...
async function unblockAccessUser(req){const j=await call('/access/users/'+encodeURIComponent(req.userId)+'/unblock',{method:'POST',headers:{'content-type':'application/json'},body:'{}'});await refresh();return j}
window.__RONA_PORTAL_BACKEND__=Object.freeze({attachSignedContractToExistingContract,replaceSignedContractVersion,downloadSignedContract,createAccessUser,setAccessUserPassword,linkContractsToUser,revokeBinding,restoreBinding,blockAccessUser,unblockAccessUser,syncCanonical:async()=>{await refresh();return{ok:true,status:'SERVER_READ_REFRESHED'}}});
