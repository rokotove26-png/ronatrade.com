...
const CLIENT_OBSOLETE_DS_INVOICE_GUARD = `<script id="rona-client-obsolete-ds-invoice-guard-v1">(()=>{const norm=v=>String(v??'').replace(/\\s+/g,' ').trim().toLocaleLowerCase('ru-RU');const obsolete=v=>/^(?:дс|ds)\\s*(?:(?:и|and|\\/|&)\\s*)?(?:инвойс(?:ы)?|invoices?)$/i.test(norm(v));const remove=()=>{const controls=[...document.querySelectorAll('a,button,[role="tab"],[role="menuitem"],li')];for(const n of controls){if(!obsolete(n.textContent))continue;(n.closest('li,[role="menuitem"]')||n).remove()}const oldHeading=[...document.querySelectorAll('h1,h2,h3')].find(n=>obsolete(n.textContent));if(oldHeading){const deals=[...document.querySelectorAll('a,button,[role="tab"],[role="menuitem"]')].find(n=>norm(n.textContent)==='сделки');if(deals)try{deals.click()}catch{}}};window.__RONA_REMOVE_OBSOLETE_DS_INVOICES__=remove;if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',remove,{once:true});else remove();window.addEventListener('pageshow',remove,{passive:true});document.addEventListener('click',()=>{setTimeout(remove,0);setTimeout(remove,120);setTimeout(remove,320)},true);document.addEventListener('change',()=>{setTimeout(remove,0);setTimeout(remove,160)},true)})();<\/script>`;
...
export async function onRequest(context) {
  ...
  headers.set('x-rona-client-admin-sync','role-v3-contract-authoritative-deal-v5-canonical-compact-v2-no-ds-invoices-price-bounded');
  ...
  if(source.includes('rona-client-obsolete-ds-invoice-guard-v1'))return new Response(source,{status:response.status,statusText:response.statusText,headers});
