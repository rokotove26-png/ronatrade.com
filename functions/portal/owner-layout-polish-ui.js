...
window.__RONA_CANONICAL_SECTION_RECOVERY__='20260824-independent-access-analytics-v4';
...
function enforce(){if(busy)return;busy=true;try{if(window.__RONA_CLIENTS_AGENTS_V4_READY__===true){root.classList.add('rona-access-v4-ready');root.classList.remove('rona-access-fallback');root.dataset.ronaAccessRecovery='ready';showOwnedPage('#page-access','#rona-ca4')}if(window.__RONA_ANALYTICS_V2_READY__===true){root.classList.add('rona-analytics-v2-ready');root.classList.remove('rona-analytics-fallback');root.dataset.ronaAnalyticsRecovery='ready';showOwnedPage('#page-analytics','#rona-analytics-v2')}}finally{busy=false}}
function watchPages(){['#page-access','#page-analytics'].forEach(function(sel){var p=document.querySelector(sel);if(!p||p.__ronaCanonicalOwnerWatch)return;p.__ronaCanonicalOwnerWatch=true;new MutationObserver(function(){queueMicrotask(enforce)}).observe(p,{childList:true,subtree:true,attributes:true,attributeFilter:['style','class','aria-hidden']})});document.addEventListener('click',function(ev){var b=ev.target&&ev.target.closest&&ev.target.closest('#nav button[data-page]');if(!b||!['access','analytics'].includes(String(b.dataset.page||'')))return;[0,60,160,360,800,1600].forEach(function(ms){setTimeout(enforce,ms)})},true)}
async function access(){try{await load('/portal/clients-agents-canonical-guard-ui?v=20260824-recovery-4','access-guard');try{await waitFor(function(){return window.__RONA_ACCESS_FUNCTIONAL_ARCHIVE_READY__===true||window.__RONA_CLIENTS_AGENTS_V4_READY__===true},'ACCESS_ARCHIVE_OR_READY',7000)}catch(e){window.__RONA_ACCESS_RECOVERY_ARCHIVE_WARNING__=String(e&&e.message||e)}if(window.__RONA_CLIENTS_AGENTS_V4_READY__===true){enforce();return}await load('/portal/clients-agents-v4-ui?v=20260824-2018-home-parity-v1-recovery-4','access-v4');await waitFor(function(){return window.__RONA_CLIENTS_AGENTS_V4_READY__===true},'ACCESS_V4',7000);enforce()}catch(e){root.classList.add('rona-access-fallback');root.dataset.ronaAccessRecovery='fallback';window.__RONA_ACCESS_RECOVERY_ERROR__=String(e&&e.message||e);console.error('[RONA access independent recovery]',e)}}
async function analytics(){try{await load('/portal/analytics-v2-ui?v=20260824-v3-market-rona-lpg-recovery-4','analytics-v2');await waitFor(function(){return window.__RONA_ANALYTICS_V2_READY__===true},'ANALYTICS_V2',7000);enforce()}catch(e){root.classList.add('rona-analytics-fallback');root.dataset.ronaAnalyticsRecovery='fallback';window.__RONA_ANALYTICS_RECOVERY_ERROR__=String(e&&e.message||e);console.error('[RONA analytics independent recovery]',e)}}
