...
function installStyle(){
  for(const id of ['ronaAdminPaymentsStatusBadgesV1','ronaAdminPaymentsStatusBadgesV2','ronaAdminPaymentsStatusBadgesV3','ronaAdminPaymentsStatusIndicatorsV4','ronaAdminPaymentsStatusIndicatorsV5'])document.getElementById(id)?.remove();
  if(document.getElementById('ronaAdminPaymentsPaymentFactStatusV6'))return;
...
function setBusinessStatus(status,raw){
  ...
  if(value==='CONDITIONAL'||value==='УСЛОВНО'||value==='УСЛОВНОЕ'){
    if(String(status.textContent||'').trim()!=='Условное')status.textContent='Условное';
    status.dataset.businessTone='conditional';
  }
}
function setPaymentFact(deal,fact,status){
  ...
  el.dataset.paymentFactTone=fact.tone;
  if(String(el.textContent||'').trim()!==fact.label)el.textContent=fact.label;
