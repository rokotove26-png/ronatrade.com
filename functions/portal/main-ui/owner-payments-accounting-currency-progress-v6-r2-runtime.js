...
const RENDER_HOOK_TO="window.__RONA_OWNER_PAYMENTS_V6_RENDER__=render;window.__RONA_OWNER_PAYMENTS_V6_REFRESH__=()=>refreshAfter();";
...
for(const [name,token] of Object.entries({VERSION_FROM,RENDER_ANCHOR,GRID_FROM,PURPOSE_FROM,RENDER_HOOK_FROM}))if(!ownerPaymentsV6AuthorityRuntime.includes(token))throw new Error('OWNER_PAYMENTS_V6_R2_SOURCE_DRIFT_'+name);

const ownerPaymentsV6R2Runtime=ownerPaymentsV6AuthorityRuntime
  .replace(VERSION_FROM,VERSION_TO)
  .replace(RENDER_ANCHOR,SUMMARY_HELPERS+RENDER_ANCHOR)
