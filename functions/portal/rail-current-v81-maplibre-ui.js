...
function ensureRailCompactDarkStyle(){
  ...
  var s=el('style');s.id='ronaRailCompactDarkStyle';s.textContent=[
    ...
    '.rona-rail-v4-work>.rona-rail-v4-card{height:960px!important;max-height:960px!important;min-height:960px!important;overflow:hidden!important;align-self:start!important;display:flex!important;flex-direction:column!important}',
    ...
    '.rona-rail-v7-control--mode{width:auto!important;min-width:43px!important;padding:0 7px!important;font-size:8.6px!important;letter-spacing:.02em!important}',
    '.rona-rail-v7-control.is-active{background:#17495d!important;border-color:rgba(93,214,244,.72)!important;color:#fff!important}',
    '.rona-rail-v7-legend{z-index:8!important;right:8px!important;top:8px!important;max-width:215px!important;padding:7px 8px!important;border-color:rgba(93,214,244,.18)!important;background:rgba(4,18,29,.91)!important;box-shadow:0 5px 18px rgba(0,0,0,.26)!important;color:#dff8ff!important}',
...
function removeRailTariffPanel(){
  try{
    var page=q('#page-monitoring');if(!page)return false;
    var removed=false;
    qa('.rona-rail-tariff-section,.rona-rail-v4-matrix-preserved,[data-rail-subsection="tariffs"]',page).forEach(function(node){
      if(node&&node.parentNode){node.remove();removed=true}
    });
    var hs=qa('h1,h2,h3,h4,h5,.section-title,.card-title',page);
    for(var i=0;i<hs.length;i++){
      var t=String(hs[i].textContent||'').replace(/\s+/g,' ').trim().toLowerCase();
      if(t.indexOf('матриц')<0||t.indexOf('тариф')<0)continue;
      var matrix=hs[i].closest('section,.rona-owner-card,.card,.panel,[data-section]')||hs[i].parentElement;
      if(matrix&&matrix.parentNode){matrix.remove();removed=true}
    }
    window.__RONA_RAIL_TARIFF_MATRIX_REMOVED__=true;
    return removed
  }catch(_e){return false}
}
var RONA_RAIL_FALLBACK_DESKTOP_CARD_HEIGHT=960;
function railFrozenDesktopCardHeight(left,right){
  var frozen=Number(window.__RONA_RAIL_FROZEN_DESKTOP_CARD_HEIGHT__);
  if(Number.isFinite(frozen)&&frozen>=320)return Math.round(frozen);
  var selectedId=String(window.__RONA_RAIL_SELECTED_DEAL_ID__||'');
  if(selectedId==='DEAL-2026-004'&&left&&right){
    var leftRect=left.getBoundingClientRect(),rightRect=right.getBoundingClientRect();
    var measured=Math.round(leftRect.bottom-rightRect.top);
    if(measured>=700&&measured<=1400){
      window.__RONA_RAIL_FROZEN_DESKTOP_CARD_HEIGHT__=measured;
      window.__RONA_RAIL_MAP_HEIGHT_BASELINE__={
        mode:'OWNER_FRAME_BOTTOM_FROZEN',
        sourceDealId:selectedId,
  ...
  return RONA_RAIL_FALLBACK_DESKTOP_CARD_HEIGHT
}
function alignRailMapHeightToOperations(){
  try{
    ...
    var fixedHeight=railFrozenDesktopCardHeight(left,right);
    ...
    window.__RONA_RAIL_MAP_HEIGHT_ALIGNMENT__={
      mode:'FROZEN_OWNER_FRAME_BOTTOM',
      fixedHeight:fixedHeight,
      dealKey:window.__RONA_RAIL_SELECTED_DEAL_KEY__||null,
...
removeRailTariffPanel();
bindRailMapHeightAlignment();
scheduleRailMapHeightAlignment();
[120,420,900].forEach(function(ms){setTimeout(function(){removeRailTariffPanel()},ms)});
...
window.__RONA_RAIL_CURRENT_REPAIR__=function(){
  try{
    ...
    removeRailTariffPanel();
    sync();
    [0,120,420,900].forEach(function(ms){setTimeout(function(){removeRailTariffPanel()},ms)});
    bumpRailCompactDarkStyle();
    return true
  }catch(_e){return false}
};
