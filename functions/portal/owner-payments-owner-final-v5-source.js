...
function responseHeaders(base){const h=new Headers(base||{});h.set('cache-control','no-store, no-cache, must-revalidate');h.set('pragma','no-cache');h.set('x-content-type-options','nosniff');h.set('x-frame-options','DENY');h.set('x-rona-owner-payments-final-source','AUTHENTICATED_FINANCE_CANON');h.delete('content-length');return h}
...
export async function onRequest(context){
  ...
  const body=await upstream.arrayBuffer(),headers=responseHeaders(upstream.headers);for(const cookie of setCookies)headers.append('set-cookie',cookie);headers.set('x-rona-owner-finance-canon','d6429144-5a12-4a9e-a57e-7d9e345f94a3');return new Response(body,{status:upstream.status,statusText:upstream.statusText,headers});
}
