...
const MUTATION_FUNCTION='rona-owner-payment-authority-v4';
...
export async function onRequest(context){
  ...
  const body=await upstream.arrayBuffer(),out=headers(upstream.headers);for(const cookie of setCookies)out.append('set-cookie',cookie);out.set('x-rona-owner-payment-authority','V4_TRUSTED_SERVER_PATH');return new Response(body,{status:upstream.status,statusText:upstream.statusText,headers:out});
}
