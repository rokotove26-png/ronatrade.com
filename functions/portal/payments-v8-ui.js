...
const BUILD='payments-v8-admin-ui-v3-passport-routing-20260917';
...
function paymentCard(item,index){const beneficiary=text(item?.bank_beneficiary_name||item?.beneficiary_name);const route=text(item?.bank_route_reference||item?.bank_document);const recipient=text(item?.recipient);const dl=node('dl',{class:'rona-payments-v8-passport-kv'},...kv('Получатель',recipient,'recipient'),...kv('Банковский получатель',beneficiary,'bank-beneficiary'),...kv('Банковский маршрут',route,'bank-route'),...kv('Платёж',item?.payment_id||item?.funding_event_id||'—'),...kv('Назначение',item?.purpose||item?.original_payment_purpose||'—'));const amount=item?.amount??item?.funding_amount??item?.allocated_funding_amount;const currency=item?.currency||item?.funding_currency;if(amount!==null&&amount!==undefined)dl.append(...kv('Сумма',fmtNumber(amount)+(currency?' '+upper(currency):'')));return node('article',{class:'rona-payments-v8-passport-card',dataset:{passportItem:String(index),passportKind:item?.kind||'payment'}},node('h3',{text:item?.kind==='settlement'?'Расчётный платёж':item?.kind==='unlinked'?'Расчётный платёж · маршрут требует проверки':'Финансирование'}),dl)}
function openPassport(dealId){const deal=asArray(projection?.deals).find(d=>text(d?.deal_id)===text(dealId));if(!deal)return;installPassportStyle();closePassport();const passport=deal?.payment_passport||null;const overlay=node('div',{id:PASSPORT_MODAL_ID,class:'rona-payments-v8-passport-overlay',dataset:{passportDealId:deal?.deal_id||'',passportContract:passport?.contract||'missing'}});const modal=node('section',{class:'rona-payments-v8-passport-modal',role:'dialog','aria-modal':'true','aria-labelledby':'ronaPaymentsV8PassportTitle'});const head=node('header',{class:'rona-payments-v8-passport-head'},node('div',{},node('div',{class:'rona-owner-muted',text:'ПАСПОРТ ПЛАТЕЖА'}),node('h2',{id:'ronaPaymentsV8PassportTitle',text:deal?.deal_id||'Сделка'})),node('button',{type:'button',class:'rona-payments-v8-passport-close',dataset:{paymentsV8PassportClose:'true'},text:'Закрыть'}));const scroll=node('div',{class:'rona-payments-v8-passport-scroll'});if(!passport){scroll.append(node('div',{class:'rona-payments-v8-passport-empty',text:'Платёжный паспорт отсутствует в текущей проекции.'}))}else{scroll.append(node('div',{class:'rona-payments-v8-passport-summary'},node('div',{class:'rona-payments-v8-passport-card'},node('h3',{text:'Получено'}),node('div',{text:money(passport?.funding_received)})),node('div',{class:'rona-payments-v8-passport-card'},node('h3',{text:'Потрачено'}),node('div',{text:money(passport?.funding_spent)})),node('div',{class:'rona-payments-v8-passport-card'},node('h3',{text:'Остаток'}),node('div',{text:money(passport?.funding_remaining)}))));const items=passportItems(passport);scroll.append(node('h3',{class:'rona-payments-v8-passport-section-title',text:'Платежи и банковский маршрут'}));if(!items.length)scroll.append(node('div',{class:'rona-payments-v8-passport-empty',text:'Платёжные события в текущем паспорте отсутствуют.'}));else{const list=node('div',{class:'rona-payments-v8-passport-list'});items.forEach((item,index)=>list.append(paymentCard(item,index)));scroll.append(list)}}modal.append(head,scroll);overlay.append(modal);document.body.appendChild(overlay);document.documentElement.classList.add('rona-payments-v8-passport-open');overlay.querySelector('.rona-payments-v8-passport-close')?.focus();window.__RONA_PAYMENTS_V8_PASSPORT__={status:'OPEN',dealId:deal?.deal_id||null,contract:passport?.contract||null,openedAt:new Date().toISOString()}}
...
function render(){
  ...
  try{
    ...
    window.__RONA_PAYMENTS_V8_UI__={status:'READY',owner:OWNER,sourceAsOf:projection.source_as_of||null,dealCount:asArray(projection.deals).length,moneyDisplayContract:globalThis.__RONA_PAYMENTS_MONEY_DISPLAY__?.contract||null,passportModal:'CANONICAL_V8',renderedAt:new Date().toISOString()};
    return true;
...
document.addEventListener('click',event=>{const passport=event.target?.closest?.('[data-payments-v8-passport-deal]');if(passport){event.preventDefault();openPassport(passport.dataset.paymentsV8PassportDeal);return}if(event.target?.closest?.('[data-payments-v8-passport-close]')||event.target?.id===PASSPORT_MODAL_ID){event.preventDefault();closePassport();return}const b=event.target?.closest?.('#nav button[data-page="payments"],[data-page="payments"]');if(b)scheduleRender()},true);
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&document.getElementById(PASSPORT_MODAL_ID))closePassport()},true);
...
export async function onRequest(){
  return new Response(SCRIPT,{status:200,headers:{
    ...
    'x-rona-payments-passport-routing':'counterparty-beneficiary-fallback-v1',
