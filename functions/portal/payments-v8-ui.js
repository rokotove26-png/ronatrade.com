...
async function load(){
  ...
  loading=(async()=>{try{const r=await fetch(ENDPOINT,{credentials:'same-origin',cache:'no-store',headers:{accept:'application/json','cache-control':'no-store'}});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error('PAYMENTS_V8_BOOTSTRAP_HTTP_'+r.status);const p=extract(j);if(!p)throw new Error('PAYMENTS_V8_PROJECTION_MISSING');projection=p;window.__RONA_PAYMENTS_V8_UI_ERROR__=null;disableLegacyPassportRuntime();bridgeProjection();return p}catch(error){window.__RONA_PAYMENTS_V8_UI_ERROR__=String(error&&error.message?error.message:error);scheduleRetry();return null}finally{loading=null}})();
  return loading;
}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{disableLegacyPassportRuntime();load()},{once:true});else{disableLegacyPassportRuntime();load()}
})();`;
