...
function responseHeaders(base){const h=new Headers(base||{});h.set('cache-control','no-store, no-cache, must-revalidate');h.set('pragma','no-cache');h.set('x-content-type-options','nosniff');h.set('x-frame-options','DENY');h.set('x-rona-owner-payments-v4-source','AUTHENTICATED_FINANCE_READ');h.delete('content-length');return h}
async function callSource(token,request){const h=new Headers({authorization:`Bearer ${token}`,accept:'application/json'});const requestId=request.headers.get('x-request-id');if(requestId)h.set('x-request-id',requestId);return fetch(`${SUPABASE_URL}/functions/v1/${SOURCE_FUNCTION}/admin/sync`,{method:'GET',headers:h,cache:'no-store'})}
export async function onRequest(context){
  ...
  const body=await upstream.arrayBuffer(),headers=responseHeaders(upstream.headers);for(const cookie of setCookies)headers.append('set-cookie',cookie);headers.set('x-rona-owner-payments-v4-backend',SOURCE_FUNCTION);return new Response(body,{status:upstream.status,statusText:upstream.statusText,headers});
}
