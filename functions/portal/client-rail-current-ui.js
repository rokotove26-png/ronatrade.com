...
const START_TO="function start(){ensureClientCanonicalOwnerStyle();ensureStyle();ensureV6Style();if(!ensureClientRailMount())return;snapshot=null;paint();bind();waitAdminReady();window.__RONA_CLIENT_RAIL_REFRESH__=function(){return sync()}}";
...
async function clientRailFetch(url,required){try{var source=url.indexOf('/shipments')>=0?'client-rail-current-ui:shipments':'client-rail-current-ui:rail',r=await fetch(url,{credentials:'same-origin',cache:'no-store',headers:{accept:'application/json','x-rona-client-source':source}}),j=await r.json().catch(function(){return{}});if(required&&(!r.ok||j&&j.ok===false))throw new Error(String(j&&j.code||'HTTP_'+r.status));return{ok:r.ok&&(!j||j.ok!==false),payload:j||{},status:r.status}}catch(e){if(required)throw e;return{ok:false,payload:{code:String(e&&e.message||e||'REQUEST_FAILED')},status:0}}}
