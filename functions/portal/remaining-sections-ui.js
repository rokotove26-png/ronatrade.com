...
function radioCanonicalSeed(snapshot){
  if(radioCanonicalState.loadedAt||!snapshot||typeof snapshot!=='object')return false;
  const hasItems=Array.isArray(snapshot.radio_messages),hasClients=Array.isArray(snapshot.radio_clients);
  if(!hasItems&&!hasClients)return false;
  const items=hasItems?snapshot.radio_messages:[],clients=hasClients?snapshot.radio_clients:[];
  radioCanonicalState.items=items;radioCanonicalState.clients=clients;radioCanonicalState.signature=radioCanonicalSignature(items,clients);
  radioCanonicalState.loadedAt=Date.now()-16000;radioCanonicalState.error=null;
