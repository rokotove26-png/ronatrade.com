...
function radioRoot(){const p=page('radio');if(!p)return null;style();let r=q(':scope>.rona-rs-root[data-kind="radio"]',p);if(!r){r=el('div','rona-rs-root');r.dataset.kind='radio';p.append(r)}let title=qa('h1,h2,h3,h4,h5,h6,.rona-visual-title,[class*="title"],[class*="Title"]',p).find(n=>!r.contains(n)&&!n.closest('.rona-visual-hero,[data-rona-remaining-r2],.rona-rs-root')&&norm(n.textContent)==='радиорубка')||null;let header=null;if(title){header=title.closest('header,.page-head,.section-head,[data-rona-page-head]')||title;if(header===p||r.contains(header)||header.closest('.rona-visual-hero,[data-rona-remaining-r2],.rona-rs-root'))header=null}if(!header){header=qa(':scope>*',p).find(n=>n!==r&&!n.classList.contains('rona-rs-loading')&&!n.matches('.rona-visual-hero,[data-rona-remaining-r2],.rona-rs-root')&&!n.querySelector('.rona-visual-hero,[data-rona-remaining-r2],.rona-rs-root')&&(()=>{const t=norm(n.textContent);return t.includes('радиорубка')&&t.length<240})())||null}if(header&&header.parentElement!==p)p.insertBefore(header,r);r.replaceChildren();p.classList.remove('rona-rs-gated');q(':scope>.rona-rs-loading',p)?.remove();qa(':scope>*',p).forEach(n=>{if(n===r)return;if(n===header){n.style.removeProperty('display');n.removeAttribute('aria-hidden');return}n.style.display='none'});if(header)header.after(r);return r}
function renderRadio(){const d=snap();if(!d)return;radioStyle();const xs=Array.isArray(d.radio)?d.radio:[],r=radioRoot();if(!r)return;r.classList.add('rona-radio-command');const count=k=>xs.filter(x=>String(x.item_kind||'').toUpperCase()===k).length;
...
`;
  ...
  headers.set('cache-control','no-store, no-cache, must-revalidate');headers.set('pragma','no-cache');headers.set('expires','0');headers.set('x-rona-remaining-sections','r2-radio-native-header-command-body-v3');headers.set('x-rona-radio-owner','native-header-direct-body-v3');headers.set('x-rona-market-news-owner','dedicated-current-content-health-v6');headers.delete('content-length');headers.delete('etag');
