...
`;
  ...
  const forbidden=["'аналитика':'analytics'","'новости топливного рынка снг':'news'",'function renderAnalytics(){','function renderNews(){','function publicationCard(){',"root('analytics'","root('news'","kind==='analytics'",'ronaMarketNewsTopRuntimeV8','__RONA_MARKET_NEWS_TOP_RUNTIME_V8__'];
  ...
  headers.set('cache-control','no-store, no-cache, must-revalidate');headers.set('pragma','no-cache');headers.set('expires','0');headers.set('x-rona-remaining-sections','r2-radio-direct-mission-control-v2');headers.set('x-rona-radio-owner','remaining-sections-direct-render-v2');headers.set('x-rona-market-news-owner','dedicated-current-content-health-v6');headers.delete('content-length');headers.delete('etag');
