...
function radioRoot(){const p=page('radio');if(!p)return null;style();let r=q(':scope>.rona-rs-root[data-kind="radio"]',p);if(!r){r=el('div','rona-rs-root');r.dataset.kind='radio';p.append(r)}let title=qa('h1,h2,h3,h4,h5,h6,.rona-visual-title,[class*="title"],[class*="Title"]',p).find(n=>!r.contains(n)&&norm(n.textContent)==='радиорубка')||null;let header=null;if(title){header=title.closest('.rona-visual-hero,[class*="hero"],[class*="Hero"],header,[class*="head"],[class*="Head"]')||title.parentElement;if(header===p||r.contains(header))header=null}if(!header){header=qa(':scope>*',p).find(n=>n!==r&&!n.classList.contains('rona-rs-loading')&&(()=>{const t=norm(n.textContent);return t.includes('радиорубка')&&t.length<240})())||null}if(header&&header.parentElement!==p)p.insertBefore(header,r);r.replaceChildren();p.classList.remove('rona-rs-gated');q(':scope>.rona-rs-loading',p)?.remove();qa(':scope>*',p).forEach(n=>{if(n===r)return;if(n===header){n.style.removeProperty('display');n.removeAttribute('aria-hidden');return}n.style.display='none'});if(header)header.after(r);return r}
