...
const AGENT_BRIDGE = "<script id=\"rona-g82-agent-same-origin-bridge\">(()=>{'use strict';\nif(window.__RONA_AGENT_MESSAGE_BRIDGE_V1__)return;\nwindow.__RONA_AGENT_MESSAGE_BRIDGE_V1__='AGENT_ADMIN_CANONICAL_MESSAGE_V1';\nconst diag=window.__RONA_AGENT_BOOT_DIAGNOSTIC__={version:'STAGE2A_AGENT_BOOT_DIAGNOSTIC_V1',startedAt:new Date().toISOString(),finished:false,portalReady:false,httpStatus:null,data:null,bootResult:null,agentId:null,error:null,failReason:null};\nlet loading=false;\nconst sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));\nasync function waitPortal(){\n  for(let i=0;i<200;i++){\n    if(window.RONA_AGENT_PORTAL&&typeof window.RONA_AGENT_PORTAL.boot==='function'){diag.portalReady=true;return window.RONA_AGENT_PORTAL}\n    await sleep(25)\n  }\n  throw new Error('AGENT_PORTAL_RUNTIME_NOT_READY')\n}\nfunction snapshotFailure(){\n  diag.agentId=window.RONA_AGENT_PORTAL?.getAgentId?.()||null;\n  diag.failReason=String(document.querySelector('#page-home .red-note')?.textContent||document.querySelector('#topScope')?.textContent||'').trim().slice(0,500)||null;\n}\nasync function boot(){\n  if(loading)return;loading=true;diag.finished=false;diag.error=null;\n  try{\n    const portal=await waitPortal();\n    const r=await fetch('/portal/api/v1/agent/bootstrap',{credentials:'same-origin',cache:'no-store',headers:{accept:'application/json','x-rona-client-source':'AGENT_CANONICAL_MESSAGE_V1'}});\n    diag.httpStatus=r.status;\n    if(r.status===401){diag.error='PORTAL_ACCESS_DENIED';diag.finished=true;location.replace('/portal/login?next=%2Fportal%2Fagent');return}\n    const j=await r.json().catch(()=>null);\n    const data=j?.data||null;\n    diag.data=data?{agentPersonId:String(data.agentPersonId||''),userIdPresent:Boolean(data.userId),displayAliasPresent:Boolean(data.displayAlias),legalEntityId:String(data.legalEntity?.id||''),legalEntityNamePresent:Boolean(data.legalEntity?.name),clients:Array.isArray(data.clients)?data.clients.length:null,deals:Array.isArray(data.deals)?data.deals.length:null,applications:Array.isArray(data.applications)?data.applications.length:null,settlements:Array.isArray(data.settlements)?data.settlements.length:null,documents:Array.isArray(data.documents)?data.documents.length:null,messages:Array.isArray(data.messages)?data.messages.length:null}:null;\n    if(!r.ok||!data){diag.error=String(j?.code||'AGENT_BOOTSTRAP_INVALID');portal.failClosed?.(diag.error);snapshotFailure();diag.finished=true;return}\n    const result=portal.boot(data);\n    diag.bootResult=result===true;\n    snapshotFailure();\n    if(diag.agentId!==String(data.agentPersonId||''))diag.error='AGENT_PORTAL_BOOT_NOT_COMMITTED';\n    diag.finished=true\n  }catch(error){\n    diag.error=String(error?.message||error||'AGENT_BOOT_FAILED').slice(0,500);\n    try{window.RONA_AGENT_PORTAL?.failClosed?.('Не удалось получить подтверждённый серверный контекст.')}catch(_e){}\n    snapshotFailure();diag.finished=true\n  }finally{loading=false}\n}\nasync function submitMessage(event){\n  const button=event.target?.closest?.('#page-messages .actions .btn');\n  if(!button)return;\n  event.preventDefault();event.stopImmediatePropagation();\n  if(button.disabled)return;\n  const page=document.getElementById('page-messages');\n  if(!page)return;\n  const relatedObject=String(page.querySelector('.card-body select')?.value||'').trim();\n  const subjectInput=page.querySelector('.card-body input');\n  const messageInput=page.querySelector('.card-body textarea');\n  const subject=String(subjectInput?.value||'').trim();\n  const message=String(messageInput?.value||'').trim();\n  if(!message){window.toast?.('Введите сообщение.');return}\n  button.disabled=true;\n  const idempotencyKey=crypto.randomUUID();\n  try{\n    const r=await fetch('/portal/api/v1/agent/messages',{method:'POST',credentials:'same-origin',cache:'no-store',headers:{accept:'application/json','content-type':'application/json','x-idempotency-key':idempotencyKey,'x-rona-client-source':'AGENT_CANONICAL_MESSAGE_V1'},body:JSON.stringify({subject,message,relatedObject:relatedObject||null,idempotencyKey})});\n    const j=await r.json().catch(()=>null);\n    if(!r.ok||j?.ok===false)throw new Error(String(j?.code||('HTTP_'+r.status)));\n    if(subjectInput)subjectInput.value='';if(messageInput)messageInput.value='';\n    await boot();window.toast?.('Сообщение отправлено.')\n  }catch(error){console.error('RONA agent messages',error);window.toast?.('Сообщение не отправлено. Повторите попытку.')}\n  finally{button.disabled=false}\n}\ndocument.addEventListener('click',submitMessage,true);\nif(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else queueMicrotask(boot);\nwindow.addEventListener('pageshow',()=>queueMicrotask(boot),{passive:true});\n})();</script>";
...
const RADIO_BROADCAST_RUNTIME = '<script id="rona-portal-radio-broadcast-v1" src="/assets/portal-runtime/portal-radio-broadcast-v1.js?v=20260925-stage2c1-v1" defer data-rona-radio="stage2c1"></script>';
...
async function serveStaticProtected(context, session, kind) {
  ...
  if(kind==='client'){
    ...
    const transformed=new HTMLRewriter()
      ...
      .on('body',new BodyAppend(clientPresence+RADIO_BROADCAST_RUNTIME))
      .transform(response);
    return secureResponse(transformed,session.setCookies,true);
  }
  const agentPresence=impersonation?.data?'':presenceBridge('AGENT');
  const transformed = impersonation?.data
    ? new HTMLRewriter().on('head',new HeadPrepend(bridge)).on('body',new BodyAppend(AGENT_BRIDGE+RADIO_BROADCAST_RUNTIME)).transform(response)
    : new HTMLRewriter().on('body',new BodyAppend(AGENT_BRIDGE+agentPresence+RADIO_BROADCAST_RUNTIME)).transform(response);
  return secureResponse(transformed, session.setCookies, true);
}
