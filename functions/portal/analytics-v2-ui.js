...
;(()=>{'use strict';
  ...
  function render(){
    ...
    const kpis=el('div','an2-kpis');kpis.append(kpi('Platts',dateRu(src.latest),'Последний подтверждённый источник'),kpi('Argus','Нет ряда','Только требуемый Argus European Products / EUROBOB Oxy'),kpi('Горизонт',targetMonth,String(chart.chart_semantics||'CURRENT FACT / MTD / FORWARD')));r.append(kpis);
