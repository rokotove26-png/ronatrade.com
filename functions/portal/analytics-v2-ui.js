...
const RENDER_TO="function render(){const r=ensureRoot();if(!r)return false;r.replaceChildren();const title=el('div','rona-global-sticky-title rona-analytics-canonical-title');title.dataset.page='analytics';title.append(el('div','rona-global-sticky-title-text rona-analytics-canonical-title-text','Аналитика'));r.append(title);header(r);topKpis(r);controls(r);marketBlock(r);ronaPriceForecast(r);commentary(r);for(const x of qa('.an2-market-forecast',r))x.classList.add('rona-owner-card');document.documentElement.classList.add('rona-analytics-v2-ready');document.documentElement.dataset.ronaAnalyticsUi='v3-market-rona-bases-lpg';window.__RONA_ANALYTICS_V2_READY__=true;return true}";
