...
const SOURCE_FUNCTION='rona-owner-ai-sync';
const ACCESS_COOKIE='rona_portal_at',REFRESH_COOKIE='rona_portal_rt';
const FINANCE_CANON='eabba23f-70b9-4d40-86ef-3d0578c71d4a';
...
function responseHeaders(base){const h=new Headers(base||{});h.set('cache-control','no-store, no-cache, must-revalidate');h.set('pragma','no-cache');h.set('x-content-type-options','nosniff');h.set('x-frame-options','DENY');h.set('x-rona-owner-payments-v6-source','AUTHENTICATED_FINANCE_CANON');h.delete('content-length');return h}
async function callSource(token,request){const h=new Headers({authorization:`Bearer ${token}`,accept:'application/json'}),requestId=request.headers.get('x-request-id');if(requestId)h.set('x-request-id',requestId);return fetch(`${SUPABASE_URL}/functions/v1/${SOURCE_FUNCTION}/admin/sync`,{method:'GET',headers:h,cache:'no-store'})}
export async function onRequest(context){
  const request=context.request;if(request.method!=='GET')return new Response(JSON.stringify({ok:false,code:'METHOD_NOT_ALLOWED'}),{status:405,headers:responseHeaders({'content-type':'application/json; charset=utf-8'})});
  ...
  let upstream=await callSource(access,request);
  if(upstream.status===401&&refreshToken){const next=await refresh(refreshToken);if(next.ok&&next.data?.access_token&&next.data?.refresh_token){access=next.data.access_token;refreshToken=next.data.refresh_token;setCookies=[accessCookie(access,next.data.expires_in),refreshCookie(refreshToken)];upstream=await callSource(access,request)}}
  const body=await upstream.arrayBuffer(),headers=responseHeaders(upstream.headers);for(const cookie of setCookies)headers.append('set-cookie',cookie);headers.set('x-rona-owner-finance-canon',FINANCE_CANON);headers.set('x-rona-owner-payments-semantics','ADMIN_PAYMENTS_DEAL_ACCOUNTING_CURRENCY_PROGRESS_V6');return new Response(body,{status:upstream.status,statusText:upstream.statusText,headers});
}
