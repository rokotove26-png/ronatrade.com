...
function impersonationEnded(code='IMPERSONATION_SESSION_INVALID'){const h=headers(new Headers({'content-type':'application/json; charset=utf-8','x-rona-impersonation-ended':'1'}));h.append('set-cookie',clearImpersonationCookie());return new Response(JSON.stringify({ok:false,code,returnTo:'/portal/admin'}),{status:409,headers:h})}
