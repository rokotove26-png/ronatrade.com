...
const TO=`function activateRailV5(){paint();setTimeout(function(){sync()},0)}
function bind(){var nav=q('#nav button[data-page="monitoring"]');if(nav&&!nav.__ronaRailV4Bound){nav.__ronaRailV4Bound=true;nav.addEventListener('click',function(){setTimeout(paint,0);setTimeout(paint,120);setTimeout(function(){sync()},350)})}if(!window.__ronaRailV5PageChange){window.__ronaRailV5PageChange=true;window.addEventListener('rona:admin-pagechange',function(ev){if(String(ev&&ev.detail&&ev.detail.page||'')!=='monitoring')return;activateRailV5()})}}
