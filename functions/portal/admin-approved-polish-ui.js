...
window.__RONA_ADMIN_APPROVED_POLISH__='20260828-claims-radio-dialogs-single-owner-v3';
...
function enforceAccessSingleOwner(){
  ...
  page.dataset.ronaAccessUiOwner='clients-agents-current-v4';
  document.documentElement.dataset.ronaAccessOwner='clients-agents-current-v4';
  window.__RONA_APPROVED_ACCESS_OPEN__=false;
}

window.addEventListener('rona:admin-pagechange',ev=>{const page=String(ev?.detail?.page||'');if(page==='claims')ensureStyle();if(page==='messages')enforceRadioSingleOwner();if(page==='access')enforceAccessSingleOwner()});
ensureStyle();installDialogs();installRadioSingleOwner();enforceAccessSingleOwner();
...
export async function onRequest(){
  return new Response(SCRIPT,{status:200,headers:{
    ...
    'x-rona-admin-polish':'claims-radio-dialogs-single-owner-v3',
    'x-rona-access-owner':'clients-agents-current-v4',
    'x-rona-shell-mutation':'claims-layout-radio-single-owner-dialog-service'
  }});
}
