...
function secureHeaders(contentType='application/json; charset=utf-8'){const h=new Headers({'content-type':contentType});for(const[k,v]of Object.entries(SECURITY_HEADERS))h.set(k,v);return h}
...
async function portalEventSubmit(access,request){
  const headers=new Headers({authorization:`Bearer ${access}`,accept:'application/json'});
  for(const name of ['content-type','x-request-id','x-correlation-id','x-idempotency-key']){const value=request.headers.get(name);if(value)headers.set(name,value)}
  return fetch(`${PORTAL_API}/v1/events`,{method:'POST',headers,body:await request.clone().arrayBuffer()});
}
async function authenticatedRequest(request,handler){
  ...
  if(!access)return{response:json({ok:false,code:'PORTAL_ACCESS_DENIED'},401,clearCookies())};
  let r=await handler(access);
  if(r.status===401&&refresh){const next=await authRefresh(refresh);if(next.ok&&next.data?.access_token&&next.data?.refresh_token){access=next.data.access_token;setCookies=tokenCookies(next.data);r=await handler(access)}}
  if(r.status===401)return{response:json({ok:false,code:'PORTAL_ACCESS_DENIED'},401,clearCookies())};
  return{upstream:r,setCookies};
}
function relay(r,setCookies=[]){const h=secureHeaders(r.headers.get('content-type')||'application/json; charset=utf-8');const requestId=r.headers.get('x-request-id');if(requestId)h.set('x-request-id',requestId);for(const c of setCookies)h.append('set-cookie',c);return new Response(r.body,{status:r.status,statusText:r.statusText,headers:h})}
export async function onRequest(context){
  const request=context.request,url=new URL(request.url),base='/portal/api/v1/events';
  if(!sameOrigin(request))return json({ok:false,code:'ORIGIN_DENIED'},403);
  if(request.method==='POST'&&(url.pathname===base||url.pathname===base+'/')){
    const auth=await authenticatedRequest(request,access=>portalEventSubmit(access,request));
    if(auth.response)return auth.response;
    return relay(auth.upstream,auth.setCookies);
  }
  ...
  const auth=await authenticatedRequest(request,access=>rpc(access,eventId));
  if(auth.response)return auth.response;
  const r=auth.upstream,body=await r.json().catch(()=>null);
  if(!r.ok||!body||typeof body!=='object')return json({ok:false,code:'EVENT_STATUS_BACKEND_ERROR'},r.status>=500?503:502,auth.setCookies);
  if(body.found!==true)return json({ok:false,code:String(body.code||'EVENT_NOT_FOUND')},body.code==='EVENT_NOT_FOUND'?404:403,auth.setCookies);
  return json({ok:true,event:body.event},200,auth.setCookies);
}
