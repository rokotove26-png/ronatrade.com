...
export function patchAdminOperationsCommandCenterV5(script){
  ...
  patched=replaceRequired(
    ...
    "  if(uncheckedDocs.length)queueRows.push({tone:'amber',name:'Документы · требуется контроль',meta:String(uncheckedDocs.length)+' документов не отмечены как проверенные',target:'documents'});\n  for(const x of opsAlerts)queueRows.push({tone:ronaFdV5Key(x?.severity)==='CRITICAL'||ronaFdV5Key(x?.severity)==='ERROR'?'red':ronaFdV5Key(x?.severity)==='INFO'?'cyan':'amber',name:String(x?.title||'Операционный сигнал'),meta:String(x?.meta||x?.detail||'Требуется проверка'),target:x?.target||'home',dealId:x?.deal_id||null});\n  const criticalCount=conflicts.length+Number(opsMetrics.critical_total||0);\n  const attentionCount=queueRows.length+Number(opsMetrics.attention_total||0);",
    'operations-alerts'
  );
