...
function withHeaders(response, mode){
  ...
  h.set('x-rona-rail-stable-owner','approved-current-first-fallback-v1');
...
export async function onRequest(context){
  try{
    ...
    if(current?.status===200)return withHeaders(current,'APPROVED_CURRENT_V8_2');
  }catch(_){ }
  try{
    const fallback=await stableRail(context);
    return withHeaders(fallback,'STABLE_CURRENT_V4_FALLBACK');
  }catch(e){
