function currentUiRuntime(){
  ...
  function enforceSelectedPage(){
    ...
    for(const p of Array.from(document.querySelectorAll('main .page[id^="page-"]')))p.classList.toggle('active',p===target);
    for(const b of Array.from(document.querySelectorAll('#nav button[data-page]'))){const on=b===button;b.classList.toggle('active',on);if(on)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current')}
  }
  ...
  function installNavigationStability(){
    ...
    document.addEventListener('click',event=>{const b=event.target?.closest?.('#nav button[data-page]');if(!b)return;armNavigation(String(b.dataset.page||''));queueMicrotask(enforceSelectedPage)},true);
  ...
  async function createAccess(payload){
    for(let i=0;i<20;i++){const backend=window.__RONA_PORTAL_BACKEND__;if(backend&&typeof backend.createAccessUser==='function')return backend.createAccessUser(payload);await sleep(100)}
...
export async function onRequest(){
  return new Response(SCRIPT,{status:200,headers:{
    ...
    'x-rona-clients-agents-ui':'current-only-v2',
    'x-rona-access-create':'client-agent-v2',
    'x-rona-admin-nav-stability':'v2',
    'x-rona-shell-parity':'canonical-home-v2',
