...
export async function onRequestPost({request}){
 if(!sameOrigin(request))return redirect('/portal/recovery?error=invalid',clearCookies());
 const ct=request.headers.get('content-type')||'';let otp='';
 if(ct.includes('application/x-www-form-urlencoded')||ct.includes('multipart/form-data')){const f=await request.formData();otp=String(f.get('otp')||'').trim()}else if(ct.includes('application/json')){const b=await request.json().catch(()=>({}));otp=String(b.otp||'').trim()}
 if(!/^\d{6,10}$/.test(otp))return redirect('/portal/recovery?error=invalid',clearCookies());
 const r=await fetch(`${SUPABASE_URL}/auth/v1/verify`,{method:'POST',headers:{apikey:SUPABASE_PUBLISHABLE_KEY,'content-type':'application/json',accept:'application/json'},body:JSON.stringify({type:'recovery',email:OWNER_EMAIL,token:otp})});
 const data=await r.json().catch(()=>({}));
 if(!r.ok||!data?.access_token||!data?.refresh_token||String(data?.user?.id||'')!==OWNER_ID||String(data?.user?.email||'').toLowerCase()!==OWNER_EMAIL){if(data?.access_token)await logout(data.access_token);return redirect('/portal/recovery?error=invalid',clearCookies())}
