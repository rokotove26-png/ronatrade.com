...
async function sessionMe(accessToken){for(let attempt=0;attempt<2;attempt++){const controller=new AbortController(),timer=setTimeout(()=>controller.abort('RONA_SESSION_TIMEOUT'),3000);try{const r=await fetch(`${PORTAL_API}/session/me`,{headers:{apikey:SUPABASE_PUBLISHABLE_KEY,authorization:`Bearer ${accessToken}`,accept:'application/json'},signal:controller.signal});if(r.ok){const j=await r.json();if(j?.ok&&j?.user)return j}if(r.status===401||r.status===403)return null}catch{}finally{clearTimeout(timer)}if(attempt<1)await new Promise(resolve=>setTimeout(resolve,250));}return null}
