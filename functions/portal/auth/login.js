...
const BOOTSTRAP_CONTROL = `${SUPABASE_URL}/functions/v1/rona-owner-bootstrap-control-20260816`;
const OWNER_AUTH_ID = 'c4a167ae-cd4f-4296-8f13-ef09ced41968';
...
function resetCookie(maxAge = 600) { return `${RESET_COOKIE}=1; Max-Age=${maxAge}; Path=/portal; Secure; HttpOnly; SameSite=Strict`; }
...
function tokenCookies(tokens) { const expires=Math.min(Math.max(Number(tokens?.expires_in||3600),60),7200); return [accessCookie(tokens.access_token,expires),refreshCookie(tokens.refresh_token,604800),`${RESET_COOKIE}=; Max-Age=0; Path=/portal; Secure; HttpOnly; SameSite=Strict`]; }
function bootstrapCookies(tokens) { return [accessCookie(tokens.access_token,600),refreshCookie(tokens.refresh_token,600),resetCookie(600)]; }
...
async function sessionMe(accessToken){try{const r=await fetch(`${PORTAL_API}/session/me`,{headers:{authorization:`Bearer ${accessToken}`,accept:'application/json'}});if(!r.ok)return null;const j=await r.json();return j?.ok&&j?.user?j:null}catch{return null}}
async function bootstrapState(accessToken){try{const r=await fetch(BOOTSTRAP_CONTROL,{headers:{authorization:`Bearer ${accessToken}`,accept:'application/json'}});if(!r.ok)return null;const j=await r.json();return j?.ok?j:null}catch{return null}}
...
export async function onRequestPost({request}){
  ...
  const authId=String(login.data?.user?.id||'');
  if(authId===OWNER_AUTH_ID){
    const state=await bootstrapState(login.data.access_token);
    if(!state){await logout(login.data.access_token);return json({ok:false,code:'PASSWORD_GATE_UNAVAILABLE'},503,clearCookies());}
    if(state.must_change_password){const cookies=bootstrapCookies(login.data);return (request.headers.get('accept')||'').includes('application/json')?json({ok:true,redirect:'/portal/password-reset',must_change_password:true},200,cookies):redirect('/portal/password-reset',cookies);}
  }
  ...
  return (request.headers.get('accept')||'').includes('application/json')?json({ok:true,redirect:target},200,cookies):redirect(target,cookies);
}
