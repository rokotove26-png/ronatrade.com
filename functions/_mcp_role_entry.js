...
function consentScript(segment,nonce){
  ...
  return `<script nonce="${nonce}">(()=>{const form=document.querySelector('form[action="${authorize}"]');if(!form||!globalThis.crypto?.getRandomValues)return;const button=form.querySelector('button[type="submit"]');let busy=false;const encode=b=>{let s='';for(const x of b)s+=String.fromCharCode(x);return btoa(s).replaceAll('+','-').replaceAll('/','_').replace(/=+$/,'')};const showError=code=>{let box=form.querySelector('[data-rona-oauth-error]');if(!box){box=document.createElement('div');box.dataset.ronaOauthError='true';box.style.cssText='margin-top:12px;color:#ffb4b4;font:600 13px/1.4 system-ui,sans-serif';form.append(box)}box.textContent=code==='access_denied'?'Эта учетная запись не имеет прав владельца/администратора RONA Trade для подключения AI-контура.':code==='invalid_request'?'Запрос авторизации устарел. Вернитесь в ChatGPT и нажмите «Переподключить» ещё раз.':'Не удалось подтвердить подключение. Повторите попытку.'};form.addEventListener('submit',async event=>{event.preventDefault();if(busy)return;busy=true;if(button)button.disabled=true;form.querySelector('[data-rona-oauth-error]')?.remove();const bytes=crypto.getRandomValues(new Uint8Array(32)),continuation=encode(bytes),data=new URLSearchParams(new FormData(form));data.set('continuation_nonce',continuation);try{const response=await fetch('${authorize}/prepare',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body:data.toString(),credentials:'omit',cache:'no-store'});if(response.status!==204){const payload=await response.json().catch(()=>null);showError(String(payload?.error||'prepare_failed'));busy=false;if(button)button.disabled=false;return}location.replace('${authorize}/complete?nonce='+encodeURIComponent(continuation));}catch(_e){showError('network_error');busy=false;if(button)button.disabled=false;}});})();</script>`;
}
