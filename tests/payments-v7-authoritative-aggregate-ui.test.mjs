...
assert.deepEqual(zero, { rows: [{ currency: 'USD', amount: 0 }], verify: false });
...
assert.deepEqual(nonzero, { rows: [{ currency: 'USD', amount: 25 }], verify: false });
...
assert.deepEqual(unknown, { rows: [], verify: true });
...
assert.deepEqual(partial, { rows: [{ currency: 'USD', amount: 80 }], verify: true });
...
assert.deepEqual(total, { rows: [{ currency: 'USD', amount: 100 }], verify: false });
