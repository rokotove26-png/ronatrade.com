...
select test_application_v2.expect_error($q$update portal_private.client_applications set status='ACCEPTED_AWAITING_DEAL_REGISTRATION' where application_id=test_application_v2.get('authority_receipt')::jsonb->>'application_id'$q$,'APPLICATION_PRICE_AGREEMENT_REQUIRED');
...
insert into portal_private.owner_application_workflow(application_key,business_status,counter_offer_used,client_counter_response,counter_price,counter_currency)
 select id,'REVIEW',true,'ACCEPTED',617.43,'USD' from portal_private.client_applications
 where application_id=test_application_v2.get('authority_receipt')::jsonb->>'application_id';
update portal_private.client_applications set status='ACCEPTED_AWAITING_DEAL_REGISTRATION'
 where application_id=test_application_v2.get('authority_receipt')::jsonb->>'application_id';
...
-- Active registered Deal is protected. A rejected transaction must not tear its lineage.
...
begin;
update portal_private.client_applications set status='REJECTED' where application_id=test_application_v2.get('authority_receipt')::jsonb->>'application_id';
select test_application_v2.expect_error('set constraints all immediate','APPLICATION_RETIREMENT_BLOCKED');
rollback;
select test_application_v2.check_true((select status::text='DEAL_REGISTERED' from portal_private.client_applications where application_id=test_application_v2.get('authority_receipt')::jsonb->>'application_id'),'blocked active-Deal deletion rolls back the entire lifecycle action');
