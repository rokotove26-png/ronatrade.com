...
test('Admin main UI keeps V7-V8.9 baselines under V9 production recovery',()=>{
  ...
  assert.match(main,/patchAdminOperationsCommandCenterV9/);
  assert.match(main,/admin-operations-command-center-v9\\.js/);
  assert.match(v9,/patchAdminOperationsCommandCenterV89 as patchV89/);
  assert.match(v9,/let patched=patchV89\\(script\\)/);
...
test('V9 bounds startup dependencies and requires canonical Deals Current V4 before READY',()=>{
  const v9=read('functions/portal/admin-operations-command-center-v9.js');
  assert.match(v9,/ronaOpsV90Bounded/);
  assert.match(v9,/OPERATIONS_READ_MODEL/);
  assert.match(v9,/DEALS_MODULE_LOAD/);
  assert.match(v9,/__RONA_ADMIN_LOAD_MODULE__/);
  assert.match(v9,/reason==='INITIAL'\\|\\|reason==='RECOVERY_RETRY'/);
  assert.match(v9,/ADMIN_DEALS_CURRENT_V4/);
  assert.match(v9,/DEALS_CURRENT_V4_REFRESH_UNAVAILABLE/);
  assert.match(v9,/OPERATIONS_PRODUCTION_RECOVERY_V1/);
  assert.match(v9,/ADMIN_OPERATIONS_V9_POLLING_FORBIDDEN/);
  assert.doesNotMatch(v9,/setInterval\\(/);
});
