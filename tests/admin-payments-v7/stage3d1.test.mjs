...
test('AW — PROVIDER READY AFTER INITIAL SEED',async()=>{ const raw=await readAdminPaymentsV7RawSources(readinessPort(true)); assert.equal(raw.capabilities.paymentBusinessAuthorityReady,true); const s=source(true,true,CURRENT,[...BASE_CLAIMS,...seedClaims()]), r=rec(s); assert.equal(buildAdminPaymentsV7Projection(s).owner_exception_queue.length,0); assert.equal(r.find(v=>v.payment_id==='PAYEV-2026-000008').reconciliation_class,'OWNER_ASSERTED_ALLOCATED_SYSTEM_AUTHORITY_NOT_MATERIALIZED'); assert.equal(r.find(v=>v.payment_id==='PAYEV-2026-000009').reconciliation_class,'ASSOCIATED_BANK_FEE'); });
