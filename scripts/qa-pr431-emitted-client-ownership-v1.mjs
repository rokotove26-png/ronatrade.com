...
for(const token of ['ALL_AUTHORIZED_CONTEXT_DIRECTORY','getCompanyDirectory','AUTHORITATIVE_AUTHORIZED_CONTEXT_DIRECTORY_DB','CURRENT_EFFECTIVE_CONTRACTUAL_ONLY','validateCompleteDirectory','CLIENT_COMPANY_DIRECTORY_INCOMPLETE','planDirectory','state.validated','state.active','window.__RONA_CLIENT_COMPANY_DIRECTORY_AUTHORITY__=MARK','materializePendingCard','canonical-directory-pending',"OPEN_AUTHORIZED_DIRECTORY_CONTEXT_PAGESHOW_INVALIDATION",'rona:client-company-directory-invalidated'])if(!directory.includes(token))throw new Error(`PR431_EMITTED_DIRECTORY_CONTRACT_MISSING:${token}`);
for(const forbidden of ['setInterval(','REFRESH_MS=30000'])if(directory.includes(forbidden))throw new Error(`PR431_EMITTED_DIRECTORY_POLLING_FORBIDDEN:${forbidden}`);
