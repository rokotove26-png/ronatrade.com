...
for(const [kind,spec] of Object.entries(ASSETS)){
  const path=join(ROOT,...spec.path.split('/'));
  if(!(await exists(path))) throw new Error(`CANONICAL_ASSET_MISSING: ${spec.path}`);
  const bytes=await readFile(path);
...
const integrity={
  architecture:'CURRENT_ONLY_ADMIN_SHELL_WITH_FROZEN_CANONICAL_ASSETS',
  ...
  visual_transform:'CURRENT_ADMIN_SHELL_REFERENCES_FROZEN_CANONICAL_ASSETS',
...
console.log(`RONA direct build PASS: ${files.length} public files; Admin CURRENT_ONLY ${sha256(adminBytes)} (${adminBytes.length} bytes), no legacy Admin runtime in deployment; Agent ${SOURCES.agent.sha256} CURRENT_ONLY; Client ${SOURCES.client.sha256}; PNG ${ASSETS.png.sha256}/${ASSETS.png.bytes}; SVG ${ASSETS.svg.sha256}/${ASSETS.svg.bytes}.`);