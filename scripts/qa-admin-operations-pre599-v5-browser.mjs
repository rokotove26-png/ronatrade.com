...
const adminBootstrap={
  ...
  applications:[{application_id:'QA-APP-001',client_id:'RONA-QA-C001',legal_name:'QA Client',product:'Дизель',owner_status:'NEW'}],
  deals:[{deal_id:'QA-DEAL-001',client_id:'RONA-QA-C001',legal_name:'QA Client',contract_id:'RONA-QA-CTR-001',business_status:'EXECUTING',stage:'EXECUTING',finance_status:'DUE',resource_status:'CONFIRMED',delivery_status:'IN_PROGRESS',accounting_closure_status:'ACTIVE',next_action_text:'Подтвердить отгрузку',document_count:1,unchecked_document_count:1}],
  dealDocuments:[{document_id:'QA-DOC-001',deal_id:'QA-DEAL-001',document_kind:'INVOICE',authoritative_filename:'qa-invoice.pdf',checked_by_admin:false}],
  documents:[],payments:[],cash:[],
  rail:[{deal_id:'QA-DEAL-001',gu12_number:'QA-GU12-001',wagons:[{wagon_number:'QA-WGN-001',status:'WAITING'}]}],
  publications:[],operationalConflicts:[],claims:[],
  operations:{
    metrics:{critical_total:0,attention_total:1,automation_issues:2,open_tasks:3,pending_reverse_events:4},
    alerts:[{severity:'WARNING',title:'QA operational alert',meta:'Candidate visual regression proof',target:'home',deal_id:'QA-DEAL-001'}],
    tasks:[{task_id:'QA-TASK-001',title:'QA operational task',description:'Verify Flightdeck',status:'OPEN',priority:'HIGH',deal_id:'QA-DEAL-001'}],
    reverseEvents:[{event_id:'QA-EVENT-001',event_type:'QA_REVERSE',processing_state:'PENDING',acknowledgement_state:'NEW',deal_id:'QA-DEAL-001'}],
    railRuntime:[{production_polling_enabled:true,mode:'LIVE'}],
    freshness:{source_as_of:now,deals:now}
  },
...
try{
  ...
  await page.evaluate(()=>window.__RONA_ADMIN_NAVIGATE__('home'));
  await page.locator('#page-home.active').waitFor({state:'visible',timeout:10000});
  ...
  const homeVisual=await page.evaluate(()=>{
    const root=document.querySelector('#page-home .rona-flightdeck-v5');
    const instruments=document.querySelector('#page-home .rona-fd-v5__instruments');
    const gauge=document.querySelector('#page-home .rona-fd-v5-gauge');
    const gaugeLabel=document.querySelector('#page-home .rona-fd-v5-gauge__label');
    const gaugeValue=document.querySelector('#page-home .rona-fd-v5-gauge__value');
    const screenTitle=document.querySelector('#page-home .rona-fd-v5-screen__title');
    const search=document.querySelector('.topbar input.search');
    const searchButton=document.querySelector('.topbar .rona-search-focus');
    const role=document.querySelector('.topbar .role-pill');
    const logout=document.querySelector('.topbar [data-action="logout"]');
    const cols=getComputedStyle(instruments).gridTemplateColumns.trim().split(/\s+/).filter(Boolean);
    return{
      rootClass:root?.className||'',
      owner:root?.getAttribute('data-rona-operations-command-center')||'',
      flightdeck:root?.getAttribute('data-rona-flightdeck')||'',
      gauges:document.querySelectorAll('#page-home .rona-fd-v5-gauge').length,
      instrumentColumns:cols.length,
      gaugeMinHeight:parseFloat(getComputedStyle(gauge).minHeight),
      gaugeLabelFont:parseFloat(getComputedStyle(gaugeLabel).fontSize),
      gaugeValueFont:parseFloat(getComputedStyle(gaugeValue).fontSize),
      screenTitleFont:parseFloat(getComputedStyle(screenTitle).fontSize),
      denseMetrics:document.querySelectorAll('#page-home .rona-ops-v4__metrics').length,
      searchWidth:search?.getBoundingClientRect().width||0,
      searchButton:searchButton?.textContent?.trim()||'',
      role:role?.textContent||'',
      logout:logout?.textContent||'',
      globalSearch:typeof window.__RONA_ADMIN_GLOBAL_SEARCH__,
      refreshMode:window.__RONA_ADMIN_HOME_AUTOREFRESH__||'',
      text:root?.innerText||''
    };
  });
  assert(homeVisual.rootClass.includes('rona-flightdeck-v5'),'pre-599 Flightdeck root missing');
  assert(homeVisual.owner==='v5','V5 ownership marker missing: '+homeVisual.owner);
  assert(homeVisual.flightdeck==='v5-full-rebuild','Flightdeck baseline marker missing: '+homeVisual.flightdeck);
  assert(homeVisual.gauges===6,'expected six canonical KPI gauges, got '+homeVisual.gauges);
  assert(homeVisual.instrumentColumns===3,'desktop KPI grid must be three columns, got '+homeVisual.instrumentColumns);
  assert(homeVisual.gaugeMinHeight>=138,'KPI frame too dense: '+homeVisual.gaugeMinHeight);
  assert(homeVisual.gaugeLabelFont>=12,'KPI label too small: '+homeVisual.gaugeLabelFont);
  assert(homeVisual.gaugeValueFont>=40,'KPI value too small: '+homeVisual.gaugeValueFont);
  assert(homeVisual.screenTitleFont>=17,'panel title too small: '+homeVisual.screenTitleFont);
  assert(homeVisual.denseMetrics===0,'post-599 dense metrics renderer still present');
  assert(homeVisual.searchWidth>=360&&homeVisual.searchWidth<=480.5,'topbar search width regressed: '+homeVisual.searchWidth);
  assert(homeVisual.searchButton.includes('Поиск'),'topbar Search button missing');
  assert(homeVisual.role.includes('Администратор'),'Admin control missing');
  assert(homeVisual.logout.includes('Выход'),'Logout control missing');
  assert(homeVisual.globalSearch==='function','global search function missing');
  assert(homeVisual.refreshMode==='authority-v1-30s-safe','30s authority refresh marker missing: '+homeVisual.refreshMode);
  assert(homeVisual.text.includes('RONA TRADE · OPERATIONS FLIGHTDECK'),'pre-599 Flightdeck identity missing');
  assert(homeVisual.text.includes('AUTOMATION · 2 issues · 3 open tasks · 4 reverse events'),'current automation information missing');
  assert(homeVisual.text.includes('QA operational alert'),'current operational alert missing');

  await mkdir('artifacts',{recursive:true});
  await page.locator('#page-home .rona-flightdeck-v5').screenshot({path:'artifacts/admin-home-pre599-v5.png'});

  await page.evaluate(()=>window.__RONA_ADMIN_GLOBAL_SEARCH__('QA-DEAL-001'));
  const searchLayer=page.locator('#ronaOpsV5SearchLayer');
  await searchLayer.waitFor({state:'visible',timeout:5000});
  assert((await searchLayer.innerText()).includes('Сделка QA-DEAL-001'),'global search did not return exact deal');
  await searchLayer.getByRole('button',{name:/Сделка QA-DEAL-001/}).click();
  await page.waitForFunction(()=>window.__RONA_DEALS_REQUESTED_ID__==='QA-DEAL-001',{timeout:5000});
  await stableSelection(page,'deals',100);
  assert(await page.evaluate(()=>window.__RONA_DEALS_REQUESTED_ID__)==='QA-DEAL-001','exact deal deep-link state missing');

  await clickSection(page,'documents');
  await clickSection(page,'payments');
