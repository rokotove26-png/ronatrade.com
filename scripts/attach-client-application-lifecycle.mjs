...
const homePreload=`<link id="rona-client-home-command-center-preload-v1" rel="preload" as="script" href="${homeSrc}" fetchpriority="high">`;
html=html.slice(0,headClose)+homePreload+criticalGuard+homeGuard+paymentsGuard+html.slice(headClose);
const close=html.toLowerCase().lastIndexOf('</body>');
if(close<0)throw new Error('CLIENT_BODY_CLOSE_MISSING');
const bridge=`<script id="${homeId}" src="${homeSrc}" defer fetchpriority="high"></script><script id="${homeVisualId}" src="${homeVisualSrc}" defer></script><script id="${connectionId}" src="${connectionSrc}" async></script><script id="${id}" src="${src}" defer></script><script id="${paymentsSanitationId}" src="${paymentsSanitationSrc}" defer></script><script id="${paymentsId}" src="${paymentsSrc}" defer></script><script id="${paymentsCanonicalId}" src="${paymentsCanonicalSrc}" defer></script>`;
if(bridge.indexOf(`id="${homeId}"`)>bridge.indexOf(`id="${connectionId}"`))throw new Error('CLIENT_HOME_BOOT_PRIORITY_ORDER_INVALID');
if(!bridge.includes(`id="${connectionId}" src="${connectionSrc}" async`))throw new Error('CLIENT_SERVER_CONNECTION_MUST_NOT_BLOCK_CLIENT_HOME');
...
integrity.client_runtime.server_connection_indicator={id:connectionId,src:connectionSrc,marker:connectionMarker,replaces:'TOPBAR_DEALS_COUNTER',server_signal:'OBSERVED_SAME_ORIGIN_PORTAL_API_TRAFFIC',extra_network_requests:false,loading:'ASYNC_NON_BLOCKING',states:['CHECKING','ONLINE','DEGRADED','OFFLINE']};
