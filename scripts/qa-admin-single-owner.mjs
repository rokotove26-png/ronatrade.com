...
need(has(operationsV89,"patchAdminOperationsCommandCenterV88 as patchV88"),'Operations Command Center V8.9 does not preserve V8.8 baseline');
need(has(operationsV89,"window.__RONA_PAYMENTS_V8_OPEN_PASSPORT__")&&has(operationsV89,"ownerApplication2BFilter=application2BBucket(app)"),'Operations Command Center V8.9 exact object routing is missing');
...
need(has(operationsV87,"OPERATIONS_COMMAND_CENTER_VERSION='v8.7-finance-actions-v1'"),'Operations Command Center V8.7 version marker is missing');
need(has(operationsV87,"patchAdminOperationsCommandCenterV86 as patchV86"),'Operations Command Center V8.7 does not preserve the V8.6 canonical controls baseline');
need(has(operationsV87,"actionDomain==='PAYMENT'?'Оплата'")&&has(operationsV87,"direct&&direct!=='home'&&direct!=='deals'"),'Operations Command Center V8.7 Finance action routing is missing');
...
need(has(operationsV82,"OPERATIONS_COMMAND_CENTER_VERSION='v8.2-action-queue-normalized-v1'"),'Operations Command Center V8.2 version marker is missing');
need(has(operationsV82,"patchAdminOperationsCommandCenterV81 as patchV81"),'Operations Command Center V8.2 does not preserve the V8.1 recovery baseline');
need(has(operationsV82,"taskSourceEventIds=new Set")&&has(operationsV82,"const attentionCount=queueRows.length"),'Operations Command Center V8.2 normalized action queue is missing');
