...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='POST'&&u.pathname===`/portal/admin-authority/contracts/${encodeURIComponent(contractId)}/signed-document/attach`){
    ...
    if(!body.includes(Buffer.from('%PDF')))return json(res,{ok:false,code:'PDF_SIGNATURE_INVALID'},400);
    pdfReady=true;return json(res,{ok:true,data:{contractId,clientId,documentId:'DOC-QA-001',bilateralSignedConfirmed:true,serverConfirmed:true}})
  }
