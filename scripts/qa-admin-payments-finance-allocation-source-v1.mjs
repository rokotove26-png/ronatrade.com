...
assert(main.includes("const PAYMENTS_RUNTIME=String.raw`"),'Payments canonical runtime patch missing');
assert(main.includes("window.__RONA_ADMIN_PAYMENTS_FINANCE_PROJECTION_V1__='20260912-v1'"),'Payments runtime marker missing');
assert(main.includes('canonicalFinanceArray(f.incomingPayments)'),'UI must render trusted incomingPayments');
assert(main.includes('f?.incomingPaymentAllocations'),'UI must consume incomingPaymentAllocations');
assert(main.includes('f?.paymentAllocationSummaries'),'UI must consume server unallocated summaries');
assert(main.includes('f?.dealAllocationTotals'),'UI must consume server deal allocation totals');
assert(main.includes('canonicalFinanceArray(f.outgoingPayments)'),'outgoingPayments must remain distinct');
assert(!main.includes('canonicalFinanceArray(f.payments)'),'UI must not infer incoming rows from generic payments');
assert(!main.includes('frag?.payments'),'UI must not infer allocations from generic payments');
assert(main.includes("String(a.allocation_status||'').toUpperCase()==='VERIFIED'"),'UI must display only VERIFIED deal allocations');
assert(main.includes("String(x.deal_allocation_status||'').toUpperCase()==='CONFIRMED'"),'outgoing KPI must require confirmed deal allocation');
assert(main.includes("financePill('Требует верификации','warn')"),'fail-closed TO_VERIFY presentation missing');
assert(main.includes("type:'search'"),'Payments search control missing');
assert(main.includes("'x-rona-payments-ui':'finance-authoritative-allocation-v1'"),'Payments runtime header marker missing');
assert(!/usd\s*equivalent|market\s*fx|цб\s*рф/i.test(main),'frontend FX synthesis marker detected');
