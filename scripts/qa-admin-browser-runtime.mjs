...
try{
  ...
  const login=modal.locator('[data-rona-access-field="login"]'),email=modal.locator('[data-rona-access-field="email"]');
  assert(await login.count()===1,'access modal: separate login input missing');assert(await email.count()===1,'access modal: separate email input missing');assert(await login.getAttribute('aria-label')==='Единый логин','access modal: login semantic label missing');assert(await email.getAttribute('aria-label')==='Электронная почта','access modal: email semantic label missing');
  const loginLabel=modal.locator('label[data-rona-access-label="Единый логин"] > .rona-access-field-label'),emailLabel=modal.locator('label[data-rona-access-label="Электронная почта"] > .rona-access-field-label');
  assert(await loginLabel.count()===1&&await loginLabel.isVisible(),'access modal: login visual label missing');assert(await emailLabel.count()===1&&await emailLabel.isVisible(),'access modal: email visual label missing');assert(compactText(await loginLabel.innerText())==='Единый логин','access modal: wrong login visual label');assert(compactText(await emailLabel.innerText())==='Электронная почта','access modal: wrong email visual label');
