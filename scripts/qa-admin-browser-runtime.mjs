...
try{
  ...
  await clickSection(page,'access');await page.locator('#page-access #rona-ca4').waitFor({state:'visible',timeout:10000});const create=page.locator('#page-access #rona-ca4 button[data-rona-create-access="primary"]');await create.waitFor({state:'visible',timeout:10000});await create.click();const modal=page.locator('.rona-access-full');await modal.waitFor({state:'visible',timeout:5000});const accessText=compactText(await modal.innerText());
  for(const marker of ['Тип доступа','Ф.И.О.','Единый логин','Электронная почта','Телефон','Роль привязки','Компании и договоры клиента','Открыть без контракта','Загрузить договор'])assert(accessText.includes(marker),`access modal: ${marker} missing`);
  assert(await modal.getByLabel('Единый логин',{exact:true}).count()===1,'access modal: separate login input missing');assert(await modal.getByLabel('Электронная почта',{exact:true}).count()===1,'access modal: separate email input missing');assert(await page.locator('.ca-modal').count()===0,'access modal: simplified competing modal opened');const role=modal.locator('select').first();await role.selectOption({label:'Агент'});await page.waitForTimeout(100);assert(compactText(await modal.innerText()).includes('Профиль агента'),'access modal: Профиль агента missing');await modal.locator('button:has-text("Отмена")').click();await stableSelection(page,'access',1200);
