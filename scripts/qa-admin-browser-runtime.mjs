...
try{
  ...
  for(const marker of ['Тип доступа','Ф.И.О.','Телефон','Роль привязки','Компании и договоры клиента','Открыть без контракта','Загрузить договор'])assert(accessText.includes(marker),`access modal: ${marker} missing`);
  assert(await modal.locator('input[placeholder="Единый логин"]').count()===1,'access modal: separate login input missing');assert(await modal.locator('input[type="email"]').count()===1,'access modal: separate email input missing');assert(await page.locator('.ca-modal').count()===0,'access modal: simplified competing modal opened');const role=modal.locator('select').first();await role.selectOption({label:'Агент'});await page.waitForTimeout(100);assert(compactText(await modal.innerText()).includes('Профиль агента'),'access modal: Профиль агента missing');await modal.locator('button:has-text("Отмена")').click();await stableSelection(page,'access',1200);
