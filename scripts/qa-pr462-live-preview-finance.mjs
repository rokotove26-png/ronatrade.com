...
try{
  ...
  const online=page.locator('#rona-owner-build-indicator');await online.waitFor({state:'visible',timeout:30000});await waitForPageState(page,()=>{const n=document.querySelector('#rona-owner-build-indicator');return n?.dataset?.state==='ok'&&/Данные:\s*загружены/i.test(String(n?.textContent||''))},'ADMIN_BOTTOM_INDICATOR_NOT_ONLINE');const onlineBox=await online.boundingBox();assert(onlineBox&&onlineBox.y>500,'ADMIN_BOTTOM_INDICATOR_NOT_LOWER_SCREEN');console.log('ADMIN_BOTTOM_INDICATOR_ONLINE=PASS');
