...
assert(SECRET,'ADMIN_BROKER_SECRET_MISSING_IN_GITHUB');
assert(/^\d+$/.test(RUN_ID),'GITHUB_RUN_ID_REQUIRED');
await mkdir(artifacts,{recursive:true});

const brokerHeaders={authorization:`Bearer ${SECRET}`,'content-type':'application/json','x-github-repository':REPO,'x-github-sha':HEAD,'x-github-ref':'refs/heads/release/public-go-live-v1.1','x-github-run-id':RUN_ID};
async function broker(path=''){
  const r=await fetch(BROKER+path,{method:'POST',headers:brokerHeaders,body:'{}'}),j=await r.json().catch(()=>null);
  if(!r.ok||j?.ok!==true)throw new Error(`ADMIN_BROKER${path||'/'}_HTTP_${r.status}_${String(j?.code||'INVALID_RESPONSE')}`);
  return j;
}
...
try{
  ...
  const passport004=await apiProof(page,'/portal/payment-passport-current?dealId=DEAL-2026-004','LIVE_PAYMENT_PASSPORT_004');console.log('LIVE_PAYMENT_PASSPORT_AVAILABLE=PASS');
  ...
  await writeFile(`${artifacts}/network-proof.json`,JSON.stringify({origin:ORIGIN,head:HEAD,network,runtimeErrors},null,2));console.log('LIVE_PREVIEW_FINANCE_DATA=PASS');
} finally {
