...
try{
  ...
  await page.waitForFunction(()=>window.__RONA_ACCESS_FULL_SEMANTICS_READY__===true,{timeout:5000});
  const accessText=compact(await modal.innerText());
  for(const marker of ['Тип доступа','Ф.И.О.','Телефон','Роль привязки','Компании и договоры клиента','Открыть без контракта','Загрузить договор'])assert(accessText.includes(marker),`access: missing visible ${marker}`);
  const controls=modal.locator('.rona-access-full-grid input,.rona-access-full-grid select');
  assert(await controls.count()>=6,'access: incomplete primary field set');
  const login=controls.nth(2),email=controls.nth(3);
  assert(await login.evaluate(n=>n.tagName==='INPUT'),'access: login is not a separate input');
  assert(await email.evaluate(n=>n.tagName==='INPUT'),'access: email is not a separate input');
  assert(await login.getAttribute('aria-label')==='Единый логин','access: login semantic label missing');
  assert(await email.getAttribute('aria-label')==='Электронная почта','access: email semantic label missing');
  const loginLabel=login.locator('xpath=ancestor::label[1]/*[contains(@class,"rona-access-field-label")]');
  const emailLabel=email.locator('xpath=ancestor::label[1]/*[contains(@class,"rona-access-field-label")]');
  assert(await loginLabel.count()===1&&await loginLabel.isVisible(),'access: login visual label missing');
  assert(await emailLabel.count()===1&&await emailLabel.isVisible(),'access: email visual label missing');
  assert(compact(await loginLabel.innerText())==='Единый логин','access: wrong login visual label');
  assert(compact(await emailLabel.innerText())==='Электронная почта','access: wrong email visual label');
