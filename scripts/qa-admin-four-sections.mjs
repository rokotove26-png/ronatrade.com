...
try{
  ...
  const login=modal.locator('[data-rona-access-field="login"]');
  const email=modal.locator('[data-rona-access-field="email"]');
  assert(await login.count()===1,'access: separate login field missing');
  assert(await email.count()===1,'access: separate email field missing');
  ...
  const loginLabel=modal.locator('label[data-rona-access-label="Единый логин"] > .rona-access-field-label');
  const emailLabel=modal.locator('label[data-rona-access-label="Электронная почта"] > .rona-access-field-label');
