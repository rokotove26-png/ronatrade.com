...
const homeNeedle=`<script id="${homeId}"`;
const homeAt=html.indexOf(homeNeedle);
if(homeAt<0)throw new Error('CLIENT_CONTEXT_AUTHORITY_HOME_BRIDGE_MISSING');
const bridge=`<script id="${id}" src="${src}" defer></script>`;
html=html.slice(0,homeAt)+bridge+html.slice(homeAt);
