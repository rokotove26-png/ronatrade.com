...
async function measure(browser, token, width, height) {
  ...
  const ratio = proof.page.width ? proof.host.width / proof.page.width : 0;
  if (width > 760) assert(ratio >= 0.58 && ratio <= 0.62, `DESKTOP_RATIO_${ratio}`);
  else assert(ratio >= 0.98 && ratio <= 1.01, `MOBILE_RATIO_${ratio}`);
  await context.close();
  return { ...proof, ratio };
}
...
try {
  ...
  const proofs = [];
  proofs.push(await measure(browser, issued.session.access_token, 1920, 1080));
  proofs.push(await measure(browser, issued.session.access_token, 1440, 1000));
  proofs.push(await measure(browser, issued.session.access_token, 600, 1100));
