...
for(const required of [marker,legacyMarker,'AUTHORITATIVE_AUTHORIZED_CONTEXT_DIRECTORY_DB','CURRENT_EFFECTIVE_CONTRACTUAL_ONLY','validateCompleteDirectory','planDirectory','getCompanyDirectory','refreshCompanyDirectory','data-rona-company-contract-download','/v1/client/storage/','signed-url','CLIENT_COMPANY_DIRECTORY_INCOMPLETE'])if(!runtime.includes(required))throw new Error(`PR431_COMPANY_DIRECTORY_RUNTIME_CONTRACT_MISSING: ${required}`);
...
for(const forbidden of ['window.__RONA_CLIENT_CONTRACT_DOWNLOAD_V3__=LEGACY_CONTRACT_MARK','window.__RONA_CLIENT_CONTRACT_DOWNLOAD_V2__=LEGACY_CONTRACT_MARK','window.__RONA_CLIENT_CONTRACT_DOWNLOAD_V1__=LEGACY_CONTRACT_MARK'])if(runtime.includes(forbidden))throw new Error(`PR431_COMPANY_DIRECTORY_PREQUARANTINE_FORBIDDEN: ${forbidden}`);
...
const integrity=JSON.parse(await readFile(integrityPath,'utf8'));integrity.client_runtime=integrity.client_runtime||{};integrity.client_runtime.emitted_sha256=sha256(Buffer.from(html,'utf8'));integrity.client_runtime.emitted_bytes=Buffer.byteLength(html);integrity.client_runtime.pr431_company_directory={id,src,marker,scope:'ALL_SERVER_AUTHORIZED_CLIENT_CONTRACT_CONTEXTS',source:'AUTHORITATIVE_AUTHORIZED_CONTEXT_DIRECTORY_DB',documents_predicate:'CURRENT_EFFECTIVE_CONTRACTUAL_ONLY',activation:'COMPLETE_DIRECTORY_ATOMIC_COMMIT',missing_or_partial_projection:'NON_DESTRUCTIVE_KEEP_STABLE_OWNER',refresh_failure:'KEEP_LAST_VALIDATED_DIRECTORY',contract_download:'SERVER_AUTHORIZED_SHORT_LIVED_SIGNED_URL',selected_context_required_for_directory:false,legacy_selected_context_owner:'PRESERVED_UNTIL_ATOMIC_DIRECTORY_COMMIT',visual_runtime_owner:false,visual_redesign:false,hardcoded_business_entities:false};await writeFile(integrityPath,JSON.stringify(integrity,null,2)+'\n','utf8');
console.log(`PR431_COMPANY_DIRECTORY_ATTACH=PASS marker=${marker} order=authority->directory-atomic->legacy-preserved source=AUTHORITATIVE_AUTHORIZED_CONTEXT_DIRECTORY_DB visual-owner=false`);
