...
for(const token of [
  ...
  'whenReady','getCurrentContext','getCurrentProjection','getAuthorizedContexts','selectionRequired','subscribe','scopedBootstrapResponse','requires_context_selection','selected_context','PROJECTION_TTL_MS=15000','refreshCurrentProjection','x-rona-client-force-refresh','refreshProjectionOnLifecycleEvent',"refreshProjectionOnLifecycleEvent('pageshow',false)","refreshProjectionOnLifecycleEvent('visible',false)","refreshProjectionOnLifecycleEvent('online',true)",
...
console.log('CLIENT_CONTEXT_SELECTION_AUTHORITY_QA=PASS source=SERVER_SESSION_AUTHORITY public_api=RONA_CLIENT_CONTEXT current_projection_ttl=15s refresh_mode=event-driven pageshow_visibility=fresh-cache-aware online=force-network periodic_timer=absent force_refresh=enabled visual_context=DIRECT_SELECTED_CONTEXT_SLOTS company_grid=INTERNAL_STATE_CONTEXTS_EXACT_PAIR global_text_replacement=NONE header=GENERIC contract-download=REMOVED bootstrap=SELECTED_ONLY_OR_EMPTY sole-context-auto-select=guarded observer=CHILD_LIST_ONLY order=HEAD_DEFER_BEFORE_CONSUMERS');
