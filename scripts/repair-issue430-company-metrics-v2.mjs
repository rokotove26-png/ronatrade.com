...
const loadRepair="function adoptBaseDirectory(){const rows=state.base?.getCompanyDirectory?.();const validated=validateCompleteDirectory({company_directory_source:DIRECTORY_SOURCE,company_directory:Array.isArray(rows)?rows:[]});if(!validated)return false;state.directory=validated.rows.map(clone);state.validated=true;state.lastLoad=Date.now();state.generation+=1;if(!renderDirectory(state.generation))scheduleRender(0);return true}async function loadDirectory(force=false){if(!force&&adoptBaseDirectory())return state.directory;if(state.loading)return state.loading;if(!force&&state.validated&&Date.now()-state.lastLoad<REFRESH_MS)return state.directory;";
