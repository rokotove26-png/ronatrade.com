...
integrity.client_runtime.section_first_paint={
  ...
  legacy_visual_flash_allowed:false,runtime_dom_visibility_preserved:true,hardcoded_business_entities:false,business_logic_changed:false,functional_empty_state_changed:true
};
integrity.client_runtime.emitted_sha256=sha256(emitted);
integrity.client_runtime.emitted_bytes=emitted.length;
await writeFile(htmlPath,html,'utf8');
await writeFile(integrityPath,JSON.stringify(integrity));
console.log(`CLIENT_SECTION_FIRST_PAINT=PASS sha256=${integrity.client_runtime.emitted_sha256} bytes=${emitted.length}; deals=current-only authoritative-empty; payments=current-only; shielding=opaque-overlay; runtime_dom=measurable; legacy_flash=blocked`);
