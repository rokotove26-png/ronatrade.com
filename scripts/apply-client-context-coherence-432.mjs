...
const consumerPath='dist/assets/portal-runtime/client-application-lifecycle-v1.js';
const COHERENCE_MARK='RONA_CLIENT_CONTEXT_COHERENCE_432_V1';
const CONSUMER_MARK='ISSUE432_CONTEXT_SWITCH_RELOAD_QUEUE_V1';
...
for(const token of [
  ...
  "refreshingDirectory:null",
  "directory:{snapshot:null,error:null,generation:0}",
  "AUTHORITATIVE_AUTHORIZED_CONTEXT_DIRECTORY_DB",
  "CURRENT_EFFECTIVE_CONTRACTUAL_ONLY",
  "captureCompanyDirectory(body?.data,rawContexts,'wrapped-bootstrap-capture')",
  "getCompanyDirectory:()=>state.directory.snapshot",
  "whenCompanyDirectory",
  "refreshCompanyDirectory",
  "getCompanyDirectorySnapshot:()=>cloneDirectorySnapshot(state.directory.snapshot)"
...
if(/RONA-C005|ГазОнэ|GazOne|RONA-C005-IN-2026-001/iu.test(consumer))throw new Error('ISSUE432_CONSUMER_BUSINESS_SPECIFIC_HARDCODE_FORBIDDEN');
new Function(consumer);
await writeFile(consumerPath,consumer,'utf8');

console.log(`CLIENT_CONTEXT_COHERENCE_432=PASS marker=${COHERENCE_MARK} ttl_ms=${PROJECTION_MAX_AGE_MS} no_store=backend-revalidate mutation_invalidation=central generation_guard=true context_isolation=true single_flight=true consumer_context_switch_queue=${CONSUMER_MARK} visual_delta=none`);
