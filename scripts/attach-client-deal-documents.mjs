...
for(const required of ['Статус реализации','Оформление сделки','Подписание документов','Подтверждение ресурса','Оплата','Отгрузка и поставка','Закрывающие документы и завершение','Выполнено','В работе','Предстоит','Требует решения','RONA_CLIENT_CONTEXT','function currentContext()','authority.subscribe','/v1/client/deal-documents/state?clientId=','SERVER_AUTHORITATIVE_REALIZATION_V1',"const STAGE_ORDER=['contract','documents','resource','payment','logistics','close']",'function ensureFlow(root)',"ronaRealizationOwner='server-authoritative-v6-strict-context'",'data-rona-authoritative-deal-id','data-rona-authoritative-context'])
  if(!lifecycleRuntime.includes(required))throw new Error(`CLIENT_DEAL_REALIZATION_STATUS_GENERIC_UI_MISSING: ${required}`);
if(lifecycleRuntime.includes('/v1/client/bootstrap')||lifecycleRuntime.includes('ctx.map(')||lifecycleRuntime.includes('REFRESH_MS=7000')||lifecycleRuntime.includes('setInterval(()=>refresh'))throw new Error('CLIENT_DEAL_REALIZATION_PARALLEL_OR_PERIODIC_SOURCE_FORBIDDEN');
...
console.log(`CLIENT_DEAL_DOCUMENTS_BRIDGE=PASS sha256=${integrity.client_runtime.emitted_sha256} bytes=${emitted.length}; scope=CURRENT_AUTHORIZED_CLIENT_CONTEXT; context=RONA_CLIENT_CONTEXT_AUTHORITY; passport=PRESENTATION_ONLY_V1; realization_status=SERVER_AUTHORITATIVE_SINGLE_OWNER_V6_STRICT_CONTEXT; native_close=preserved; retired_local_realization_renderer=removed; single owner=${docsId}`);
