...
const lifecycleSrc='/assets/portal-runtime/client-deal-lifecycle-v1.js?v=20260922-canonical-deal-state-v7';
...
const lifecycleMarker='20260922-client-deal-realization-status-v7-canonical-deal-state';
...
for(const required of ['Статус реализации','Оформление сделки','Подписание документов','Подтверждение ресурса','Оплата','Отгрузка и поставка','Закрывающие документы и завершение','Выполнено','В работе','Предстоит','Требует решения','RONA_CLIENT_CONTEXT','function currentContext()','authority.subscribe','CLIENT_DEAL_STATE_V1','RONA_CLIENT_DEAL_STATE_V1',"const STAGE_ORDER=['contract','documents','resource','payment','logistics','close']",'function ensureFlow(root)',"ronaRealizationOwner='client-deal-state-v1'",'function acceptCanonicalDetail(detail)','deal_state','data-rona-authoritative-deal-id','data-rona-authoritative-context'])
  if(!lifecycleRuntime.includes(required))throw new Error(`CLIENT_DEAL_REALIZATION_STATUS_GENERIC_UI_MISSING: ${required}`);
for(const forbidden of ['/v1/client/deal-documents/state','/v1/client/bootstrap','async function getJson','fetch(','ctx.map(','REFRESH_MS=7000','setInterval(()=>refresh','setTimeout(()=>refresh'])if(lifecycleRuntime.includes(forbidden))throw new Error(`CLIENT_DEAL_REALIZATION_PARALLEL_OR_PERIODIC_SOURCE_FORBIDDEN: ${forbidden}`);
for(const forbiddenInference of ['function evidence(','function lifecycle(','cardTextOutside','fieldValue(root','resourceDone=','paymentPct=pctMatch','Статусы формируются из текущей карточки сделки','[data-rona-command-heading]'])
...
integrity.client_runtime.deal_documents_bridge={
  ...
  passport_id:passportId,passport_src:passportSrc,passport_marker:passportMarker,passport_scope:'ALL_AUTHORIZED_CLIENT_DEAL_DRAWERS',passport_data_policy:'PRESENTATION_ONLY_FROM_RONA_CLIENT_DEAL_STATE_V1',passport_layout:'NATIVE_RIGHT_DRAWER_PRESERVED',passport_close_behavior:'NATIVE_DRAWER_CONTROL_UNTOUCHED',passport_detector:'SEMANTIC_LABEL_COVERAGE_NATIVE_DRAWER_VISIBILITY',
  lifecycle_id:lifecycleId,lifecycle_src:lifecycleSrc,lifecycle_marker:lifecycleMarker,lifecycle_scope:'CURRENT_AUTHORIZED_CLIENT_CONTEXT',lifecycle_data_policy:'RONA_CLIENT_DEAL_STATE_V1_ONLY',lifecycle_source:'PASSPORT_CANONICAL_DETAIL_EVENT',lifecycle_status_model:['DONE','CURRENT','PENDING','BLOCKED'],lifecycle_visual:'CONNECTED_VERTICAL_TIMELINE_WITH_PROGRESS',lifecycle_refresh:'PASSPORT_OPEN_EVENT_ONLY',lifecycle_fail_closed:true,lifecycle_single_owner:true,lifecycle_host_owner:'CLIENT_DEAL_STATE_V1',retired_local_realization_renderer:'PHYSICALLY_REMOVED',
...
console.log(`CLIENT_DEAL_DOCUMENTS_BRIDGE=PASS sha256=${integrity.client_runtime.emitted_sha256} bytes=${emitted.length}; scope=CURRENT_AUTHORIZED_CLIENT_CONTEXT; context=RONA_CLIENT_CONTEXT_AUTHORITY; passport=RONA_CLIENT_DEAL_STATE_V1; realization_status=CANONICAL_DEAL_STATE_EVENT_ONLY; native_close=preserved; retired_local_realization_renderer=removed; single owner=${docsId}`);
