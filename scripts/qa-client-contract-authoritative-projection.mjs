...
for(const marker of ['20260902-client-contract-v4-current-context-authority','20260829-client-contract-v3-authoritative-projection-v5','RONA_CLIENT_CONTEXT','authority.subscribe','/v1/client/context?clientId=','current_external_contract_number','legal_name','function hydrateFrozenClientModel',"typeof CLIENT_CONTEXTS!=='undefined'",'function currentContractDocument',"type(d)==='SIGNED_CONTRACT'&&materialized(d)",'new MutationObserver',"document.addEventListener('click',()=>scheduleRender(140),true)",'Скачать договор PDF','/v1/client/storage/','/signed-url','storage_object_id','Файл подписанного контракта не опубликован в кабинете','CLIENT_CONTRACT_REFRESH_POLICY','OPEN_CONTEXT_PROJECTION_PAGESHOW_INVALIDATION','rona:client-current-projection','rona:client-contract-download-invalidated','primeCompanyDirectoryLater'])assert(runtime.includes(marker),`runtime missing ${marker}`);
for(const forbidden of ['/v1/client/bootstrap','function selectedContextEntry','function hookContextSetter','01/РТ-01-1926','01/РТ-02-1926','01/PT-02-1926','setInterval(','refreshCompanyDirectoryNow'])assert(!runtime.includes(forbidden),`runtime contains retired/polling owner ${forbidden}`);
...
console.log(`CLIENT_CONTRACT_AUTHORITATIVE_PROJECTION=PASS context=RONA_CLIENT_CONTEXT scope=CURRENT_CONTEXT_ONLY refresh=OPEN_CONTEXT_PROJECTION_PAGESHOW_INVALIDATION periodic_polling=absent generic_click_network_refresh=absent issue432_overlay=${issue432Overlay} pr431_overlay=${pr431Overlay} composition=EXACT_ONLY`);
