...
const src='/assets/portal-runtime/client-background-section-preload-v1.js?v=20260902-current-context-v5';
const marker='20260902-client-background-section-preload-current-context-v5';
const sha256=b=>createHash('sha256').update(b).digest('hex');

const runtime=await readFile(runtimePath,'utf8');
if(!runtime.includes(marker))throw new Error(`CLIENT_BACKGROUND_PRELOAD_MARKER_MISSING: ${marker}`);
for(const required of [
  'RONA_CLIENT_CONTEXT','getCurrentContext','selectionRequired','authority.subscribe',
...
for(const forbidden of [
  '/v1/client/bootstrap','getAuthorizedContexts','state.contexts.map(preloadContext)','/v1/client/market-intelligence','MARKET_INTELLIGENCE_REFRESH_MS','readMarketIntelligence',"markSection('analytics'","markSection('market_news'",'method:\'POST\'','method:"POST"','/v1/client/applications\'','/v1/events\''
...
console.log('CLIENT_BACKGROUND_SECTION_PRELOAD=PASS selected CURRENT_CONTEXT only; no authorized-context catalog consumption');
