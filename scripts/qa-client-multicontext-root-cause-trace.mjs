...
try{
  ...
  await page.evaluate(fixtures=>{
    ...
    const passport=()=>{const r=[...document.querySelectorAll('.rona-deal-command-center-v3,[data-rona-deal-passport]')].find(visible);if(!r)return{visible:false};const flow=r.querySelector('#rona-deal-realization-flow-v3');return{visible:true,binding:r.dataset.ronaAuthoritativeBinding||'',deal_id:r.dataset.ronaAuthoritativeDealId||'',client_id:r.dataset.ronaAuthoritativeClientId||'',contract_id:r.dataset.ronaAuthoritativeContractId||'',context:r.dataset.ronaAuthoritativeContext||'',lifecycle_owner:flow?.dataset?.ronaRealizationOwner||'',lifecycle_text:norm(flow?.innerText||''),text:norm(r.innerText)}};
  ...
  async function activateProductionControl(kind){return page.evaluate(kind=>{
    ...
    let target=specs.ids.map(id=>document.getElementById(id)).find(visible)||all.find(n=>visible(n)&&specs.tokens.some(t=>[norm(n.textContent),norm(n.getAttribute('aria-label')),norm(n.getAttribute('title'))].some(x=>x===t||x.includes(t))));
  ...
  async function waitPrices(f,timeout=4500){const start=performance.now(),requestStart=requests.length,action=await activateProductionControl('prices');if(!action.ok)return{ready:false,representative:false,ms:Math.round(performance.now()-start),action,request:null};const ready=await page.waitForFunction(({c,k})=>{const s=window.__RONA_CLIENT_PRICE_SYNC_STATE__;return s?.authority==='SERVER_AUTHORITATIVE_PRICE_PROJECTION'&&s?.context?.client_id===c&&s?.context?.contract_id===k&&Array.isArray(s?.prices)&&s.prices.length>0&&s.prices.every(p=>(!p.client_id||p.client_id===c)&&(!p.contract_id||p.contract_id===k))},{c:f.client_id,k:f.contract_id},{timeout}).then(()=>true).catch(()=>false);const request=requests.slice(requestStart).find(r=>r.path==='/portal/api/v1/client/prices'&&r.client_id===f.client_id&&r.contract_id===f.contract_id&&r.source==='client-price-sync-v1:prices')||null;return{ready:ready&&!!request,representative:true,ms:Math.round(performance.now()-start),action,request}}
  ...
  async function openRealPassport(f,timeout=5000){
    ...
    const target=await page.waitForFunction(d=>{const n=[...document.querySelectorAll('[data-open-deal]')].find(x=>x.getAttribute('data-open-deal')===d);if(!n)return null;const s=getComputedStyle(n),r=n.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>0&&r.height>0?{tag:n.tagName,text:String(n.textContent||'').trim(),class_name:String(n.className||''),deal:n.getAttribute('data-open-deal')}:null},f.deal_id,{timeout:2500}).then(h=>h?.jsonValue?.()||null).catch(()=>null);
    if(!target)return{visible:false,bound:false,lifecycle:false,representative:false,ms:Math.round(performance.now()-start),deals_action:dealsAction,target:null,drawer:null};
    await page.locator(`[data-open-deal="${f.deal_id}"]`).filter({visible:true}).first().click({timeout:2500}).catch(()=>{});
    ...
    const drawer=await page.evaluate(({c,k,d})=>{const visible=n=>{if(!n||!n.isConnected||n.hidden)return false;const s=getComputedStyle(n),r=n.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>0&&r.height>0};const r=[...document.querySelectorAll('.rona-deal-command-center-v3,[data-rona-deal-passport]')].find(visible);if(!r)return null;const flow=r.querySelector('#rona-deal-realization-flow-v3');return{binding:r.dataset.ronaAuthoritativeBinding||'',deal_id:r.dataset.ronaAuthoritativeDealId||'',client_id:r.dataset.ronaAuthoritativeClientId||'',contract_id:r.dataset.ronaAuthoritativeContractId||'',context:r.dataset.ronaAuthoritativeContext||'',passport_marker:r.dataset.ronaDealPassport||'',lifecycle_owner:flow?.dataset?.ronaRealizationOwner||'',has_synthetic_marker:!!r.closest('[data-rona-diagnostic-native-passport]'),text:String(r.innerText||'').replace(/\s+/g,' ').trim().slice(0,600),expected:c+'|'+k+'|'+d}},{c:f.client_id,k:f.contract_id,d:f.deal_id});
    return{visible:!!drawer,bound:bound&&drawer?.binding==='authoritative-binding'&&drawer?.deal_id===f.deal_id&&drawer?.client_id===f.client_id&&drawer?.contract_id===f.contract_id,lifecycle,representative:!!target&&!drawer?.has_synthetic_marker,ms:Math.round(performance.now()-start),deals_action:dealsAction,target,drawer};
  }
  async function latePriorProbe(name){
    ...
    await page.evaluate(prior=>window.dispatchEvent(new CustomEvent('rona:client-current-projection',{detail:{client_id:prior.client_id,contract_id:prior.contract_id,source:'diagnostic-stale-prior-completion'}})),prior);
    await page.waitForTimeout(180);const after=await snapshot(`${name}:late-prior-after`);const changes=[];
    if(!exactPair(after.selected,cur))changes.push({consumer:'selected',value:after.selected});
    if(!exactPair(after.projection,cur))changes.push({consumer:'authoritative_projection',value:after.projection});
    if(after.home_status!=='ready'||!exactPair(after.home,cur))changes.push({consumer:'home',value:{status:after.home_status,state:after.home}});
    if(!exactPair(after.prices?.context,cur)||!Array.isArray(after.prices?.prices)||!after.prices.prices.length)changes.push({consumer:'prices',value:after.prices});
    if(after.deals_live!=='ready'||!after.projection?.deals?.some(d=>d.deal_id===cur.deal_id&&d.client_id===cur.client_id&&d.contract_id===cur.contract_id))changes.push({consumer:'deals',value:{live:after.deals_live,deals:after.projection?.deals}});
    if(!after.passport?.visible||after.passport?.binding!=='authoritative-binding'||after.passport?.deal_id!==cur.deal_id||after.passport?.client_id!==cur.client_id||after.passport?.contract_id!==cur.contract_id)changes.push({consumer:'passport',value:after.passport});
    const bad=foreign(after,cur);if(bad)changes.push({consumer:'business_dom',value:bad});
    return{requested_prior:{client_id:prior.client_id,contract_id:prior.contract_id,deal_id:prior.deal_id},before:{selected:before.selected,projection:before.projection,home:before.home,prices:before.prices,passport:before.passport,visible_business:before.visible_business},after:{selected:after.selected,projection:after.projection,home:after.home,prices:after.prices,passport:after.passport,visible_business:after.visible_business},committed_changes:changes};
  }
  ...
  async function eventProbe(type){const barrier=await waitRequestQuiescence();const start=requests.length;await page.evaluate(type=>{if(type==='focus')window.dispatchEvent(new Event('focus'));else if(type==='pageshow')window.dispatchEvent(new PageTransitionEvent('pageshow'));else document.dispatchEvent(new Event('visibilitychange'))},type);await sleep(250);await waitRequestQuiescence(450,2500);return{type,barrier,requests:requests.slice(start).map(r=>({...r}))}}
  ...
  await waitRequestQuiescence();const idleBefore={requests:requests.length,dom:await page.locator('*').count()};await page.waitForTimeout(5000);const idleAfter={requests:requests.length,dom:await page.locator('*').count()};const idle={request_delta:idleAfter.requests-idleBefore.requests,dom_delta:idleAfter.dom-idleBefore.dom};await snapshot('A:idle-stable');
  ...
  for(const cycle of cycles){
    ...
    if(!cycle.prices.representative)failures.push({code:'PRICES_REAL_CONSUMER_UNAVAILABLE',cycle:cycle.name,action:cycle.prices.action});
  ...
  const realContext=requests.filter(r=>r.path==='/portal/api/v1/client/context');for(const r of realContext){if(!r.source.startsWith('client-context-selection-authority-v1:'))failures.push({code:'UNAPPROVED_CONTEXT_OWNER',request:r});if(r.client_id!==r.response_client_id||r.contract_id!==r.response_contract_id)failures.push({code:'CONTEXT_RESPONSE_SCOPE_MISMATCH',request:r})}
  ...
  const eventFailures=[];for(const probe of event_probes){const business=probe.requests.filter(r=>r.path.startsWith('/portal/api/v1/client/'));const approved=business.filter(r=>r.path==='/portal/api/v1/client/deal-documents/state'&&r.source==='client-deal-lifecycle-v1'&&r.client_id===FIXTURES.A.client_id&&r.contract_id===FIXTURES.A.contract_id);const disallowed=business.filter(r=>!approved.includes(r));const byPath=new Map();for(const r of business){const key=`${r.path}|${r.source}`;byPath.set(key,(byPath.get(key)||0)+1)}const duplicate=[...byPath].filter(([,n])=>n>1);if(disallowed.length||duplicate.length)eventFailures.push({event:probe.type,requests:probe.requests,disallowed,duplicate})}if(eventFailures.length)failures.push({code:'FOCUS_PAGESHOW_VISIBILITY_FANOUT',events:eventFailures});
  ...
  const priceProof=cycles.map(c=>({cycle:c.name,action:c.prices.action,request:c.prices.request,state:{context:c.prices_snapshot.prices?.context,authority:c.prices_snapshot.prices?.authority,count:c.prices_snapshot.prices?.prices?.length||0}}));
  const passportProof=cycles.map(c=>({cycle:c.name,target:c.passport.target,drawer:c.passport.drawer,bound:c.passport.bound,lifecycle:c.passport.lifecycle}));
  console.log(TARGET_LABEL==='PRE_FIX'?'FIRST_DIVERGENCE_TRACE_PRE_FIX':'FIRST_DIVERGENCE_TRACE',JSON.stringify(first));
  console.log('A_B_A_TRACE',JSON.stringify(cycles.map(c=>({name:c.name,selected:c.sync.selected,legacy:c.sync.legacy,projection_ready:c.projection_ready,home_ready:c.home_ready,prices_ready:c.prices.ready,deals_ready:c.deals.ready,passport:{visible:c.passport.visible,bound:c.passport.bound,lifecycle:c.passport.lifecycle,representative:c.passport.representative},late_prior_committed_changes:c.late_prior.committed_changes}))));
