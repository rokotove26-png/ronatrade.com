...
try{
  ...
  const state=await page.evaluate(()=>{
    const d=document.documentElement,home=document.getElementById('page-home')||document.getElementById('homePage')||document.querySelector('[data-page-panel="home"],[data-page-id="home"]');
    ...
    return{
      contextReady:d.dataset.ronaClientContextReady||'',
      selection:d.dataset.ronaClientContextSelection||'',
      current:window.RONA_CLIENT_CONTEXT?.getCurrentContext?.()||null,
      authority:d.dataset.ronaClientContextAuthority||'',
      homeState:d.dataset.ronaClientHomeState||'',
      homeReady:d.dataset.ronaClientHomeReady||'',
      homePrepaint:d.dataset.ronaClientHomePrepaint||'',
      homePrepaintRelease:d.dataset.ronaClientHomePrepaintRelease||'',
      homeDegraded:d.dataset.ronaClientHomeDegraded||'',
      bodyText:String(document.body?.innerText||'').replace(/\s+/g,' ').trim().slice(0,2500),
      bodyVisible:visible(document.body),
      homeVisible:visible(home),
      ownerVisible:visible(owner),
      ownerText:String(owner?.innerText||'').replace(/\s+/g,' ').trim().slice(0,1500),
      guardMedia:guard?.media||'',
      homeRuntime:window.__RONA_CLIENT_HOME_RUNTIME__||null,
      homeCurrentOnly:window.__RONA_CLIENT_HOME_CURRENT_ONLY__||null,
      currentProjection:window.RONA_CLIENT_CONTEXT?.getCurrentProjection?.()||null
    };
  });
  ...
  console.log('CLIENT_IMPERSONATION_VISUAL_DIAG='+JSON.stringify({state,pageErrors,requests}));
  if(!state.bodyVisible)throw new Error('CLIENT_BODY_NOT_VISIBLE '+JSON.stringify(state));
  if(!state.homeVisible)throw new Error('CLIENT_HOME_NOT_VISIBLE '+JSON.stringify(state));
  if(!state.ownerVisible)throw new Error('CLIENT_HOME_OWNER_NOT_VISIBLE '+JSON.stringify(state));
  if(!state.bodyText)throw new Error('CLIENT_BODY_TEXT_EMPTY '+JSON.stringify(state));
