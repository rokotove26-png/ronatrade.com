...
function paymentsV7OwnerQueue(queue,deals){if(!queue.length)return null;const owner=e('section',{class:'rona-payments-v7-owner'},e('h3',{text:'Требуется решение Owner'}));for(const item of queue){const candidates=new Set(paymentsV7Array(item?.candidate_deal_ids).map(String));const select=e('select',{'data-owner-deal':'',ariaLabel:'Сделка'});select.append(e('option',{value:'',text:'Выберите сделку'}));for(const deal of deals){const key=paymentsV7Text(deal?.deal_key);if(!key)continue;const hint=candidates.has(key)||candidates.has(paymentsV7Text(deal?.deal_id));select.append(e('option',{value:key,text:(paymentsV7Text(deal?.deal_id)||'Deal')+' · '+(paymentsV7Text(deal?.client_display)||'—')+(hint?' · подсказка':'')}))}const bind=e('button',{type:'button','data-owner-action':'BIND_TO_DEAL',text:'Привязать к сделке'}),advance=e('button',{type:'button','data-owner-action':'ASSIGN_ADVANCE_PAYMENT',text:'Авансовый платеж'}),error=e('div',{class:'rona-payments-v7-owner-error'}),buttons=[bind,advance];bind.addEventListener('click',()=>paymentsV7OwnerAction(item,'BIND_TO_DEAL',select,buttons,error));advance.addEventListener('click',()=>paymentsV7OwnerAction(item,'ASSIGN_ADVANCE_PAYMENT',select,buttons,error));owner.append(e('article',{class:'rona-payments-v7-owner-item','data-payment-key':paymentsV7Text(item?.payment_key)},e('div',{class:'rona-payments-v7-owner-fact'},e('strong',{text:paymentsV7Array(item?.payment_ids).join(', ')||paymentsV7Text(item?.exception_id)||'Payment'}),e('span',{text:paymentsV7RawMoney(item?.payment_amount,item?.payment_currency)}),e('small',{text:paymentsV7Text(item?.counterparty_name)||paymentsV7Text(item?.reconciliation_class)||'GENUINELY_UNALLOCATED'})),e('div',{class:'rona-payments-v7-owner-actions'},select,bind,advance,error)))}return owner}
