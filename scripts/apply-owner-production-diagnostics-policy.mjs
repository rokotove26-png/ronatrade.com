...
const TARGET = 'functions/portal/owner-ui-chunks/chunk15-base.js';
...
const output = enforce(source);
await writeFile(TARGET, output, 'utf8');
const verified = await readFile(TARGET, 'utf8');
if (
  verified.includes("n.id='rona-owner-build-indicator'") ||
  verified.includes('setInterval(updateIndicator,1000)') ||
  !verified.includes(BUILD_INDICATOR_DISABLED)
) {
  throw new Error('OWNER_BUILD_INDICATOR_POLICY_POSTCHECK_FAILED');
}
