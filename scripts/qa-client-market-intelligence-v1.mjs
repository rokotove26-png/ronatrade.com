...
for(const token of [
  ...
  'id="rona-client-market-intelligence-v1"','client-market-intelligence-v1.js?v=20260921-analytics-event-driven-v5',
  'id="rona-client-market-news-admin-parity-v1"','client-market-news-admin-parity-v1.js?v=20260921-news-event-driven-v6',
...
for(const token of ["load('open')",'/v1/client/market-intelligence','RONA_CLIENT_MARKET_INTELLIGENCE_V1','public_chart',"rona:client:context-changed",'rona:client-market-intelligence-invalidated','marketNavTarget'])must(analytics,token,'analytics runtime');
for(const token of ['rona:client:background-sections','__RONA_CLIENT_BACKGROUND_CACHE__','background-cache','background-event','REFRESH_MS=3600000','REFRESH_MS=60000','setInterval('])forbid(analytics,token,'analytics runtime');

for(const token of ['/v1/client/market-intelligence','source_published_at','duplicate_group',"rona:client:context-changed",'rona:client-market-intelligence-invalidated','#page-market-news>.rona-market-news-current','.mn-masthead','.mn-front','.mn-grid','.mn-dialog',"const DIALOG_ID='rona-client-market-news-dialog'","id:DIALOG_ID,class:'mn-dialog'",'p.append(d)',"if(ev.target?.closest?.('#page-market-news'))return"])must(news,token,'news runtime');
for(const token of ['rona:client:background-sections','consumeBackground','refreshIfActive','refreshHourly','AUTO_REFRESH_MS','setInterval(','/portal/owner-api','/admin/analytics-bootstrap',"root.append(d)"])forbid(news,token,'news runtime');
...
if(mi?.trigger!=='PORTAL_OPEN'||mi?.refresh_ms!==null||mi?.refresh_policy!=='OPEN_SECTION_CONTEXT_INVALIDATION'||mi?.invalidation_event!=='rona:client-market-intelligence-invalidated')throw new Error('Market Intelligence event-driven metadata invalid');
...
console.log('CLIENT_MARKET_INTELLIGENCE_QA=PASS current-context preload isolated; one Client news renderer; Analytics/News event-driven with no periodic timer');
