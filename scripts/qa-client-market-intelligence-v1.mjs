...
for(const token of ["REFRESH_POLICY='OPEN_CONTEXT_CHANGE_INVALIDATION'",'rona:client-market-news-invalidated','/v1/client/market-intelligence','source_published_at','duplicate_group',"rona:client:context-changed",'#page-market-news>.rona-market-news-current','.mn-masthead','.mn-front','.mn-grid','.mn-dialog',"const DIALOG_ID='rona-client-market-news-dialog'","id:DIALOG_ID,class:'mn-dialog'",'p.append(d)',"if(ev.target?.closest?.('#page-market-news'))return"])must(news,token,'news runtime');
for(const token of ['rona:client:background-sections','consumeBackground','refreshIfActive','AUTO_REFRESH_MS=','setInterval(','/portal/owner-api','/admin/analytics-bootstrap',"root.append(d)"])forbid(news,token,'news runtime');
...
console.log('CLIENT_MARKET_INTELLIGENCE_QA=PASS current-context preload isolated; one Client news renderer; canonical gateway preserved; Analytics/News open+context-change+explicit-invalidation only; periodic polling absent');
