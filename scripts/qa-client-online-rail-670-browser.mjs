...
try{
  ...
  assert(view.overlay.version==='CLIENT_RAIL_ROUTE_OVERLAY_V6_COHORT_VISUAL'&&view.overlay.renderedPointCount>=2,'production route overlay state missing '+JSON.stringify(view.overlay));
  ...
  assert(view.legendRows>=2,'route cohort legend missing '+JSON.stringify(view));
  assert(view.focus.focusMode==='OPERATIONAL','operational focus is not default '+JSON.stringify(view.focus));
