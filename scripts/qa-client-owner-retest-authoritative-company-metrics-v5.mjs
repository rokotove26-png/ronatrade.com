...
if(!identityState.A.ok||!identityState.B.ok)fail(failures,'COMPANY_CARD_VISIBLE_IDENTITY_LEAK',identityState);
...
const bProjectionReady=await wait(page,b=>{const p=window.RONA_CLIENT_CONTEXT?.getCurrentProjection?.(),client=String(p?.contract?.client_id||p?.client_id||''),contract=String(p?.contract?.contract_id||p?.contract_id||'');return client===b.client_id&&contract===b.contract_id&&Array.isArray(p?.applications)&&p.applications.some(a=>String(a?.application_id||'')==='APP-B-1')},FIX.B,5000);
await nav(page,'applications');
const bApplicationVisible=await wait(page,()=>[...document.querySelectorAll('[data-rona-live-application-id]')].some(el=>el.getAttribute('data-rona-live-application-id')==='APP-B-1'&&(el.offsetWidth||el.offsetHeight)),null,5000);
...
if(!bProjectionReady||!bApplicationVisible||!bApplicationProof.projection.includes('APP-B-1')||!bApplicationProof.visible.includes('APP-B-1'))fail(failures,'B_APPLICATION_CURRENT_PROJECTION_NOT_VISIBLE',{bProjectionReady,bApplicationVisible,bApplicationProof});
