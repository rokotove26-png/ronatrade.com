...
for(const token of ['getCompanyDirectory','refreshCompanyDirectory','ALL_AUTHORIZED_CONTEXT_DIRECTORY','AUTHORITATIVE_AUTHORIZED_CONTEXT_DIRECTORY_DB','CURRENT_EFFECTIVE_CONTRACTUAL_ONLY','validateCompleteDirectory','authorizedSnapshot','CLIENT_COMPANY_DIRECTORY_INCOMPLETE','planDirectory','state.validated','state.active','/v1/client/storage/','signed-url','LEGACY_CONTRACT_MARK',"OPEN_AUTHORIZED_DIRECTORY_CONTEXT_PAGESHOW_INVALIDATION",'rona:client-company-directory-invalidated'])if(!text.directory.includes(token))throw new Error(`PR431_DIRECTORY_RUNTIME_CONTRACT_MISSING:${token}`);
for(const forbidden of ['setInterval(','REFRESH_MS=30000'])if(text.directory.includes(forbidden)||text.attach.includes(forbidden))throw new Error(`PR431_DIRECTORY_POLLING_FORBIDDEN:${forbidden}`);
