...
for(const token of ['whenCurrentProjection','refreshCurrentProjection','invalidateCurrentProjection','ghost_endpoint:false','periodic_refresh:false','pageshow_network_refresh:false','CLIENT_APPLICATIONS_RECURRING_REFRESH_FORBIDDEN','post_submit_projection_invalidation:true'])if(!text.apps.includes(token))throw new Error(`PR431_APPLICATIONS_CONTRACT_MISSING:${token}`);
for(const token of ["for(const forbidden of ['applications-projection','REFRESH_MS=30000','setInterval(()=>load(false)','periodic_refresh:true'])if(runtime.includes(forbidden))",'periodic_refresh:false','pageshow_network_refresh:false'])if(!text.apps.includes(token))throw new Error(`PR431_APPLICATIONS_EVENT_DRIVEN_GUARD_MISSING:${token}`);
if(!text.apps.includes("for(const forbidden of ['applications-projection'"))throw new Error('PR431_GHOST_ENDPOINT_BUILD_GATE_MISSING');
...
console.log('PR431_SYSTEM_ADMIN_ONE_SHOT_SOURCE_QA=PASS directory=atomic-complete-all-context applications=current-projection-event-driven preview=hostname-authoritative parent-proxy=restored signed-addendum=preserved visual-owner=false hardcoded-fixtures=false');
