...
async function assertCrossCases(page,aTotal='42000'){
  const rubRow=page.locator('#page-payments tbody tr',{hasText:'PAY-QA-RUB-003'});assert(await rubRow.count()===1,'RUB PAYMENT must render once');const rubCells=rubRow.locator('td'),rubAllocation=compact(await rubCells.nth(7).innerText()),rubUnallocated=compact(await rubCells.nth(8).innerText());assert(rubAllocation.includes('DEAL-C')&&rubAllocation.includes('400000'),'RUB deal allocation mismatch: '+rubAllocation);assert(rubUnallocated.includes('100000'),'RUB unallocated residue mismatch: '+rubUnallocated);
  const summary=page.locator('#page-payments .rona-owner-card',{hasText:'Финансовая картина по сделкам'});const dealARow=summary.locator('tbody tr',{hasText:'DEAL-A'});assert(await dealARow.count()===1,'DEAL-A finance summary must render once');const dealAText=compact(await dealARow.innerText());assert(dealAText.includes(aTotal),'multiple-payments-to-one-deal total missing: '+dealAText);assert(dealAText.includes('ClientA'),'multi-client summary missing Client A');assert((await summary.innerText()).includes('Client B')&&(await summary.innerText()).includes('Client C'),'multi-client rows missing')
}
async function waitForFinanceRefresh(page,delay=650){const before=syncCalls;await page.waitForTimeout(delay);assert(syncCalls>before,`trusted Finance refresh did not advance (${before} -> ${syncCalls})`)}
...
try{
  ...
  const page=await context.newPage();page.on('pageerror',e=>errors.push('pageerror:'+String(e.message||e)));page.on('response',r=>{if(r.status()>=400)errors.push(`http:${r.status()}:${r.url()}`)});page.on('console',m=>{if(m.type()==='error'&&!m.text().startsWith('Failed to load resource:'))errors.push('console:'+m.text())});
  ...
  await assertSplit(page);await assertCrossCases(page);

  const beforeRepeated=syncCalls;await page.waitForTimeout(900);assert(syncCalls>=beforeRepeated+2,`expected repeated trusted refreshes, got ${beforeRepeated} -> ${syncCalls}`);await assertSplit(page);assert(await page.locator('#page-payments tbody tr',{hasText:'PAY-QA-SPLIT-001'}).count()===1,'repeated refresh duplicated PAYMENT row');assert(await page.locator('#page-payments .rona-fin-kpi').count()===4,'repeated refresh duplicated KPI totals');

  const beforeReload=syncCalls;await page.reload({waitUntil:'domcontentloaded'});await page.waitForFunction(()=>window.__RONA_OWNER_ADMIN_READY__===true,{timeout:15000});await assertSplit(page);assert(syncCalls>beforeReload,'hard reload did not fetch fresh Finance state');

  financeRevision=2;await waitForFinanceRefresh(page);await assertSplit(page,'35000','45000','20000');await assertCrossCases(page,'47000');

  financeRevision=3;await waitForFinanceRefresh(page);await openPayments(page);const stale=await splitRow(page),staleCells=stale.locator('td'),staleAllocation=await staleCells.nth(7).innerText(),staleUnallocated=await staleCells.nth(8).innerText();assert(!staleAllocation.includes('DEAL-A')&&!staleAllocation.includes('DEAL-B'),'stale/missing allocation was assigned to deal');assert(staleAllocation.includes('Нераспределено'),'missing Finance allocation must fail closed');assert(staleUnallocated.includes('Требует верификации'),'conflicting/stale summary must show TO_VERIFY');

  financeRevision=1;await waitForFinanceRefresh(page);await assertSplit(page);await assertCrossCases(page);
  ...
  await page.setViewportSize({width:900,height:800});await page.locator('#page-payments .rona-fin-filter button',{hasText:'Поступило'}).click();await page.waitForTimeout(150);assert(await page.locator('#page-payments.active').isVisible(),'Payments page not responsive/visible at 900px');await assertSplit(page);
  ...
  console.log('PAYMENTS_PAGE_LOAD=PASS');console.log('INCOMING_RECEIPTS=PASS');console.log('DEAL_ALLOCATION_EXACT=PASS');console.log('SPLIT_ALLOCATION=PASS');console.log('UNALLOCATED_RESIDUE=PASS');console.log('OUTGOING_PAYMENTS_DISTINCT=PASS');console.log('BANK_FEES_RULE=PASS');console.log('MULTI_PAYMENT_ONE_DEAL=PASS');console.log('MULTI_CLIENT=PASS');console.log('MULTI_CURRENCY=PASS');console.log('FINANCE_REFRESH_PROPAGATION=PASS');console.log('HARD_REFRESH=PASS');console.log('NO_DUPLICATES=PASS');console.log('STATUS_LAYERS_DISTINCT=PASS');console.log('RESPONSIVE_RUNTIME=PASS');console.log('NO_RUNTIME_ERRORS=PASS');console.log(JSON.stringify({origin,syncCalls,errors}));
  await context.close();
