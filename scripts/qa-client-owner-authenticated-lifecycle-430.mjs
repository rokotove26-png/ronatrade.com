...
async function exactPreview(){for(let i=0;i<72;i++){const data=await githubJson(`/commits/${HEAD}/check-runs?per_page=100`),runs=Array.isArray(data?.check_runs)?data.check_runs:[],run=runs.find(x=>x?.name==='Cloudflare Pages'&&x?.head_sha===HEAD&&x?.status==='completed'&&x?.conclusion==='success'),urls=previewUrls(run?.output?.summary);if(run&&urls.length===1)return{origin:urls[0],checkRunId:run.id,commentId:null,source:'CHECK_RUN'};const comments=await githubJson(`/issues/${PR_NUMBER}/comments?per_page=100&page=1`).catch(()=>[]),short=HEAD.slice(0,7).toLowerCase(),cloudflare=(Array.isArray(comments)?comments:[]).find(x=>String(x?.user?.login||'')==='cloudflare-workers-and-pages[bot]'&&String(x?.body||'').toLowerCase().includes(short)),commentUrls=previewUrls(cloudflare?.body);if(cloudflare&&commentUrls.length===1)return{origin:commentUrls[0],checkRunId:null,commentId:cloudflare.id||null,source:'CLOUDFLARE_PR_COMMENT'};await sleep(5000)}throw new Error('EXACT_HEAD_CLOUDFLARE_PREVIEW_NOT_READY')}
...
const proof={schema:'ISSUE430_REAL_AUTHENTICATED_LIFECYCLE_PROOF_V1',exact_head:HEAD,immutable_preview:`${preview.origin}/portal/client`,cloudflare_check_run_id:preview.checkRunId,cloudflare_comment_id:preview.commentId,preview_source:preview.source,cold_reload:null,relogin:null,cleanup:{first:false,second:false}};
try{
