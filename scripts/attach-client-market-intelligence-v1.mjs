...
const analyticsSrc='/assets/portal-runtime/client-market-intelligence-v1.js?v=20260921-analytics-event-driven-v5';
const analyticsMarker='20260921-client-market-intelligence-v3-event-driven';
const newsId='rona-client-market-news-admin-parity-v1';
const newsSrc='/assets/portal-runtime/client-market-news-admin-parity-v1.js?v=20260921-news-event-driven-v6';
...
for(const required of [analyticsMarker,'/v1/client/market-intelligence','RONA_CLIENT_MARKET_INTELLIGENCE_V1','public_chart',"load('open')",'rona:client-market-intelligence-invalidated','marketNavTarget'])
...
for(const forbidden of ['rona:client:background-sections','__RONA_CLIENT_BACKGROUND_CACHE__','background-cache','background-event','setInterval(','REFRESH_MS=3600000','REFRESH_MS=60000'])if(analyticsRuntime.includes(forbidden))throw new Error(`CLIENT_ANALYTICS_EVENT_DRIVEN_ISOLATION_FAILED: ${forbidden}`);
await writeFile(analyticsRuntimePath,analyticsRuntime,'utf8');

let newsRuntime=await readFile(newsRuntimePath,'utf8');
for(const required of [newsMarker,newsVisualContract,'#page-market-news>.rona-market-news-current','.mn-masthead','.mn-front','.mn-grid','.mn-dialog','/v1/client/market-intelligence','source_published_at','duplicate_group','rona:client-market-intelligence-invalidated'])
...
newsRuntime=replaceRequired(newsRuntime,"function activate(){ensureRoot();consumeBackground();render();loadData(true)}","function activate(){ensureRoot();render();loadData(true)}",'NEWS_ACTIVATE_BACKGROUND');
newsRuntime=replaceRequired(newsRuntime,"  ensureRoot();consumeBackground();loadData(true);\n  window.addEventListener('pageshow',()=>{consumeBackground();if(isActive())loadData(true)},{passive:true});\n  window.addEventListener('rona:client-market-intelligence-invalidated',()=>loadData(true),{passive:true});\n  window.addEventListener('rona:client:background-sections',consumeBackground,{passive:true});\n","  ensureRoot();loadData(true);\n  window.addEventListener('pageshow',()=>{if(isActive())loadData(true)},{passive:true});\n  window.addEventListener('rona:client-market-intelligence-invalidated',()=>loadData(true),{passive:true});\n  document.addEventListener('rona:client:context-changed',()=>loadData(true));\n",'NEWS_START_BACKGROUND');
...
for(const forbidden of ['rona:client:background-sections','consumeBackground','refreshIfActive','refreshHourly','AUTO_REFRESH_MS','setInterval('])if(newsRuntime.includes(forbidden))throw new Error(`CLIENT_NEWS_EVENT_DRIVEN_ISOLATION_FAILED: ${forbidden}`);
if(!newsRuntime.includes('rona:client-market-intelligence-invalidated'))throw new Error('CLIENT_NEWS_INVALIDATION_EVENT_MISSING');
...
integrity.client_runtime.market_intelligence={
  id:analyticsId,src:analyticsSrc,marker:analyticsMarker,mode:'ADMIN_PRINCIPLE_CLIENT_SAFE_PROJECTION',endpoint:'/portal/api/v1/client/market-intelligence',trigger:'PORTAL_OPEN',refresh_ms:null,refresh_policy:'OPEN_SECTION_CONTEXT_INVALIDATION',invalidation_event:'rona:client-market-intelligence-invalidated',background_preload_dependency:false,background_30s_event_dependency:false,
...
console.log('CLIENT_MARKET_INTELLIGENCE_ATTACH=PASS Analytics and Market News are event-driven: portal open, section open, context change, pageshow and explicit invalidation; no periodic timer; Client News has one renderer and isolated article dialog');