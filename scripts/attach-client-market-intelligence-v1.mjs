...
analyticsRuntime=replaceRequired(analyticsRuntime,"  state.timer=setInterval(()=>load('interval'),REFRESH_MS);\n","  window.addEventListener('rona:client-market-intelligence-invalidated',()=>load('invalidation'),{passive:true});\n",'ANALYTICS_TIMER_REMOVAL');
for(const forbidden of ['rona:client:background-sections','__RONA_CLIENT_BACKGROUND_CACHE__','background-cache','background-event','REFRESH_MS=','setInterval('])if(analyticsRuntime.includes(forbidden))throw new Error(`CLIENT_ANALYTICS_EVENT_DRIVEN_FAILED: ${forbidden}`);
...
newsRuntime=replaceRequired(newsRuntime,"const AUTO_REFRESH_MS=3600000;\nlet autoRefreshTimer=null;","const REFRESH_POLICY='OPEN_CONTEXT_CHANGE_INVALIDATION';\nlet invalidationBound=false;",'NEWS_REFRESH_POLICY');
newsRuntime=replaceRequired(newsRuntime,"function refreshIfActive(){if(document.visibilityState==='visible'&&isActive())loadData(true)}","function invalidate(){if(document.visibilityState==='visible')loadData(true)}",'NEWS_INVALIDATION_HANDLER');
newsRuntime=replaceRequired(newsRuntime,"function startAutoRefresh(){if(autoRefreshTimer)return;autoRefreshTimer=setInterval(refreshIfActive,AUTO_REFRESH_MS)}","function bindInvalidation(){if(invalidationBound)return;invalidationBound=true;window.addEventListener('rona:client-market-news-invalidated',invalidate,{passive:true})}",'NEWS_TIMER_REMOVAL');
...
for(const forbidden of ['rona:client:background-sections','consumeBackground','refreshIfActive','AUTO_REFRESH_MS=','setInterval('])if(newsRuntime.includes(forbidden))throw new Error(`CLIENT_NEWS_EVENT_DRIVEN_FAILED: ${forbidden}`);
if(!newsRuntime.includes("REFRESH_POLICY='OPEN_CONTEXT_CHANGE_INVALIDATION'")||!newsRuntime.includes('rona:client-market-news-invalidated'))throw new Error('CLIENT_NEWS_EVENT_DRIVEN_CONTRACT_MISSING');
