...
reset role;
do $$begin if exists(select 1 from portal_private.payment_allocations where payment_key='44444444-4444-4444-8444-444444444441') then raise exception 'DRY_RUN_MUTATED'; end if; end$$;
...
\set ON_ERROR_STOP off
set role service_role;
insert into portal_private.owner_payment_authority_history_v4(event_type,action,entity_id,payment_id,deal_id,allocated_amount,currency,actor_user_id,idempotency_key,source_system,source_timestamp)
values('CLIENT_PAYMENT_ALLOCATION','AUTHORIZE',gen_random_uuid(),'PAY-QA-IN','DEAL-QA-A',1,'USD','aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa','bypass-history','OWNER_AUTHORIZED_CLIENT_PAYMENT_ALLOCATION_V4',now());
reset role;
\set ON_ERROR_STOP on
...
set role service_role;
select public.owner_deal_spend_allocate_v4(
 'PAY-QA-OUT','DEAL-QA-B',900000,'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa','qa-out-reassign',
 (select id from portal_private.owner_deal_spend_allocations_v4 where idempotency_key='qa-out-apply'),false) as outgoing_reassign;
reset role;
do $$declare v_old uuid;begin
 select id into v_old from portal_private.owner_deal_spend_allocations_v4 where idempotency_key='qa-out-apply';
 if not exists(select 1 from portal_private.owner_deal_spend_allocations_v4 where id=v_old and authority_state='SUPERSEDED' and lifecycle_state='SUPERSEDED') then raise exception 'OLD_NOT_SUPERSEDED'; end if;
 ...
 if not exists(select 1 from portal_private.owner_payment_authority_history_v4 where event_type='OWNER_AUTHORIZED_DEAL_SPEND' and action='SUPERSEDE' and entity_id=v_old) then raise exception 'SUPERSEDE_HISTORY_MISSING'; end if;
end$$;
