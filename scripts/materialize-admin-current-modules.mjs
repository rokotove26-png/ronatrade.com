...
async function materializeAnalyticsCurrent(){
  ...
  for(const token of ['approved-v4.3.1-single-owner','RONA TRADE · ANALYTICS','АИ-92','АИ-95','ДТ','LPG / СУГ','Platts LPG / СУГ','Petromarket · DAP benchmark','1075.25','1226.75','DATA-DRIVEN','FACT / CALCULATION / FORECAST','Аналитический вывод'])requireMarker(source,token,'approved analytics static runtime');
  for(const token of ['rona-analytics-canonical-title','ronaAnalyticsCanonicalHomeVisualV1','home-canonical-frames-title-v1','LPG Hairatan','Hairatan / Petromarket / Platts propane ref','Комментарий Коммерческого директора','Аналитическая лента'])if(source.includes(token))throw new Error(`STATIC_ANALYTICS_STALE_MARKER: ${token}`);
  if(response.headers.get('x-rona-analytics-visual')!=='approved-hero-v431')throw new Error('APPROVED_ANALYTICS_VISUAL_HEADER_MISSING');
  return source;
}
...
for(const marker of ["window.__RONA_CLIENTS_AGENTS_CURRENT__='20260826-single-owner-v4'",'ronaCreateAccess','admin-authority','Тип доступа','Договоры клиента','Профиль агента','Роль пользователя','openWithoutContract','История и права','Сменить пароль','Будет создан клиентский доступ по выбранным договорам.','Будет создан доступ в кабинет агента.','Создать доступ'])requireMarker(access,marker,'clients-agents static runtime');
