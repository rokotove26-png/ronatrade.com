...
])requireMarker(remaining,marker,'materialized Radio owner');
for(const forbidden of [
  "STAGE_2A_CORRECTIVE_CLIENT_CHAT_V1_LIVE_OWNER",
  "radioCanonicalRequest('/v1/admin/bootstrap')"
])if(remaining.includes(forbidden))throw new Error(`STATIC_RADIO_STALE_OWNER_MARKER: ${forbidden}`);
...
for(const marker of ["window.__RONA_CLIENTS_AGENTS_CURRENT__='20260828-single-owner-v5'","window.__RONA_ACCESS_CURRENT_OWNER__='clients-agents-current-v5'","window.__RONA_ACCESS_FUNCTIONAL_BUILD__='single-owner-impersonation-json-v8-20260920'",'ronaCreateAccess','admin-authority','Тип доступа','Роль пользователя','Ф.И.О. пользователя','Единый логин','Электронная почта','Пароль','Повторите пароль','Разрешённые компании / контракты','Компании назначаются агентам отдельно','Учётная запись создаётся независимо от статуса PDF','История и права','Сменить пароль','Создать пользователя',"await mutate('/access/users',payload)"])requireMarker(access,marker,'clients-agents static runtime');
for(const forbidden of ['openCanonicalAccessModal','installCanonicalAccessCreate','approved-canonical-v3.4.13','admin-canonical-create-access-v441-ui','rona-approved-access-mask','rona-canonical-access-mask','openWithoutContract','Открыть учётную запись до подтверждения PDF'])if(access.includes(forbidden))throw new Error(`STATIC_ACCESS_COMPETING_OWNER_MARKER: ${forbidden}`);
