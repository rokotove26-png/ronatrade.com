...
async function materializeAnalyticsCurrent(){
  ...
  for(const token of ['live-market-intelligence-v2','owner-bootstrap-v2','/portal/api/v1/admin/analytics','LIVE DATA','РАЗРЫВ ДАННЫХ','ФАКТ / MTD / FORWARD','Старый snapshot не используется','Устаревший локальный snapshot намеренно скрыт','Аналитический вывод','ronaAnalyticsBalancedLayoutV2','ronaAnalyticsCanonicalHomeVisualV1','ronaAnalyticsDesignerChartV2','rona-analytics-canonical-title'])requireMarker(source,token,'live analytics static runtime');
  for(const token of ['LOCAL_INPUT','LEGACY_DELTA','1075.25','1226.75','function pricingBridgeFor','function calculateRonaScenario','approved-v4.3.2-pricing-bridge-single-owner','RONA_Admin_LK_LOCAL_v4_3_2_Analytics_PricingBridge_Ready_Local.html'])if(source.includes(token))throw new Error(`STATIC_ANALYTICS_STALE_MARKER: ${token}`);
  if(response.headers.get('x-rona-analytics-ui')!=='live-market-intelligence-v2')throw new Error('LIVE_ANALYTICS_UI_HEADER_MISSING');
  if(response.headers.get('x-rona-analytics-owner')!=='commercial-director-live-data')throw new Error('LIVE_ANALYTICS_OWNER_HEADER_MISSING');
  if(response.headers.get('x-rona-analytics-source-file')!=='RONA_Admin_LK_LIVE_Market_Intelligence_v2')throw new Error('LIVE_ANALYTICS_SOURCE_FILE_HEADER_MISSING');
  if(response.headers.get('x-rona-analytics-data-source')!=='owner_analytics_admin_bootstrap')throw new Error('LIVE_ANALYTICS_DATA_SOURCE_HEADER_MISSING');
  if(response.headers.get('x-rona-analytics-refresh')!=='30s-pagechange-focus-root-rebind')throw new Error('LIVE_ANALYTICS_REFRESH_HEADER_MISSING');
  return source;
}
