...
async function materializeAnalyticsCurrent(){
  ...
  for(const token of ['live-market-intelligence-v1','owner-bootstrap-v1','/portal/api/v1/admin/analytics','LIVE DATA','РАЗРЫВ ДАННЫХ','ФАКТ / MTD / FORWARD','Старый snapshot не используется','Устаревший локальный snapshot намеренно скрыт','Аналитический вывод'])requireMarker(source,token,'live analytics static runtime');
  ...
  if(response.headers.get('x-rona-analytics-ui')!=='live-market-intelligence-v1')throw new Error('LIVE_ANALYTICS_UI_HEADER_MISSING');
  ...
  if(response.headers.get('x-rona-analytics-source-file')!=='RONA_Admin_LK_LIVE_Market_Intelligence_v1')throw new Error('LIVE_ANALYTICS_SOURCE_FILE_HEADER_MISSING');
  ...
  if(response.headers.get('x-rona-analytics-refresh')!=='300s-plus-focus')throw new Error('LIVE_ANALYTICS_REFRESH_HEADER_MISSING');
  return source;
}
