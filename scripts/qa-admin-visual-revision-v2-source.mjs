...
for(const prohibited of ['background:#555','background:#222','background:#111','data:image/'])assert.ok(!v4.includes(prohibited),`V4 contains prohibited opaque/raster pattern ${prohibited}`);
for(const preserved of ['adminVisualRevisionV3Fix',"headers.set('x-rona-admin-visual-v4','layered-frosted-glass-v4')","'data-rona-app-passport-open':String(a?.application_id||'')","text:'Ресурс одобрен'","text:'В ресурсе отказано'","text:'Отправить в сделки'"])assert.ok(index.includes(preserved),`preserved Admin hook missing ${preserved}`);
...
for(const required of ['__RONA_ADMIN_VISUAL_REVISION_V4__','layered-backdrop-frosted-glass','rona-admin-v4-page-hero','Главная','data-rona-app-passport-open'])assert.ok(emitted.includes(required),`emitted V4 runtime missing ${required}`);

console.log('V4_REAL_UI_IMPLEMENTED=PASS implementation=DOM_CSS_runtime_only');
console.log('CANONICAL_HOME_TITLE=PASS shared_page_hero_architecture=true');
