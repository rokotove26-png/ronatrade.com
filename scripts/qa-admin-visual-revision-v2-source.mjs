...
for(const required of ["__RONA_ADMIN_VISUAL_V5_FINALIZE__='20260912-v5.2'",'rona-admin-visual-v5','home-title-canonical-v5.2',"dataset.ronaCanonicalSource='shared-admin-dashboard-hero'",'rona-admin-dashboard__hero rona-admin-v2-hero rona-admin-v4-page-hero','rona-admin-v5-canonical-eyebrow','RONA TRADE · OPERATIONS','rgba(14,43,63,.64)','rgba(17,55,76,.50)','rgba(11,44,64,.58)','rgba(8,40,58,.44)','backdrop-filter:blur(19px) saturate(134%)','rona-current-deal-kpi--active','rona-current-deal-kpi--attention','rona-current-deal-kpi--complete','rgba(35,143,98,.24)','rgba(164,112,35,.24)','rgba(152,58,78,.24)',':has(.rona-fin-pill.is-warn)'])assert.ok(v5.includes(required),`V5 owner correction missing ${required}`);
...
for(const required of ['__RONA_ADMIN_VISUAL_REVISION_V4__','__RONA_ADMIN_VISUAL_V5_FINALIZE__','home-title-canonical-v5.2','rona-admin-visual-v5','rona-current-deal-kpi--attention','Главная','RONA TRADE · OPERATIONS','data-rona-app-passport-open'])assert.ok(emitted.includes(required),`emitted V5 runtime missing ${required}`);

console.log('V5_REAL_UI_IMPLEMENTED=PASS implementation=DOM_CSS_runtime_only');
console.log('CANONICAL_HOME_TITLE=PASS canonical_source=shared_admin_dashboard_hero same_component_architecture=true same_scale_spacing_treatment=true home_only_composition_removed=true');
