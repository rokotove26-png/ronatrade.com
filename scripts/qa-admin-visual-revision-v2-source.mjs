...
for(const required of ["'20260912-v4.4'",'#page-home #ronaAdminV2HomeTitle{display:grid!important','grid-template-columns:minmax(0,1fr)!important','grid-column:1!important','min-height:136px!important','rona-admin-v4-home-eyebrow','rona-admin-v4-home-subtitle',"eyebrow.textContent='RONA TRADE · OPERATIONS'",'Операционный центр и актуальное состояние исполнения.'])assert.ok(v4Finalize.includes(required),`V4 canonical Home hero missing ${required}`);
...
console.log('CANONICAL_HOME_TITLE=PASS shared_page_hero_architecture=true stacked_header=true legacy_specificity_overridden=true');
