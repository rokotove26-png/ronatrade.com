(()=>{
  ...
  function handoffRecoveryLink() {
    try {
      const topWindow = window.top;
      if (!topWindow || topWindow !== window && topWindow.location.origin !== window.location.origin) return false;
      const hash = String(topWindow.location.hash || '');
      if (!hash.startsWith('#')) return false;
      const p = new URLSearchParams(hash.slice(1));
      const isRecovery = p.get('type') === 'recovery' || p.has('error') || p.has('error_code');
      if (!isRecovery) return false;
      topWindow.location.replace('/portal/recovery' + hash);
      return true;
    } catch (_) { return false; }
  }
  if (handoffRecoveryLink()) return;
  ...
  function statusNode(doc, panel) {
    ...
    if (el) return el;
    el = doc.createElement('div'); el.dataset.ronaInlineAuthStatus = 'g82-v2'; el.setAttribute('role','status'); el.setAttribute('aria-live','polite');
  ...
  function bindDoc(doc){
    ...
    const refresh=()=>{const panel=findPanel(doc);if(panel)preparePanel(doc,panel);doc.querySelectorAll('iframe').forEach(bindFrame)};const mo=new MutationObserver(refresh);mo.observe(doc.documentElement,{childList:true,subtree:true});refresh();
  }
