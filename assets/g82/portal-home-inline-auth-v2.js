(()=>{
  ...
  const hasPanelLabel = text => {
    const t = norm(text);
    return PANEL_LABELS.some(x => t.includes(x));
  };
  const localPortalTarget = value => {
    try {
      const u = new URL(String(value || ''), window.location.origin);
      if (u.origin !== window.location.origin) return null;
      return ['/portal/select','/portal/admin','/portal/staff','/portal/agent','/portal/client'].includes(u.pathname) ? u.pathname : null;
    } catch (_) { return null; }
  };
  ...
  function nearestCredentialHost(seed, requireLabel) {
    ...
    for (let i = 0; i < 12 && cur; i += 1, cur = cur.parentElement) {
      ...
      const usr = cur.querySelector?.('input[type="email"],input[name*="email" i],input[name*="login" i],input[type="text"],input:not([type])');
  ...
  function findPanel(doc) {
    ...
    for (const pwd of passwords) {
      const host = nearestCredentialHost(pwd, true);
      if (host) return host;
    }
    for (const pwd of passwords) {
      const host = nearestCredentialHost(pwd, false);
      if (host) return host;
    }
    return null;
  }
  ...
  function fieldSet(panel) {
    ...
    let email = panel.querySelector('input[type="email"],input[name*="email" i],input[name*="login" i]');
    if (!email) {
      const texts = [...panel.querySelectorAll('input[type="text"],input:not([type])')];
      email = texts.find(x => x !== password) || null;
    }
    const form = password?.closest('form') || email?.closest('form') || panel.querySelector('form') || null;
    const primaryScope = form || panel;
    let button = primaryScope.querySelector('button[type="submit"],input[type="submit"]');
    if (!button) {
      const candidates = [...primaryScope.querySelectorAll('button,[role="button"],input[type="button"]')];
      button = candidates.find(el => /войти|login|sign in|вход/i.test(String(el.textContent || el.value || ''))) || candidates[0] || null;
    }
    if (!button && form) {
      const candidates = [...panel.querySelectorAll('button,input[type="submit"],input[type="button"],[role="button"]')];
      button = candidates.find(el => /войти|login|sign in|вход/i.test(String(el.textContent || el.value || ''))) || candidates[0] || null;
    }
    return { email, password, form, button };
  }
  ...
  function statusNode(doc, panel) {
    ...
    el.dataset.ronaInlineAuthStatus = 'g82-v2';
    el.setAttribute('role', 'status');
    el.setAttribute('aria-live', 'polite');
    el.style.cssText = 'display:none;margin-top:8px;font:500 12px/1.35 Arial,sans-serif;color:#8b1e2d;max-width:100%';
    const { form, button } = fieldSet(panel);
    const anchor = form || button?.parentElement || panel;
    anchor.appendChild(el);
    return el;
  }

  function setStatus(doc, panel, message, error = true) {
    const el = statusNode(doc, panel);
    el.textContent = message || '';
    el.style.display = message ? 'block' : 'none';
    el.style.color = error ? '#8b1e2d' : '#425466';
  }

  function setLoading(button, active) {
    if (!button) return;
    if (active) {
      if (!button.dataset.ronaOriginalLabel) button.dataset.ronaOriginalLabel = String(button.textContent || button.value || 'Войти');
      if ('disabled' in button) button.disabled = true;
      if (String(button.tagName).toUpperCase() === 'INPUT') button.value = 'Вход…';
      else button.textContent = 'Вход…';
      button.setAttribute('aria-busy', 'true');
    } else {
      if ('disabled' in button) button.disabled = false;
      const label = button.dataset.ronaOriginalLabel || 'Войти';
  ...
  function enableFields(email, password, button) {
    for (const el of [email, password, button]) {
      if (!el) continue;
  ...
  function preparePanel(doc, panel) {
    ...
    const { email, password, button } = fieldSet(panel);
    if (!email || !password || !button) return false;
    cleanLegacy(doc, panel);
    enableFields(email, password, button);
    panel.dataset.ronaInlineAuth = 'g82-v2';
    statusNode(doc, panel);
    return true;
  }
  ...
  async function authenticate(doc, panel) {
    ...
    if (!login || !secret) {
      setStatus(doc, panel, 'Введите логин и пароль.');
      return;
    }
    busyPanels.add(panel);
    setStatus(doc, panel, '');
    ...
    try {
      ...
      if (!target) {
        password.value = '';
        setStatus(doc, panel, 'Не удалось определить разрешённый кабинет. Повторите попытку.');
        return;
      }
      setStatus(doc, panel, 'Вход выполнен. Открываем кабинет…', false);
      try { window.top.location.assign(target); }
      catch (_) { window.location.assign(target); }
    } catch (_) {
      password.value = '';
      setStatus(doc, panel, 'Нет связи с сервером авторизации. Проверьте соединение и повторите попытку.');
    } finally {
      busyPanels.delete(panel);
      setLoading(button, false);
    }
  }

  function eventPanel(doc, target) {
  ...
  function bindDoc(doc) {
    if (!doc || !doc.documentElement || boundDocs.has(doc)) return;
    boundDocs.add(doc);

    doc.addEventListener('submit', e => {
      const form = e.target;
      const panel = eventPanel(doc, form);
      if (!panel) return;
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      authenticate(doc, panel);
    }, true);
    ...
    const refresh = () => {
      const panel = findPanel(doc);
      if (panel) preparePanel(doc, panel);
  ...
  function bindFrame(frame) {
    ...
    if (!frame.dataset.ronaInlineAuthFrameBound) {
      frame.dataset.ronaInlineAuthFrameBound = '1';
