...
const state={apps:[],contextKey:'',loading:false,lastLoad:0,raf:0,unsubscribe:null,resizeObserver:null,mutationObserver:null};
...
function render(){const r=root();if(!r)return false;ensureStyle();retireLegacyApplicationsPresentation(r);const active=state.apps.filter(isActive),f=currentFilter(r),rows=active.filter(a=>matches(a,f)),list=ensureList(r);list.innerHTML=rows.length?rows.map(rowHtml).join(''):`<div class="rona-live-app-empty">${active.length?'По текущему фильтру заявок нет.':'Активных заявок в выбранном контексте нет.'}</div>`;retireLegacyApplicationsPresentation(r);updateCounter(r,active.length);r.dataset.ronaApplicationsLiveRender='ready';scheduleAlign();window.dispatchEvent(new CustomEvent('rona:client-applications-rendered'));return true}
...
function observeLayout(){const r=root();if(!r)return;state.resizeObserver?.disconnect?.();if('ResizeObserver'in window){state.resizeObserver=new ResizeObserver(scheduleAlign);state.resizeObserver.observe(r);const input=searchInput(r);if(input?.parentElement)state.resizeObserver.observe(input.parentElement)}state.mutationObserver?.disconnect?.();state.mutationObserver=new MutationObserver(()=>{retireLegacyApplicationsPresentation(r);scheduleAlign()});state.mutationObserver.observe(r,{childList:true,subtree:true,attributes:true,attributeFilter:['class','style','hidden','aria-hidden']})}
function applicationsVisible(){const r=root();if(!r||!r.isConnected)return false;const s=getComputedStyle(r);return s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity)!==0}
function isApplicationsNavigation(target){const explicit=target?.closest?.('[data-page="applications"],[data-page-link="applications"],[data-page-id="applications"],[data-page-panel="applications"]');if(explicit)return true;const el=target?.closest?.('a,button,[role="tab"],[role="menuitem"]');return /^Заявки$/iu.test(norm(el?.textContent))}
function invalidateAndRefresh(reason){state.lastLoad=0;if(applicationsVisible())load(true).catch?.(()=>{});try{window.__RONA_CLIENT_APPLICATIONS_REFRESH_STATE__={mode:'EVENT_DRIVEN',reason,at:new Date().toISOString()}}catch{}}
function start(){ensureStyle();observeLayout();if(applicationsVisible())load(true);const a=authority();if(a?.subscribe)state.unsubscribe=a.subscribe(()=>{state.apps=[];state.contextKey='';state.lastLoad=0;if(applicationsVisible())load(true);setTimeout(observeLayout,0)});document.addEventListener('input',e=>{const r=root();if(r&&r.contains(e.target)&&e.target===searchInput(r))render()},true);document.addEventListener('change',e=>{const r=root();if(r&&r.contains(e.target)&&(e.target===searchInput(r)||e.target===statusSelect(r)))render()},true);document.addEventListener('click',e=>{if(isApplicationsNavigation(e.target)){setTimeout(()=>{observeLayout();load(true)},80);return}const button=e.target?.closest?.('[data-rona-open-application]');if(!button)return;const id=button.getAttribute('data-rona-open-application'),r=root(),detail=r?.querySelector(`[data-rona-application-details="${CSS.escape(id)}"]`);if(!detail)return;const open=detail.hidden;detail.hidden=!open;button.setAttribute('aria-expanded',String(open));button.textContent=open?'Скрыть':'Открыть';retireLegacyApplicationsPresentation(r);scheduleAlign()},true);window.addEventListener('resize',scheduleAlign,{passive:true});window.addEventListener('rona:client-application-submitted',()=>{state.lastLoad=0;setTimeout(()=>load(true),120);setTimeout(()=>load(true),900)});window.addEventListener('rona:client-applications-invalidated',()=>invalidateAndRefresh('explicit-invalidation'),{passive:true});window.addEventListener('pageshow',()=>{if(applicationsVisible())load(true);scheduleAlign();setTimeout(observeLayout,0)},{passive:true})}
