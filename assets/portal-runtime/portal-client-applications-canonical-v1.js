...
const state={apps:[],contextKey:'',loading:false,reloadRequested:false,reloadForce:false,lastLoad:0,timer:0,raf:0,unsubscribe:null,resizeObserver:null,mutationObserver:null};
...
async function load(force=false){if(state.loading){state.reloadRequested=true;state.reloadForce=state.reloadForce||force;return}const ctx=await currentContext().catch(()=>null);if(!ctx)return;const key=contextKey(ctx);if(!key||key==='|')return;state.loading=true;try{const a=authority();const detail=force&&a?.refreshCurrentProjection?await a.refreshCurrentProjection('applications-canonical'):await a?.whenCurrentProjection?.('applications-canonical');if(contextKey(authority()?.getCurrentContext?.())!==key)return;if(!detail)throw new Error('CLIENT_CURRENT_PROJECTION_UNAVAILABLE');state.apps=Array.isArray(detail?.applications)?detail.applications:[];state.contextKey=key;state.lastLoad=Date.now();render()}catch(error){console.error('RONA client applications canonical projection',error);const r=root();if(r)r.dataset.ronaApplicationsLiveRender='error'}finally{state.loading=false;if(state.reloadRequested){const queuedForce=state.reloadForce;state.reloadRequested=false;state.reloadForce=false;queueMicrotask(()=>load(queuedForce))}}}
...
function start(){ensureStyle();load(false);observeLayout();const a=authority();if(a?.subscribe)state.unsubscribe=a.subscribe(()=>{state.apps=[];state.contextKey='';state.lastLoad=0;load(false);setTimeout(observeLayout,0)});document.addEventListener('input',e=>{const r=root();if(r&&r.contains(e.target)&&e.target===searchInput(r))render()},true);document.addEventListener('change',e=>{const r=root();if(r&&r.contains(e.target)&&(e.target===searchInput(r)||e.target===statusSelect(r)))render()},true);document.addEventListener('click',e=>{const button=e.target?.closest?.('[data-rona-open-application]');if(!button)return;const id=button.getAttribute('data-rona-open-application'),r=root(),detail=r?.querySelector(`[data-rona-application-details="${CSS.escape(id)}"]`);if(!detail)return;const open=detail.hidden;detail.hidden=!open;button.setAttribute('aria-expanded',String(open));button.textContent=open?'Скрыть':'Открыть';retireLegacyApplicationsPresentation(r);scheduleAlign()},true);window.addEventListener('resize',scheduleAlign,{passive:true});window.addEventListener('rona:client-current-projection',()=>load(false),{passive:true});window.addEventListener('rona:client-application-submitted',()=>setTimeout(()=>load(true),120));window.addEventListener('pageshow',()=>{load(false);scheduleAlign();setTimeout(observeLayout,0)},{passive:true})}
