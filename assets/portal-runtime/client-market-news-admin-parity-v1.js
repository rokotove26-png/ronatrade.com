...
if(window.__RONA_CLIENT_MARKET_NEWS_ADMIN_PARITY_V1__)return;
window.__RONA_CLIENT_MARKET_NEWS_ADMIN_PARITY_V1__='20260902-admin-news-canonical-exact-v1';

const ADMIN_VISUAL_CONTRACT='portal-market-news-current-v1/20260827-auto-refresh-v2';
...
function rowKey(x){return String(x?.duplicate_group||x?.news_id||x?.source_version||x?.publication_id||x?.publication_item_id||x?.id||[headline(x),timestamp(x),sourceName(x)].join('|'))}
...
function installStyle(){
  ...
  if(!s){s=el('style',{id:STYLE_ID});document.head.appendChild(s)}
  s.textContent=`
...
function ensureRoot(){
  ...
  if(!root){root=el('section',{id:ROOT_ID,class:'rona-market-news-current','data-rona-market-news-owner':'admin-canonical-client-safe-v1'});}
  root.className='rona-market-news-current';
  root.setAttribute('data-rona-market-news-owner','admin-canonical-client-safe-v1');
  root.setAttribute('data-rona-market-news-visual-contract',ADMIN_VISUAL_CONTRACT);
  if(root.parentElement!==p)p.replaceChildren(root);
  else for(const child of Array.from(p.children))if(child!==root)child.remove();
  p.dataset.ronaClientMarketNewsReady='true';
  document.documentElement.dataset.ronaClientMarketNewsOwner='admin-canonical-client-safe-v1';
  return root;
}
...
function render(){
  ...
  root.dataset.ronaClientMarketNewsFingerprint=state.fingerprint;
  return true;
}
function cachedData(){const entry=window.__RONA_CLIENT_BACKGROUND_CACHE__?.[API_PATH];return entry?.ok&&entry?.body?.ok&&entry?.body?.data?entry.body.data:null}
function acceptData(data){
  if(!data||data.version!=='RONA_CLIENT_MARKET_INTELLIGENCE_V1'||!Array.isArray(data.news))return false;
  const incoming=dedupeRows(data.news.slice());
  ...
  state.loaded=true;state.error='';state.updatedAt=new Date().toISOString();
  return true;
}
async function loadData(force=false){
  if(state.loading)return;
  const cached=cachedData();if(cached)acceptData(cached);
  if(state.loaded&&!force){render();return}
  state.loading=true;state.error='';render();
  try{
    const r=await fetch(API,{credentials:'same-origin',cache:'no-store',headers:{accept:'application/json','x-rona-client-market-news-visual-owner':'admin-canonical-exact-v1'}});
    let j={};try{j=await r.json()}catch(_){ }
    if(!r.ok||j?.ok===false||!acceptData(j?.data))throw new Error(String(j?.code||'HTTP_'+r.status));
  }catch(err){
...
function isActive(){return Boolean(page()?.classList.contains('active'))}
function consumeBackground(){const data=cachedData();if(data&&acceptData(data))render()}
function activate(){ensureRoot();consumeBackground();render();loadData(true)}
function start(){
  ensureRoot();consumeBackground();loadData(true);
