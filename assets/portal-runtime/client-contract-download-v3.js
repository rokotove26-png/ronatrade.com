...
const API='/portal/api',STALE_MS=30000,STYLE_ID='ronaClientContractDownloadV3Style';
...
async function refresh(force=false){
  if(state.loading){if(force)state.pendingImmediate=true;return}if(!force&&Date.now()-state.lastLoad<STALE_MS){render();return}state.loading=true;
  try{
...
async function start(){
  ...
  window.addEventListener('rona:client-context-ready',()=>{primeFromAuthority(authority);refresh(true)});window.addEventListener('rona:client-context-changed',()=>{primeFromAuthority(authority);refresh(true)});window.addEventListener('rona:client-company-directory-ready',()=>scheduleRender(0));window.addEventListener('rona:client-company-directory-invalidated',()=>refresh(true),{passive:true});
...
const OWNER_SECTION_TYPO_MARK='RONA_CLIENT_OWNER_SECTION_TYPOGRAPHY_110_V4_EXACT_PARENT';
const OWNER_SECTION_TYPO_SCALE=1.1;
const OWNER_SECTION_TYPO_BASE_ATTR='data-rona-owner-section-typo-base-v4';
const OWNER_SECTION_TYPO_PAGE_SELECTOR='section[id^="page-"],[data-page-panel],[data-page-id]';
const OWNER_SECTION_TYPO_ANALYTICS_SELECTOR='#page-analytics,#analyticsPage,[data-page-panel="analytics"],[data-page-id="analytics"]';
const OWNER_SECTION_TYPO_SHELL_SELECTOR='nav,aside,header,footer,[role="navigation"],#nav,.topbar,.top-bar,.sidebar,[data-topbar],[data-sidebar],[data-footer-shell],[data-role-title],[data-role-pill],.role-title,.role-pill';
let ownerSectionTypoObserver=null,ownerSectionTypoQueued=false,ownerSectionTypoPending=new Set();
function ownerSectionTypoPage(el){if(!el||el.nodeType!==1)return null;const page=el.closest(OWNER_SECTION_TYPO_PAGE_SELECTOR);if(!page||page.matches(OWNER_SECTION_TYPO_ANALYTICS_SELECTOR)||page.closest(OWNER_SECTION_TYPO_ANALYTICS_SELECTOR))return null;return page}
function ownerSectionTypoHeading(page){if(!page)return null;const marked=page.querySelector('[data-page-title],.page-title,.section-title');if(marked)return marked;return [...page.querySelectorAll('h1,h2')].find(el=>ownerSectionTypoPage(el)===page)||null}
function ownerSectionTypoEligible(el){if(!ownerTypoDirectText(el))return false;const page=ownerSectionTypoPage(el);if(!page)return false;if(el.closest(OWNER_SECTION_TYPO_SHELL_SELECTOR))return false;const heading=ownerSectionTypoHeading(page);if(heading&&(el===heading||heading.contains(el)))return false;return true}
function ownerSectionTypoTargets(root){if(!root||root.nodeType!==1)return[];return[root,...root.querySelectorAll('*')].filter(el=>!el.hasAttribute(OWNER_SECTION_TYPO_BASE_ATTR)&&ownerSectionTypoEligible(el))}
function ownerSectionTypoApply(targets){const items=[...new Set(targets)].filter(el=>el?.isConnected&&!el.hasAttribute(OWNER_SECTION_TYPO_BASE_ATTR)&&ownerSectionTypoEligible(el));if(!items.length)return 0;const needsParentLayer=items.filter(el=>!el.hasAttribute(OWNER_TYPO_BASE_ATTR));if(needsParentLayer.length)ownerTypoApply(needsParentLayer);const restore=new Map();for(const el of items){for(let p=el.parentElement;p;p=p.parentElement){if(!p.hasAttribute(OWNER_SECTION_TYPO_BASE_ATTR)||restore.has(p))continue;const base=Number(p.getAttribute(OWNER_SECTION_TYPO_BASE_ATTR));if(!Number.isFinite(base)||base<=0)continue;restore.set(p,{value:p.style.getPropertyValue('font-size'),priority:p.style.getPropertyPriority('font-size')});p.style.setProperty('font-size',`${base}px`,'important')}}const baselines=items.map(el=>parseFloat(getComputedStyle(el).fontSize));for(const [el,old] of restore){if(old.value)el.style.setProperty('font-size',old.value,old.priority);else el.style.removeProperty('font-size')}let applied=0;items.forEach((el,index)=>{const base=baselines[index];if(!Number.isFinite(base)||base<=0)return;const scaled=Math.round(base*OWNER_SECTION_TYPO_SCALE*10000)/10000;el.setAttribute(OWNER_SECTION_TYPO_BASE_ATTR,String(base));el.setAttribute('data-rona-owner-section-typo-scaled-v4','1.10');el.style.setProperty('font-size',`${scaled}px`,'important');applied++});document.documentElement.dataset.ronaClientOwnerSectionTypography=OWNER_SECTION_TYPO_MARK;return applied}
function ownerSectionTypoFlush(){ownerSectionTypoQueued=false;const roots=[...ownerSectionTypoPending];ownerSectionTypoPending.clear();const targets=[];for(const root of roots)targets.push(...ownerSectionTypoTargets(root));ownerSectionTypoApply(targets)}
function ownerSectionTypoQueue(root){if(root?.nodeType===1)ownerSectionTypoPending.add(root);if(ownerSectionTypoQueued)return;ownerSectionTypoQueued=true;queueMicrotask(ownerSectionTypoFlush)}
function startOwnerSectionTypography(){if(!document.body)return;ownerSectionTypoApply(ownerSectionTypoTargets(document.body));if(ownerSectionTypoObserver)return;ownerSectionTypoObserver=new MutationObserver(records=>{for(const record of records){if(record.type==='characterData'){ownerSectionTypoQueue(record.target?.parentElement);continue}for(const node of record.addedNodes){if(node.nodeType===1)ownerSectionTypoQueue(node);else if(node.nodeType===3)ownerSectionTypoQueue(node.parentElement)}}});ownerSectionTypoObserver.observe(document.body,{childList:true,subtree:true,characterData:true});window.RONA_CLIENT_OWNER_SECTION_TYPOGRAPHY=Object.freeze({version:OWNER_SECTION_TYPO_MARK,scale:OWNER_SECTION_TYPO_SCALE,rescan:()=>ownerSectionTypoApply(ownerSectionTypoTargets(document.body))})}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',startOwnerSectionTypography,{once:true});else startOwnerSectionTypography();
})();