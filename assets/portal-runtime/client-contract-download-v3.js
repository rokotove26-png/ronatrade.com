...
const state={entries:new Map(),loading:false,lastLoad:0,renderTimer:0,currentKey:'',observer:null,setterHooked:false,rendering:false,unsubscribe:null};
...
function hydrateFrozenClientModel(){
  ...
  for(const entry of state.entries.values()){
    ...
    if(!id||!num)continue;
    const row=model[id];if(!row||typeof row!=='object')continue;
    row.contractNo=num.replace(/^№\s*/u,'');row.contractStateBlocked=String(ctx.contract_status||'').toUpperCase()!=='ACTIVE';
    if(!row.contractStateBlocked&&/уточн/i.test(String(row.status||'')))row.status='Действует';count++;
  }
  if(count)document.documentElement.dataset.ronaClientContractModel='authoritative';
  return count;
}
function selectedContextEntry(){
  const current=contextAuthority()?.getCurrentContext?.()||null;
  if(!current){state.currentKey='';return null}
  state.currentKey=norm(current.contract_id);
  return entryByContractId(current.contract_id);
}
async function refresh(force=false){
  ...
  state.loading=true;
  try{
    const authority=await authorityReady(),contexts=authority.getAuthorizedContexts(),current=authority.getCurrentContext(),currentKey=contextKey(current);
    ...
    if(current){
      ...
      if(contextKey(authority.getCurrentContext())!==currentKey)return;
      currentData=detail?.data||{};
    }
    const rows=contexts.map(base=>{
      if(current&&contextKey(base)===currentKey)return{context:effectiveContext(base,currentData),document:currentContractDocument(currentData?.documents)};
      return{context:base,document:null};
    });
    state.entries=new Map(rows.map((r,i)=>[String(r.context?.contract_id||r.context?.client_id||i),r]));
    state.currentKey=norm(current?.contract_id);state.lastLoad=Date.now();hydrateFrozenClientModel();
    window.__RONA_CLIENT_CONTRACT_DOWNLOAD_STATE__={
      version:MARK,current_contract_id:current?.contract_id||null,
      context_source:'RONA_CLIENT_CONTEXT_AUTHORITY',
      entries:rows.map(r=>({client_id:r.context?.client_id||null,legal_name:r.context?.legal_name||null,contract_id:r.context?.contract_id||null,current_external_contract_number:r.context?.current_external_contract_number||null,contract_status:r.context?.contract_status||null,document_id:r.document?.document_id||null,storage_object_id:r.document?.storage_object_id||null,current:current?contextKey(r.context)===currentKey:false})),
...
function resolveEntry(node,maxDepth=10){
  const scopes=[];let n=node instanceof Element?node:node?.parentElement||null;
  ...
  for(const entry of state.entries.values()){
    const c=entry.context||{},contract=low(c.contract_id),client=low(c.client_id),number=low(c.current_external_contract_number),tokens=identityTokens(c);let score=0;
    for(const s of scopes){
      if(contract&&s.includes(contract))score=Math.max(score,10000);if(client&&s.includes(client))score=Math.max(score,3000);if(number&&s.includes(number))score=Math.max(score,1200);
      let tokenScore=0;const compact=tokenKey(s);for(const t of tokens)if(compact.includes(t))tokenScore+=40;score=Math.max(score,tokenScore);
    }
    if(score>bestScore){nextScore=bestScore;bestScore=score;best=entry}else if(score>nextScore)nextScore=score;
  }
...
function findCompanyCard(ctx){
  ...
  for(const node of companyCardCandidates()){
    const t=low(node.textContent);if(!t||t.length>9000||(!t.includes('подписанный контракт')&&!t.includes('текущая компания')&&!t.includes('открыть компанию')&&!t.includes('переключиться')))continue;
    ...
    const score=(hc?10000:0)+(hi?3000:0)+tokenScore+500-Math.min(t.length,8000)/8;c.push({node,score,len:t.length});
  }
  c.sort((a,b)=>b.score-a.score||a.len-b.len);return c[0]?.node||null;
}
function numberEntryFor(node,current){
  const direct=resolveEntry(node,14);if(direct)return direct;
  let n=node instanceof Element?node:node?.parentElement||null;
  for(let i=0;n&&n!==document.body&&i<12;i++,n=n.parentElement){const t=low(n.textContent);if(t.includes('выбрана компания')||t.includes('активный контекст'))return current||selectedContextEntry()}
  const r=node?.getBoundingClientRect?.();if(r&&r.top>=0&&r.top<260)return current||selectedContextEntry();return current||selectedContextEntry();
}
function authoritativeContractText(value,num){
  const n=norm(num);if(!n)return String(value||'');let next=String(value||'');
  next=next.replace(/(контракт|договор)\s*(?:№|номер|:)?\s*(?:(?:номер\s+)?уточняется|[0-9][A-Za-zА-Яа-яЁё0-9/._-]{2,})/giu,(_,kind)=>kind+' № '+n);
  if(/^номер\s+уточняется$/iu.test(norm(next)))return '№ '+n;next=next.replace(/номер\s+уточняется/giu,n);return next;
}
function syncContractNumberLabels(current){
  const root=document.body;if(!root)return;const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT),pending=[];let node;
  while((node=walker.nextNode())){const v=norm(node.nodeValue);if(/номер\s+уточняется/i.test(v)||/(?:контракт|договор)\s*(?:№|номер|:)?\s*(?:[0-9][A-Za-zА-Яа-яЁё0-9/._-]{2,}|(?:номер\s+)?уточняется)/iu.test(v))pending.push(node)}
  for(const t of pending){const parent=t.parentElement;if(!parent||!visible(parent))continue;const entry=numberEntryFor(parent,current),num=norm(entry?.context?.current_external_contract_number);if(!num)continue;const before=String(t.nodeValue||''),after=authoritativeContractText(before,num);if(after!==before){t.nodeValue=after;parent.dataset.ronaContractNumberSynced='authoritative'}}
}
function escapeRe(v){return String(v).replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}
function flexibleCanonicalWordRe(word){let out='';for(const ch of String(word)){if(APOSTROPHES.includes(ch))out+='['+escapeRe(APOSTROPHES)+']?';else out+=escapeRe(ch)}return new RegExp('\\b'+out+'\\b','giu')}
function syncCompanyNameSpelling(){
  for(const entry of state.entries.values()){
    const ctx=entry.context||{},words=canonicalBusinessWords(ctx);if(!words.length)continue;const canonicals=words.filter(w=>tokenKey(w).length>=3),card=findCompanyCard(ctx),scopes=[];if(card)scopes.push(card);
    for(const el of document.querySelectorAll('header,[class*="top"],[class*="context"],[class*="company"]')){if(!visible(el))continue;const resolved=resolveEntry(el,14);if(resolved===entry&&!scopes.includes(el))scopes.push(el)}
    for(const scope of scopes){const walker=document.createTreeWalker(scope,NodeFilter.SHOW_TEXT);let t;while((t=walker.nextNode())){const parent=t.parentElement;if(!parent||!visible(parent))continue;const value=String(t.nodeValue||'');let next=value;for(const canonical of canonicals){const key=tokenKey(canonical),raw=canonical.replace(new RegExp('['+APOSTROPHES+']','g'),'');if(tokenKey(raw)===key)next=next.replace(new RegExp('\\b'+escapeRe(raw)+'\\b','giu'),canonical);next=next.replace(flexibleCanonicalWordRe(canonical),canonical)}if(next!==value){t.nodeValue=next;parent.dataset.ronaCompanyNameSynced='authoritative'}}}
  }
}
...
function syncCompanyControls(){
  ensureStyle();
  for(const entry of state.entries.values()){
    const card=findCompanyCard(entry.context);if(!card)continue;const current=currentStatusNode(card);
    if(current){
      const control=current.closest('button,a,[role="button"]')||current;if(low(current.textContent)!=='текущая компания')current.textContent='Текущая компания';
      control.dataset.ronaCurrentCompanyStatus='true';control.setAttribute('aria-current','true');control.setAttribute('aria-disabled','true');control.setAttribute('role','status');if('disabled'in control)control.disabled=true;if(control.matches('a'))control.removeAttribute('href');control.tabIndex=-1;
      if(!control.dataset.ronaCurrentCompanyGuard){control.dataset.ronaCurrentCompanyGuard='true';control.addEventListener('click',preventCurrentCompanyAction,true)}
    }else{
      const action=switchNode(card);if(action){if(low(action.textContent)!=='переключиться')action.textContent='Переключиться';const control=action.closest('button,a,[role="button"]')||action;control.setAttribute('aria-label','Переключиться на '+norm(entry.context?.legal_name||entry.context?.client_id))}
    }
  }
}
...
function clearNonCurrentEntry(entry){const card=findCompanyCard(entry.context);if(!card)return;for(const old of card.querySelectorAll('button[data-rona-contract-download-v3]'))old.remove()}
function renderEntry(entry){
  ...
  if(!entry.document?.storage_object_id){if(existing)existing.remove();if(unavailable){const msg='Файл подписанного контракта не опубликован в кабинете';if(unavailable.textContent!==msg)unavailable.textContent=msg;unavailable.setAttribute('aria-disabled','true');unavailable.dataset.ronaContractUnavailable='authoritative-storage-not-materialized'}return false}
  if(existing&&existing.dataset.storageObjectId===String(entry.document.storage_object_id))return true;if(existing)existing.remove();const b=makeButton(entry,unavailable);if(unavailable){unavailable.replaceWith(b);return true}const anchor=contractAnchor(card),host=anchor?.parentElement||card;host.appendChild(b);return true;
}
function hookContextSetter(){
  if(state.setterHooked)return;const original=window.setClientContext;if(typeof original!=='function')return;
  const wrapped=function(contractId,...args){const id=norm(contractId),entry=entryByContractId(id),authority=contextAuthority();if(entry&&authority){try{authority.select(entry.context.client_id,entry.context.contract_id)}catch(error){console.error('RONA context selection bridge',error)}}const out=original.apply(this,[contractId,...args]);scheduleRender(40);return out};
  Object.defineProperty(wrapped,'__ronaAuthoritativeWrapped',{value:true});window.setClientContext=wrapped;state.setterHooked=true;
}
function render(){
  if(state.rendering)return false;state.rendering=true;
  try{
    ensureStyle();hydrateFrozenClientModel();hookContextSetter();const current=selectedContextEntry();syncContractNumberLabels(current);syncCompanyNameSpelling();syncCompanyControls();let count=0,currentId=norm(current?.context?.contract_id);
    for(const entry of state.entries.values()){if(currentId&&norm(entry.context?.contract_id)===currentId){if(renderEntry(entry))count++}else clearNonCurrentEntry(entry)}
    document.documentElement.dataset.ronaClientContractDownloads=String(count);document.documentElement.dataset.ronaClientContractRuntime='v4-current-context';if(window.__RONA_CLIENT_CONTRACT_DOWNLOAD_STATE__)window.__RONA_CLIENT_CONTRACT_DOWNLOAD_STATE__.current_contract_id=current?.context?.contract_id||null;return true;
...
function start(){
  startObserver();hookContextSetter();const authority=contextAuthority();if(!authority){console.error('RONA contract download: context authority unavailable');return}
  state.unsubscribe=authority.subscribe(ctx=>{state.currentKey=norm(ctx?.contract_id);refresh(true)});refresh(true);
