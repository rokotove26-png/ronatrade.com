(()=>{
  ...
  const MARK='20260902-client-background-section-preload-core-only-v3';
  ...
  const RETRY_MS=2500;
  const state={version:MARK,running:false,rerun:false,timer:0,retry:0,cycle:0,lastStartedAt:null,lastCompletedAt:null,lastReason:null,contexts:[],cache:{},sections:{},errors:[]};
  ...
  function publish(reason){
    const detail={version:MARK,reason,cycle:state.cycle,last_completed_at:state.lastCompletedAt,contexts:state.contexts.map(c=>({client_id:norm(c?.client_id),contract_id:norm(c?.contract_id)})),sections:JSON.parse(JSON.stringify(state.sections))};
  ...
  async function read(path,{allowDisabled=false}={}){
    ...
    try{response=await fetch(API+path,{credentials:'same-origin',cache:'no-store',headers:{accept:'application/json','x-rona-background-preload':'v3-core-only'}})}
  ...
  function contextRows(entry){return Array.isArray(entry?.body?.data?.contexts)?entry.body.data.contexts:[]}
  ...
  async function cycle(reason='timer'){
    ...
    try{
      const boot=await read('/v1/client/bootstrap');state.contexts=contextRows(boot);markSection('company_contract','READY',{contexts:state.contexts.length});
      ...
      const allDetails=contexts.every(x=>x.detail_ok),allPrices=contexts.every(x=>x.prices_ok),empty=state.contexts.length===0;
      for(const name of ['home','applications','deals','documents','payments'])markSection(name,empty?'READY_EMPTY':allDetails?'READY':'DEGRADED',{contexts:state.contexts.length});
      ...
      for(const row of contexts)if(!row.ok)state.errors.push({scope:'context',key:row.key,detail_error:row.detail_error,prices_error:row.prices_error});
