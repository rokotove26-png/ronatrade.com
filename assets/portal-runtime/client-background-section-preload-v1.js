(()=>{
  ...
  const state={version:MARK,mode:'LAZY_BY_SECTION',running:false,cycle:0,lastStartedAt:null,lastCompletedAt:null,lastReason:null,currentContext:null,cache:{},sections:{},errors:[],unsubscribe:null};
  ...
  function publish(reason){
    ...
    for(const name of ['home','applications','deals','documents','payments','prices','market','rail'])markSection(name,selected?'DEFERRED_BY_SECTION':'READY_EMPTY',{context_selected:selected,lazy_load:true});
  ...
  function setContext(next,reason){const before=contextKey(state.currentContext),after=contextKey(next);state.currentContext=next||null;if(before!==after){state.cache={};window.__RONA_CLIENT_BACKGROUND_CACHE__=state.cache}publish(reason)}
  function start(){
    const authority=contextAuthority();
    if(!authority){state.errors.push({scope:'context-authority',error:'CLIENT_CONTEXT_AUTHORITY_UNAVAILABLE'});publish('authority-unavailable');return}
    state.unsubscribe=authority.subscribe(next=>setContext(next,'context-change'));
    const current=authority.getCurrentContext?.();if(current)setContext(current,'open');else publish('open');
    window.addEventListener('pageshow',()=>publish('pageshow'),{passive:true});
  }
