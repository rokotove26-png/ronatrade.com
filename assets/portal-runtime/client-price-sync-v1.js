...
async function request(path,init={}){
  const headers={accept:'application/json','x-rona-client-source':'client-price-sync-v1:prices',...(init.headers||{})};
...
function emitPriceUpdate(reason){try{window.dispatchEvent(new CustomEvent('rona:client-prices-updated',{detail:{reason,context:state.context?{client_id:state.context.client_id||'',contract_id:state.context.contract_id||''}:null,count:state.prices.length,loaded_at:state.priceLoadedAt?new Date(state.priceLoadedAt).toISOString():null}}))}catch{}}function clearPriceState(){state.prices=[];state.priceContextKey='';state.priceLoadedAt=0;state.renderSignature='';window.__RONA_CLIENT_PRICE_SYNC_STATE__={context:state.context,prices:[],authority:'SERVER_AUTHORITATIVE_PRICE_PROJECTION',loadedAt:null};emitPriceUpdate('cleared')}
function publishPriceState(next,source){state.prices=Array.isArray(source)?source:[];state.priceContextKey=contextKey(next);state.priceLoadedAt=Date.now();window.__RONA_CLIENT_PRICE_SYNC_STATE__={context:next,prices:state.prices,authority:'SERVER_AUTHORITATIVE_PRICE_PROJECTION',loadedAt:new Date().toISOString()};emitPriceUpdate('loaded')}
...
function maybeRefresh(event){if(isPriceInteraction(event.target)){queueRender();if(!state.prices.length||Date.now()-state.priceLoadedAt>PRICE_TTL)scheduleRefresh(false)}}
function start(){
  observePriceMount();
