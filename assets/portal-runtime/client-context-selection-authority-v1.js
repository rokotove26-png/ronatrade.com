(()=>{'use strict';
const MARK='20260902-client-context-selection-authority-v1';
...
function exposeSelection(){
  ...
  d.dataset.ronaClientContextSelection=state.selected?'selected':'required';
  if(state.selected){
    d.dataset.ronaClientId=state.selected.client_id;d.dataset.ronaContractId=state.selected.contract_id;
...
function emitSelection(source){
  ...
  window.dispatchEvent(new CustomEvent('rona:client-context-changed',{detail:{client_id:ctx?.client_id||'',contract_id:ctx?.contract_id||'',display_name:ctx?companyDisplayName(ctx):'',source:source||'SERVER_SESSION_AUTHORITY'}}));
}
function setSelected(ctx,source,emit=true){
  const next=ctx?authorized(ctx):null;const before=key(state.selected),after=key(next);
  state.selected=next;exposeSelection();scheduleSync();
  if(emit&&before!==after)emitSelection(source);
  return next;
}
...
function syncSelect(){
  const select=document.getElementById('clientContextSelect');if(!select||!state.contexts.length)return;
  const prior=state.selected||resolveSelection();if(prior)state.selected=prior;
  const counts=labelCounts();const selectedKey=key(state.selected);
  const desired=[];
  if(state.contexts.length>1&&!state.selected)desired.push({value:'',text:'Выберите компанию',disabled:true,selected:true,ctx:null});
  for(const ctx of state.contexts){const display=companyDisplayName(ctx),duplicate=counts.get(display)>1;desired.push({value:ctx.contract_id,text:duplicate?display+' · '+compactContract(ctx):display,disabled:false,selected:key(ctx)===selectedKey,ctx})}
  ...
  if(JSON.stringify(current)!==JSON.stringify(target)){
    ...
    for(const item of desired){const option=new Option(item.text,item.value,item.selected,item.selected);option.disabled=item.disabled;if(item.ctx){option.dataset.clientId=item.ctx.client_id;option.dataset.contractId=item.ctx.contract_id;option.title=item.ctx.legal_name+(item.ctx.current_external_contract_number?' · '+item.ctx.current_external_contract_number:'')}frag.appendChild(option)}
...
function syncContextScope(scope,ctx){
  ...
  for(const leaf of scope.querySelectorAll('*')){
    ...
    if(contractPos>0&&(CLIENT_ONE.test(after)||CONTRACT_ONE.test(after))){const tail=after.slice(contractPos);after=display+' · '+tail}
...
function syncVisualContext(){
  const ctx=state.selected;if(!ctx)return;
  for(const scope of contextScopes())syncContextScope(scope,ctx);
  syncHeader(ctx);
}
...
function publish(raw){
  ...
  const resolved=previous&&authorized(previous)||resolveSelection();state.selected=resolved;
  exposeSelection();scheduleSync();
  if(key(previous)!==key(state.selected))emitSelection(state.contexts.length===1?'single-authorized-context':'authoritative-selector');
  return true;
}
async function capture(response){try{if(!response?.ok)return;const body=await response.clone().json();if(body?.ok!==false)publish(body?.data?.contexts)}catch(_){}}
async function ensure(){
  if(state.ready)return state.contexts;if(state.loading)return state.loading;
  state.loading=(async()=>{const known=window.RONA_CLIENT_SERVER_CONTEXT?.contexts;if(Array.isArray(known)&&known.length&&publish(known))return state.contexts;const response=await nativeFetch(BOOT,{credentials:'same-origin',cache:'no-store',headers:{accept:'application/json'}});if(!response.ok)throw new Error('CLIENT_BOOTSTRAP_HTTP_'+response.status);const body=await response.json();if(body?.ok===false||!publish(body?.data?.contexts))throw new Error('CLIENT_CONTEXT_NOT_AUTHORIZED');return state.contexts})().finally(()=>{state.loading=null});
...
window.fetch=async function(input,init){
  ...
  const hasContext=url.searchParams.has('clientId')||url.searchParams.has('contractId');
  if(hasContext){
    await ensure().catch(()=>{});if(!state.selected){const dom=resolveSelection();if(dom)setSelected(dom,'selector',true)}
    if(state.selected){url.searchParams.set('clientId',state.selected.client_id);url.searchParams.set('contractId',state.selected.contract_id);const next=raw.startsWith('http://')||raw.startsWith('https://')?url.href:url.pathname+url.search+url.hash;return nativeFetch(next,init)}
    if(state.contexts.length>1)throw new Error('CLIENT_CONTEXT_SELECTION_REQUIRED');
  }
  return nativeFetch(input,init);
};
function onChange(event){
  const select=event.target?.closest?.('#clientContextSelect');if(!select)return;
  const ctx=contextByValue(select.value,select.selectedOptions?.[0]||null);setSelected(ctx,'selector',true);
}
function startObserver(){
  if(state.observer||!document.body)return;state.observer=new MutationObserver(()=>scheduleSync());state.observer.observe(document.body,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['value','data-client-id','data-contract-id']});scheduleSync();
}
function start(){
  document.addEventListener('change',onChange,true);window.addEventListener('pageshow',scheduleSync,{passive:true});window.addEventListener('rona:client-server-context-ready',()=>ensure().then(scheduleSync).catch(()=>{}));startObserver();ensure().then(()=>{startObserver();scheduleSync()}).catch(error=>console.error('RONA client context selection authority',error));
}
