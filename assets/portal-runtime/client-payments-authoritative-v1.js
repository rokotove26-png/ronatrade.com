...
const state={activeKey:'',detail:null,ctx:null,loading:false,reloadRequested:false,reloadForce:false,lastLoad:0,timer:0,observer:null,scheduled:false,unsubscribe:null};
...
async function projection(force=false){const authority=contextAuthority();if(!authority)throw new Error('CLIENT_CONTEXT_AUTHORITY_UNAVAILABLE');if(force&&authority.refreshCurrentProjection)return authority.refreshCurrentProjection('payments-authoritative');return authority.whenCurrentProjection?.('payments-authoritative')}
...
function render(detail,ctx){
  ...
  window.dispatchEvent(new CustomEvent('rona:client-payments-rendered',{detail:{client_id:norm(ctx?.client_id),contract_id:norm(ctx?.contract_id)}}));
  return true;
}
...
async function load(force=false){
  ...
  state.loading=true;
  try{
    const detail=await projection(force);
  ...
  }catch(error){console.error('RONA client payments projection',error);renderLoadingError('Актуальные платежные данные временно недоступны.');ready(false)}finally{state.loading=false;if(state.reloadRequested){const queuedForce=state.reloadForce;state.reloadRequested=false;state.reloadForce=false;queueMicrotask(()=>load(queuedForce))}}
}
...
function start(){
  ...
  schedule(false);
  window.addEventListener('rona:client-current-projection',()=>{const next=contextAuthority()?.getCurrentProjection?.(),ctx=contextAuthority()?.getCurrentContext?.();if(next&&ctx){state.activeKey=contextKey(ctx);state.detail=next;state.ctx=ctx;state.lastLoad=Date.now();render(next,ctx);ready(true)}},{passive:true});
  window.addEventListener('pageshow',()=>load(false),{passive:true});
  document.addEventListener('click',e=>{const t=norm(e.target?.textContent);if(t.includes('Платежи'))setTimeout(()=>load(false),120)},true);
}
