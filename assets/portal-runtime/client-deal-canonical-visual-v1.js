(()=>{
'use strict';
const MARK='20260829-client-deal-canonical-visual-v1';
if(window.__RONA_CLIENT_DEAL_CANONICAL_VISUAL__===MARK)return;
window.__RONA_CLIENT_DEAL_CANONICAL_VISUAL__=MARK;
if(location.pathname!=='/portal/client')return;

const PANEL='rona-deal-documents-v3';
const HOST='rona-deal-card-v3';
const STYLE_ID='rona-client-deal-canonical-visual-v1-style';
...
.${PANEL}__action--upload{overflow:hidden!important;border-color:rgba(248,113,113,.70)!important;background:linear-gradient(105deg,#7d1c2a,#b82e3b,#7d1c2a)!important;background-size:220% 100%!important;box-shadow:0 6px 18px rgba(127,29,29,.28),0 0 0 1px rgba(239,68,68,.06),inset 0 1px 0 rgba(255,255,255,.10)!important;animation:ronaSignedDsCanonicalPulse 2.4s ease-in-out infinite,ronaSignedDsCanonicalFlow 4s linear infinite!important;color:#fff5f5!important}
.${PANEL}__action--upload::before{content:'↑';background:rgba(255,255,255,.10);border-color:rgba(254,202,202,.32);color:#fff1f2}
.${PANEL}__action--upload::after{content:'';position:absolute;top:-35%;bottom:-35%;left:-48%;width:34%;transform:skewX(-18deg);background:linear-gradient(90deg,transparent,rgba(255,255,255,.16),transparent);animation:ronaSignedDsCanonicalSweep 3.1s ease-in-out infinite;pointer-events:none}
...
@keyframes ronaSignedDsCanonicalFlow{0%{background-position:100% 0}100%{background-position:-100% 0}}
@keyframes ronaSignedDsCanonicalPulse{0%,100%{box-shadow:0 6px 18px rgba(127,29,29,.22),0 0 0 1px rgba(239,68,68,.05)}50%{box-shadow:0 8px 24px rgba(127,29,29,.38),0 0 18px rgba(239,68,68,.22)}}
@keyframes ronaSignedDsCanonicalSweep{0%,24%{left:-48%;opacity:0}40%{opacity:1}62%{left:116%;opacity:0}100%{left:116%;opacity:0}}
...
`;
  ...
  document.documentElement.dataset.ronaDealVisual='canonical-compact-v1';
}
