...
const state={activeIds:new Set(),dealStates:new Map(),ready:false,loading:false,reloadRequested:false,reloadForce:false,lastLoad:0,timer:0,observer:null,contextKey:'',unsubscribe:null};
...
function dealFromServer(row){
  ...
  return{dealId,statusCode,statusLabel,paymentCode,paymentLabel,resourceCode,resourceLabel,businessStatus:norm(row?.business_status),paymentReceived:row?.payment_received_amount??null,paymentObligation:row?.payment_obligation_amount??null,paymentCurrency:norm(row?.payment_currency),paymentPercent:row?.payment_percent??null,statusSource:norm(row?.status_source),paymentSource:norm(row?.payment_source),resourceSource:norm(row?.resource_source)};
}
function applicationIsActive(app){
  const linkedDeal=norm(app?.deal_id||app?.dealId||app?.linked_deal_id||app?.linkedDealId),status=norm(app?.status||app?.current_status).toUpperCase();
...
async function loadAuthoritativeState(force=false){
  if(state.loading){state.reloadRequested=true;state.reloadForce=state.reloadForce||force;return;}
  let ctx=null;
  ...
  state.loading=true;
  try{
    const authority=contextAuthority();const detail=force&&authority?.refreshCurrentProjection?await authority.refreshCurrentProjection('application-lifecycle'):await authority?.whenCurrentProjection?.('application-lifecycle');
    if(contextKey(contextAuthority()?.getCurrentContext())!==key)return;
    if(!detail)throw new Error('CLIENT_CURRENT_PROJECTION_UNAVAILABLE');
  ...
  finally{state.loading=false;if(state.reloadRequested){const queuedForce=state.reloadForce;state.reloadRequested=false;state.reloadForce=false;queueMicrotask(()=>loadAuthoritativeState(queuedForce))}}
}
...
function syncApplicationCounter(root,ids){if(!state.ready)return;let count=0;for(const id of ids){const row=rowFor(root,id,ids,['Принята','Принято','Открыть','Сделка','DEAL-']);if(row&&state.activeIds.has(id)&&visible(row))count++}const next=`${count} зарегистрировано`;for(const el of root.querySelectorAll('*'))if(el.childElementCount===0&&/^\d+\s+зарегистрировано$/iu.test(norm(el.textContent))&&norm(el.textContent)!==next)el.textContent=next}
function paymentTitle(deal){const r=Number(deal.paymentReceived),o=Number(deal.paymentObligation),c=deal.paymentCurrency;if(Number.isFinite(r)&&Number.isFinite(o)&&o>0&&c)return`Получено ${r.toLocaleString('ru-RU',{maximumFractionDigits:2})} из ${o.toLocaleString('ru-RU',{maximumFractionDigits:2})} ${c}`;return 'Источник: серверная финансовая проекция'}
...
function syncDealStateStrip(row,deal){
  ...
  if(strip&&strip.getAttribute('data-rona-deal-state-strip')!=='authoritative-v8')strip.setAttribute('data-rona-deal-state-strip','authoritative-v8');if(!strip){strip=document.createElement('div');strip.setAttribute('data-rona-deal-state-strip','authoritative-v8')}
  const summarySide=row.querySelector('[data-rona-deal-summary-side]');if(summarySide){const open=summarySide.querySelector('button,a,[role="button"]');if(strip.parentElement!==summarySide)summarySide.insertBefore(strip,open||null)}else if(!strip.parentElement){const first=stale[0]||legacy[0]||null;if(first?.parentElement)first.parentElement.insertBefore(strip,first);else{const docs=[...row.querySelectorAll('*')].find(el=>el.childElementCount===0&&norm(el.textContent)==='Документы');if(docs?.parentElement)docs.parentElement.insertBefore(strip,docs);else row.append(strip)}}
  ...
  if(strip.dataset.ronaStateSignature!==signature){strip.replaceChildren(chip(deal.statusLabel,deal.statusCode,'status',deal.statusSource||'Источник: серверная операционная проекция','data-rona-current-deal-status'),chip(deal.paymentLabel,deal.paymentCode,/PAID|CONFIRMED|PARTIAL/i.test(deal.paymentCode)?'payment':'attention',paymentTitle(deal),'data-rona-finance-payment-state'),chip(deal.resourceLabel,deal.resourceCode,/CONFIRMED|EXECUT/i.test(deal.resourceCode)?'resource':'attention',deal.resourceSource||'Источник: серверная ресурсная проекция','data-rona-operations-resource-state'));strip.dataset.ronaStateSignature=signature}
}
...
document.addEventListener('change',()=>schedule(40),true);
...
window.addEventListener('rona:client-current-projection',()=>{if(operationsPageVisible())loadAuthoritativeState(false)},{passive:true});
